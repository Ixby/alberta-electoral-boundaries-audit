...
This audit evaluates two competing 2026 electoral boundary proposals against the 2019 baseline currently in force in Alberta. Six dimensions are examined using public data and identical methodology applied to all three maps: (A) population equality, (B) partisan-bias metrics from the political-science literature, (C) visual geographic coherence, (D) procedural fairness, (4) geometric data provenance, and (5) MCMC ensemble comparison. Where data permit, the two 2026 proposals are shown to diverge systematically: the minority proposal exhibits wider population dispersion (MAD 4,707 vs 3,180), a 12.2% Calgary geographic-zone asymmetry vs the majority's 0.4%, three visible spatial anomalies flagged by the commission chair himself, and fragmentation of Airdrie across four electoral divisions vs the majority's two. On partisan-bias magnitude, the audit reports **two measurements at different spatial resolutions, which disagree on sign**: a blended crosswalk approach (urban-core-plus-rural-absorption at w = 0.85) yields a **1.42 pp UCP-favorable asymmetry** for the minority map under 2023 vote input, while a higher-resolution VA-level spatial-attribution approach using Derived Provisional Geometries (DPG; see §4.1.4) yields a **4.1 pp NDP-favorable asymmetry**. Both measurements are internally consistent; the disagreement is a property of measurement resolution, not a pipeline error, and is discussed as a primary methodological finding in §5.2.5. A rationale-validation pass against the minority's 25 inventoried justifications finds six of the seven contested configurations have cleaner statutory-compliant alternatives the minority did not take (full validation at `analysis/methodology/v0_1_minority_rationales_validation.md`; five of the minority's specific population-math justifications fail against the census and the statutory population bands). All partisan-bias magnitudes remain within the 7% efficiency-gap threshold proposed by Stephanopoulos & McGhee (2014/2015); the Supreme Court of the United States vacated *Gill v. Whitford*, 585 U.S. ___ (2018), and remanded on standing grounds without adopting the 7% threshold as a judicial standard. The directional consistency of the minority's structural-asymmetry signals across five independent non-partisan-bias dimensions (population, Calgary zone, visible anomalies, rationale-failures, Airdrie fragmentation) — and the procedural departure of April 16, 2026 rejecting the majority recommendation in favor of a UCP-majority MLA committee drafting process — together support a finding of systematic structural asymmetry at a magnitude below the US statistical-significance threshold, with the partisan-bias direction remaining sensitive to measurement resolution until Elections Alberta releases official 2026 shapefiles. Sections 4 (geometry) and 5 (MCMC ensemble) rely on DPG and will be re-run within 48 hours of official shapefile release (sunset clause, §4.1.4).
...
Three modelling-uncertainty tests materially narrow the partisan-bias magnitude claim while leaving structural findings intact. These are reported up-front for transparency; the underlying methodology is in `analysis/reports/v0_1_design_critique.md` and the Monte Carlo script `analysis/scripts/v0_3_monte_carlo_ci.py`.
...
**Dataset-construction consequences of cycle lag (three-vintage sandwich).** Beyond the verdict-robustness property above, the 4–14-year lag between commission data-basis and boundary retirement materially shapes *how* the audit's upstream pipelines construct their datasets. Any reconstruction of the commission's arithmetic has to reconcile three input vintages simultaneously: (i) Statistics Canada 2021 dissemination-area geometry carrying 2021 census populations — the atomic unit used for Phase 4B DA-overlay; (ii) the commission's 2024 TBF estimate for each 2026 proposed ED — the operative published value; and (iii) the 2023 Voting Area geometry carrying 2023 votes — the spatial substrate for Phase 4C partisan-bias attribution. The ratio between (i) the 2021-population-sum-inside-a-2026-DPG-polygon and (ii) the commission's 2024 estimate of the same territory is *not* a clean scalar: Alberta's 14.69 % provincial growth between 2021 and 2024 was distributed unevenly (Calgary-ring cities such as Airdrie and Chestermere grew 20–30 %, rural territories grew 0–5 %), so applying a flat growth factor to a regionally-heterogeneous population surface introduces a structural bias into Phase 4F's per-ED validation deltas. The 81/86 majority and 87/89 minority hardstop failures documented in `data/INTEGRITY_STATUS.md` are therefore a composite signal: real DPG transcription error plus cycle-lag growth heterogeneity. Disentangling the two without official 2026 shapefiles is not tractable from public data; the §4.1.4 sunset clause binds the audit to recomputation if Elections Alberta releases the shapefiles, at which point real geometric error would be distinguishable from cycle-lag artifact. Full treatment at `analysis/reports/v0_1_cycle_lag_commentary.md` (§1).
...
**Both measurements are internally consistent.** Both are bracketed by Monte Carlo on their own dominant uncertainty (crosswalk: urban-weight + rural-baseline + per-hybrid jitter, 95 % CI crosses zero at the 7.0 % samples-with-opposite-sign point; spatial: DPG polygon-tier uncertainty, not yet Monte-Carlo-bracketed). Neither is the "true" answer at the level of resolution available to a public audit without the official 2026 shapefiles.
...
**Multi-chain convergence update (2026-04-23, Gemini Phase D.2 response).** A three-chain ReCom run with independently-seeded chains (seeds 42, 101, 2024; 50,000 ReCom proposals per chain; 10 % burn-in; script `analysis/scripts/v0_1_mcmc_multichain_ensemble.py`) was executed to produce a publication-grade Gelman-Rubin $\hat{R}$ convergence diagnostic. Per-metric split-chain $\hat{R}$ values are **efficiency gap 1.014, mean-median 1.013, declination 1.016, seats at 50/50 1.013** — all comfortably within the conventional $\hat{R} < 1.05$ acceptability threshold and close to the strict $\hat{R} < 1.01$ criterion (Gelman et al., 2013, *Bayesian Data Analysis*, 3rd ed., ch. 11). **Cross-chain convergence is therefore confirmed**, a diagnostic the single-chain run could not provide. Combined ESS across the three chains is 288–350 per metric (per-chain ESS 61–156; integrated autocorrelation time τ ≈ 289–732 per chain) — an approximate $2\times$ improvement over the single-chain ESS ~150, but still below the 1,000-draw MGGG target for lawsuit-grade percentile claims. Under the combined ensemble, the explicit tail-downgrade bounds reported above remain valid: **p100 and p1.6 claims are bounded to p95.35 / p2.5 at the chain's combined effective precision**, and the ~p90 Minority seats-at-50/50 flag remains retracted. Reviewers demanding ESS > 1,000 can reproduce with `--steps 150000` (est. 75 min); the paper does not change conclusions under that extension because the banded flags rest on the p ≤ 5 / p ≥ 95 side of the downgrade interval, not on finer decimal placements. Full convergence diagnostic at `data/v0_1_mcmc_multichain_rhat.json` and `data/v0_1_mcmc_multichain_summary.md`; raw samples at `data/v0_1_mcmc_multichain_samples.csv` (135,000 post-burn-in rows) and thinned-pooled at `data/v0_1_mcmc_multichain_pooled.csv`.
