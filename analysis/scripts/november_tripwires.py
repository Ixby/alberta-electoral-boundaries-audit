...
def run_tripwires(eds_gdf_path, cities_gdf_path=None):
    ...
    if eds.crs is None or eds.crs.is_geographic:
        print(
            "Warning: Map is not projected. Reprojecting to Alberta 10-TM for area calculations."
        )
        eds = eds.to_crs(epsg=3401)  # Alberta 10-TM
    elif eds.crs.to_epsg() != 3401:
        raise ValueError(
            f"Expected shapefile to be in Alberta 10-TM (EPSG:3401), but found EPSG:{eds.crs.to_epsg()}. November map must use authoritative CRS."
        )
    ...
    if cities_gdf_path and os.path.exists(cities_gdf_path):
        ...
        cities = cities[
            cities["name"].str.contains("|".join(target_cities), case=False, na=False)
        ]
        ...
        for _, city in cities.iterrows():
            ...
            intersections["overlap_area"] = intersections.geometry.intersection(
                city_geom
            ).area
            intersections["overlap_pct"] = (
                intersections["overlap_area"] / city_geom.area
            )
            ...
            if num_splits > expected_seats:
                print(
                    f"  [RED ALERT] {city_name} is split into {num_splits} districts (Expected: {expected_seats})."
                )
                for _, ed_row in valid_splits.iterrows():
                    ed_name = ed_row.get("name_2026", ed_row.get("ed_name", "Unknown"))
                    print(
                        f"    - {ed_name} captures {ed_row['overlap_pct']*100:.1f}% of the city."
                    )
            else:
                print(f"  [PASS] {city_name} is kept intact ({num_splits} districts).")
    else:
        print(
            "  (Skipping Mid-Sized City Integrity: No cities reference file provided.)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the November 91-seat GPKG or Shapefile",
    )
    parser.add_argument(
        "--cities",
        type=str,
        default=os.path.join(
            ROOT, "data", "shapefiles", "reference", "alberta_2021_csds.gpkg"
        ),
        help="Path to reference cities GPKG",
    )
