...
def main():
    ...
    for label, ucp_share in shares.items():
        ...
        print(f"    EG:          ours={our['EG_ours']:+.6f}  gm={them['EG_gm']:+.6f}  "
              f"diff={our['EG_ours']-them['EG_gm']:+.2e}")
        print(f"    MM:          ours={our['MM_ours']:+.6f}  gm={them['MM_gm']:+.6f}  "
              f"diff={our['MM_ours']-them['MM_gm']:+.2e}")
        print(f"    Declination: ours={our['Declination_ours']:+.6f}  "
              f"gm(adj)={them['Declination_gm']:+.6f}  "
              f"diff={our['Declination_ours']-them['Declination_gm']:+.2e}")
        print(f"    PB:          ours={our['PB_ours']:+.6f}  gm={them['PB_gm']:+.6f}  "
    ...
    for key, label in eg_variants:
        vals = [results[m].get(key, float("nan")) for m in
                ["2019_enacted", "majority_2026", "minority_2026"]]
        print(f"  {label:<{col_w-2}} {vals[0]:>+10.4f} {vals[1]:>+10.4f} {vals[2]:>+10.4f}")
    ...
    for label in ["2019_enacted", "majority_2026", "minority_2026"]:
        r = results[label]
        print(f"  {label}: t_diff={r['t_diff_UCP_adj']:+.4f}(p={r['t_p']:.4f})  "
    ...
    for label in labels:
        ...
        for our_k, gm_k, name in [
            ...
            diff = r[our_k] - r[gm_k]
            md.append(f"| {name} | {label} | {r[our_k]:+.5f} | {r[gm_k]:+.5f} "
                      f"| {diff:+.2e} |")
    ...
    for label in labels:
        r = results[label]
        md.append(f"| {label} | {r['t_diff_UCP_adj']:+.4f} | {r['t_p']:.4f} "
