...
**Population concentration.** In north/east/central Calgary (the audit's Zone A; earlier text here said "northwest" — corrected 2026-07-08 per `findings/population_equality.md` §A2), the minority map's electoral zone has a mean population 11.5% above the provincial average — above the threshold derived from the Act's own ±25% band. The majority's equivalent zone sits 2.8% above average, inside the threshold. The difference is 8.7 percentage points on a metric whose threshold is anchored to the statute, not to any academic benchmark.
...
**Boundary anchoring (§5.8.5) — retracted on canonical geometry.** Pre-shapefile analysis (Derived Provisional Geometries) showed the majority anchoring 71.0% of its perimeter to municipal edges, the minority 14.5% — a 4.9× gap. On official Elections Alberta shapefiles (received 2026-05-06), both maps fall within the 70–85% Canadian comparator norm: majority 80.0%, minority 71.8%. The DPG-era values did not survive canonical recomputation and the finding is retracted. The three chair-flagged cartographic anomalies (§5.8.2) are unaffected by this correction.
...
**Partisan-bias metrics (§5.2).** Efficiency gap, mean-median difference, and declination are measured against a 1 million-plan MCMC neutral ensemble (official Elections Alberta shapefiles, canonical run). Under 2023 vote attribution and the Amendment 10 declination sign-convention correction (2026-06-12), the minority map is more UCP-favourable than the majority on all four metrics: mean-median +4.66 pp, seats@50/50 +5.62 pp, efficiency gap +3.92 pp, declination same direction (UCP-tail, p98.79). The majority map is within the neutral null on all four metrics.

**MCMC neutral-ensemble outlier test (§5.4, Ch1).** The minority map's four partisan metrics jointly produce Mahalanobis D = 5.80 against the canonical ensemble covariance (p = 8.80×10⁻⁷ parametric χ²₄; see referee #1 finding on the ESS-bounded empirical alternative). Against the 1 million-plan canonical ensemble (n_eff 1,413–1,522), three of four metrics individually exceed the 95th percentile: mean-median at p99.97, **declination at p98.79 (UCP-tail under the Amendment 10 sign-convention correction, 2026-06-12)**, and seats@50/50 at p99.99 (ESS-adjusted lower bound ≈p98, above p95 — flag reinstated). Efficiency gap at p94.54 remains below the threshold and is not flagged. The joint Mahalanobis result stands; the corrected declination sign means the four marginals now agree on direction. The majority sits at p0.99 mean-median (NDP-tail) and within the neutral null on the other three metrics.

**Combined-channel result (§5.4, headline = Bonferroni 2026-06-10, updated 2026-07-12 per a clean ensemble rerun — DOCUMENTED CORRECTIONS C10).** Ch1 (Mahalanobis, p = 8.80×10⁻⁷) and Ch2 (SZAT swing-zone bootstrap, p = 0.0025) are combined under the **Bonferroni dependence-robust upper bound p ≤ 1.76×10⁻⁶ (≈ 1 in 568,000)** — valid under arbitrary dependence between the channels. The earlier Fisher combination (p = 4.61×10⁻⁸) is anti-conservative under positive dependence (Brown 1975) and is retired as the headline figure; it appears in the methods section as the dependence-naive value for completeness. Both channels are seeded from publicly-verifiable drand League of Entropy beacon rounds predating the shapefile release. (Updated 2026-07-08: Ch2's p = 0.0024 is the i.i.d.-flip value and does not survive a contiguity-respecting block-permutation null — p = 0.1947, variance inflation 5.79×; see `findings/szat_block_permutation.md`. SZAT is retired as a confirmatory channel; the joint headline rests on Ch1 alone. The Bonferroni bound moved with Ch1's 2026-07-12 update — see above — since Ch1 is the binding term throughout.)

**Inter-map comparison test (§5.4, Ch1-COMP).** Registered at OSF yvc7g (filed 2026-05-10 — after shapefile receipt on 2026-05-06 but before test execution; the drand seed derives from beacon round 5500000, which predates the shapefiles — see `analysis/methodology/intermap_permutation_design.md` §AV6). Does the minority-majority gap exceed the distance between randomly chosen neutral-plan pairs? Version A (EG-only): p = 0.0340 against the canonical 1.01M ensemble (round-1 published p = 0.0303 against the 250k v0_9 ensemble). Version B (Mahalanobis joint): p = 9.999×10⁻⁵ (1/10,001). Both significant. The written prediction before running was that Version A would likely fail — it passed. The inter-map Mahalanobis distance (D = 7.17, canonical rescore 2026-06-12) exceeds each map's individual distance from the ensemble centroid (minority D = 5.72, majority D = 2.80), confirming the maps are positioned on opposite flanks of partisan-metric space rather than being co-located. (Corrected 2026-07-08: this paragraph previously carried the pre-rescore values D = 7.19 / 5.71 / 2.79 and p = 0.0303; canonical values per report_academic.md §5.4.9 and findings/joint_outlier_score.json.)

**Direction-of-travel (§5.4.10).** The 2019 enacted baseline sits at the statistical edge of neutral redistricting practice — Mahalanobis D²=13.16 against the canonical 1 million-plan ensemble (p=0.0105). The majority 2026 map retreats toward the ensemble interior (D²=7.74, p=0.102 — inside the null). The minority 2026 map amplifies away from it (D²=33.64, p=8.80×10⁻⁷). SZAT boundary-choice tests confirm the pattern: the 2019→majority transition is consistent with neutral redistricting (p=0.309); the 2019→minority transition is marginally outside the null CI (p=0.053). Both 2026 proposals were drawn by the same five commissioners from the same 2019 baseline; they moved in measurably opposite directions on the partisan-metric axes the ensemble measures.
...
| Municipal-boundary anchoring *(retracted — canonical)* | 80.0% of perimeter | 71.8% of perimeter | Both within 70–85% Canadian norm; DPG-era values 71%/14.5%/4.9× did not survive shapefile recomputation |
...
*Note on why the 1 million-plan Ch1 p-value is less extreme than the 100k run:* as the ensemble grew from 100k to 1 million plans (n_eff 379 → 1,413 for the minimum metric), the Mahalanobis covariance matrix was estimated more precisely. The better-calibrated covariance shows the minority map is somewhat less extreme than the noisier 100k estimate suggested — Ch1 p moved from 1.60×10⁻⁷ to 8.80×10⁻⁷ (the 1M figure itself further updated 2026-07-12 after a clean ensemble rerun; see DOCUMENTED CORRECTIONS C10 in the academic report). This is a calibration effect, not a weakening of the finding. The dependence-naive Fisher combined p (4.61×10⁻⁸) is retained for methodology completeness but is not the headline; the Bonferroni-bounded headline (p ≤ 1.76×10⁻⁶) is the operative value. Individual metric percentiles (mean-median p99.97, seats@50/50 p99.99) are stronger, not weaker, in the 1 million-plan run.
