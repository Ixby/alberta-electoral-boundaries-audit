...
- **Highways** — OSM ways with `highway ∈ {motorway, trunk, primary, secondary}` (same major-road classes used by the v0_1 Track Y refinement in `analysis/methodology/shape_refinement.md`). Tertiary excluded — intra-city street grids would inflate every map's score uniformly and add no discriminatory power. Province-wide pull via Overpass: 41,475 ways, ~80,000 km.
...
3 maps × 2 substrates = 6 numbers. CSD column is reproduced from `data/v0_1_municipal_anchoring_summary.json` (majority/minority) and `data/2019_municipal_anchoring_summary.json` (2019).
...
- **Dangerzone metric definitions (`dangerzone_metric_definitions.md`)** — the municipal-anchoring axis is still the discriminating axis (this run shows the natural axis does not discriminate). No change needed to the ensemble scoring; the axis was correctly chosen.
...
Backward:
  ...
  data/2019_municipal_anchoring_summary.json
