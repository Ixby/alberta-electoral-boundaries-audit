...
sensitivity in `v0_1_tierC_parent_union_reference.csv`. It is **not** an
...
- `data/v0_1_majority_2026_populations.csv` — 89 majority 2026 EDs with an
  `is_hybrid` flag from the commission's own variance table.
- `data/v0_1_majority_hybrid_crosswalk.csv` — 19-row rename mapping (partial).
- `data/v0_1_minority_hybrid_crosswalk.csv` — 103-row crosswalk with `is_hybrid`
  flag and Jaccard-overlap match type from the minority appendix E.
- `data/v0_1_minority_hybrid_crosswalk_appendixE.csv` — 22-row Calgary/minority
  hybrid detail.
...
full table is in `data/v0_1_tierC_parent_union_reference.csv`. Key rows:
...
2. **Tier C parent-union reference is an upper bound only.** The union of
   ...
   sensitivity band in `v0_1_tierC_parent_union_reference.csv` is a rough
...
- `data/v0_1_compactness_scores.csv` — per-ED PP and Reock for all three maps;
...
- `analysis/v0_1_approximate_shape_analysis.py` — reproducible pipeline
- `analysis/v0_1_approximate_shape_analysis.md` — this document
