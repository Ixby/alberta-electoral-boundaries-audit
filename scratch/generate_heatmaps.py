...
def build_heatmap(map_path, out_png, name_col_guess="name_2026"):
    ...
    joined = gpd.sjoin(va_pts, eds[[name_col, "geometry"]], how="left", predicate="within")
    agg = joined.groupby(name_col).agg(ucp=("va_ucp", "sum"), ndp=("va_ndp", "sum")).reset_index()
    ...
    if still_missing.any():
        va_centroids = list(zip(
            va.geometry.centroid.x.values, va.geometry.centroid.y.values,
            va["va_ucp"].fillna(0).values, va["va_ndp"].fillna(0).values,
        ))
        for idx in eds.index[still_missing]:
            gc = eds.at[idx, "geometry"].centroid
            dists = [(((cx - gc.x) ** 2 + (cy - gc.y) ** 2), u, n) for cx, cy, u, n in va_centroids]
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("ucp_ndp_direct", [ndp_orange, ucp_blue], N=256)
    ...
    if "pop_2021" in va_render.columns:
        ...
        pop = va_two_party + va_render.get("va_other", pd.Series(0, index=va_render.index)).fillna(0)
    ...
    va_render["_fill"] = [_va_fill(s, w) for s, w in zip(va_render["parent_ucp_share"], weight.values)]
...
if __name__ == "__main__":
    ...
    if not eds_2019.exists():
        ...
        files = list((ROOT / "data").rglob("*2019*.shp")) + list((ROOT / "data").rglob("*2019*.gpkg"))
        if files:
            eds_2019 = files[0]
