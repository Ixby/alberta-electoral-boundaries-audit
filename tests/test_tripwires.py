...
sys.path.append(
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "analysis", "scripts")
    )
)
...
def test_drain_pattern_detection(tmp_path, capsys):
    ...
    cities = gpd.GeoDataFrame(
        {"name": ["Medicine Hat (CY)"], "geometry": [city_poly]}, crs="EPSG:3401"
    )
    ...
    eds = gpd.GeoDataFrame(
        {"name_2026": ["ED1", "ED2", "ED3"], "geometry": [ed1, ed2, ed3]},
        crs="EPSG:3401",
    )
    ...
    eds_valid = gpd.GeoDataFrame(
        {"name_2026": ["ED4", "ED5"], "geometry": [ed4, ed5]}, crs="EPSG:3401"
    )
