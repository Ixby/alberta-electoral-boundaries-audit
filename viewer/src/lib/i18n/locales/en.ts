...
export default {
	...
	stakes: {
		...
		q2: {
			...
			body:
				"There isn't one. The Canadian test is different: whether the boundaries give voters <em>effective representation</em> under section 3 of the Charter. The minority proposal raises serious questions under it; only a judge can answer them definitively, and no one has asked one yet."
		},
		q3: {
			heading: "What's at stake for Albertans?",
			body:
				"At a 50/50 provincial vote, the audit's measurements place the minority proposal at a structural extreme: fewer than 100 of the 1.01 million neutral comparison maps produce the same kind of seat imbalance. The imbalance matters because at 58 of 87 seats (a two-thirds supermajority) the governing party unlocks extraordinary procedural powers. It can waive standard notice periods and push public bills through multiple legislative stages in a single day, bypassing deliberation checks normally constraining it. Whether the minority proposal's tilt is large enough to push one party past the 58-seat threshold at vote shares <em>other</em> than 50/50 is a question this audit has not yet tested. Whether the imbalance is acceptable is a question for Albertans, not for this audit."
		},
	...
	boundary: {
		...
		can_3:
			'These results are consistent with maps producing strong partisan effects, and inconsistent with what the random comparison set produces.',
		cant_1:
			'The audit does <strong>not</strong> establish any commissioner intended the partisan effects it measures. Boundary geometry cannot reveal intent.',
		cant_2:
	...
	editorial_intro: {
		...
		p1: 'Every voter in Alberta lives in an <em>electoral district</em> — a slice of the province electing one person to the legislature. There are 87 districts. Each district elects one MLA. When you cast a ballot in a provincial election, you are choosing the MLA for the district you live in. This is the entire connection most Albertans have to the legislature: one MLA, one district, one vote.',
		p2: 'Those district lines are not permanent. People move, neighbourhoods grow, rural areas thin out, cities sprawl. Every eight to ten years, Alberta is supposed to redraw the lines so each district is roughly the right size and reflects the way Albertans actually live now. The body doing the redrawing is the <em>Electoral Boundaries Commission</em> — an independent commission with judges, lawyers, and public members, not politicians.',
	...
	editorial_reflect: {
		...
		region_p3:
			'What it <strong>does</strong> say: in several places on the minority proposal, rural communities are being attached as the <em>tail</em> of a district whose population centre sits in a city. Look at how the minority proposal handles Airdrie — a city of about 74,000 sliced into four districts, each one extended out into a different stretch of rural countryside. The population centre of each new district is the urban slice, not the rural tail. An MLA elected from such a district is most likely to live, campaign, and prioritise where the votes are — which means rural communities formerly represented by a dedicated rural MLA become the back half of an urban-led seat. The pattern repeats on the minority proposal in ways it does not on the majority proposal.',
		region_p4:
		...
		province_p:
			"The legislature is what you get when you sum every district's answers to the three questions above. If most districts are anchored to communities whose MLAs actually live in them, the legislature represents those communities. If most districts have rural tails attached to urban heads, the legislature represents the heads — and the tails get whatever attention is left over. The partisan question — which party wins a majority — is downstream of it. The supermajority question — whether one party crosses 58 of 87 seats and unlocks procedural shortcuts like waiving notice periods or accelerating bills through multiple stages in a single day — is downstream of <em>this</em>. At a hypothetical 50/50 provincial split, the audit's measurements place the minority proposal at a structural extreme: fewer than 100 of the 1.01 million neutral comparison maps produce the same kind of seat imbalance. Whether the imbalance pushes a party past 58 seats at the vote shares Albertans actually deliver is a question this audit has not yet directly tested; the opening questions at the top of this page are honest about the gap. Whether the answer to any of these questions matters enough to act on is, again, a question for you."
	},
	editorial_history: {
		heading: 'Context: A short history of gerrymandering',
		p1: 'The word comes from 1812. Massachusetts governor Elbridge Gerry signed off on a state-senate map whose districts were so contorted to favour his party a Boston cartoonist drew one of them as a salamander — wings, claws, a forked tongue. The cartoonist\'s pun, <em>Gerry-mander</em>, stuck. The shape stuck too: two centuries later, the word still means drawing electoral lines to engineer a partisan outcome.',
		...
		p3: '<strong>The United States</strong> treats partisan gerrymandering as a problem the federal courts mostly cannot fix. In <em>Rucho v. Common Cause</em> (2019), the U.S. Supreme Court ruled partisan gerrymanders are "political questions" outside its jurisdiction. Some states (California, Michigan) have responded by creating independent citizen commissions to draw their own lines; others (Texas, North Carolina) have continued to draw openly partisan maps and defended them on the basis <em>Rucho</em> permits it.',
	...
	glossary: {
		...
		'electoral-district': {
			...
			definition:
				'The geographic area electing one member to the provincial legislature. Often shortened to "ED" after first use. Each ED has one MLA. (The word "riding" usually refers to federal districts; in the provincial context the proper term is electoral district.)'
		},
		riding: {
			term: 'Riding',
			definition:
		...
		cracking: {
			term: 'Cracking',
			definition:
				"A gerrymandering technique splitting a voting bloc across many districts so it never reaches a majority in any single one. For example, dividing a city across four ridings so its voters are outnumbered in each."
		},
		packing: {
			term: 'Packing',
			definition:
				"A gerrymandering technique concentrating one party's voters into a small number of districts. The party wins those districts overwhelmingly but \"wastes\" many votes — leaving fewer of its voters available to compete in other districts."
		},
		draining: {
			term: 'Draining',
			definition:
		...
		'charter-s3': {
			...
			definition:
				'The section of the Canadian Charter of Rights and Freedoms guaranteeing citizens the right to vote. Canadian courts have interpreted it not as a strict "one person, one vote" rule but as a right to "effective representation."'
		},
		...
		ebc: {
			...
			definition:
				"The body drawing Alberta's provincial electoral boundaries under the EBCA. The 2026 commission was chaired by Justice Miller and split 3–2 among its commissioners, producing two competing proposals (the majority and minority proposals) rather than a single recommendation."
		},
		'lunty-committee': {
			term: 'Lunty committee',
			definition:
				"An MLA committee chaired by Brandon Lunty — an MLA appointed by the Premier — choosing between the EBC's majority and minority proposals before the November 2026 deadline. The committee is separate from the EBC; the legislature created it for this specific decision and it is not part of the standard EBCA process."
		},
		ebca: {
			term: 'EBCA',
			definition:
				'The Alberta Electoral Boundaries Commission Act — the law governing how electoral boundaries are drawn in the province. It sets up the commission, the public-hearing process, and the rules for when a new map takes effect.'
		},
		fsa: {
	...
	body: {
		...
		clean: {
			...
			intro_p2: "To answer this, the audit generated 1,010,000 computer-simulated, mathematically neutral Alberta maps using the official Elections Alberta shapefiles, holding to the exact same statutory rules and geographic boundaries the commission used. The commission's two 2026 maps were then placed into the distribution to see how normal they are. The simulation ran four independent chains of 252,500 steps each, with the base seed drawn from the Cloudflare drand beacon and pre-registered at OSF before execution.",
			...
			sub7_p1: 'The audit\'s central finding is geometric. <strong>Lane 2 — the structural-irregularity scorecard — is the foundation; Lane 1 is the proof the geometry is doing partisan work.</strong>',
			...
			details2_p: "The audit pre-registered five structural-irregularity tests on April 24, 2026 before the final simulation results were compiled. Anchoring is neutral for both maps; on the remaining four tests the minority crosses every one and the majority crosses none. Those measurements are geometric — they don't depend on any statistical sampler or any vote attribution. Lane 1 (the partisan-fairness numbers) corroborates Lane 2 strongly under canonical official shapefiles: the minority is a statistical outlier on all four pre-registered metrics simultaneously, with a joint neutral-null probability of p&nbsp;=&nbsp;6.87×10<sup>−8</sup> (pre-registered Fisher combined test — a method combining four independent test results into a single probability by multiplying their individual tail probabilities; the combined figure is far smaller than any single test's because all four point the same direction; OSF <a href=\"https://osf.io/6pt83\" rel=\"noopener\">6pt83</a>). The question of whether Lane 2's unusual geometry is the specific <em>mechanism</em> behind the Lane 1 numbers was tested and the answer is no — see <a href=\"https://github.com/Ixby/alberta-electoral-boundaries-audit/blob/master/findings/redist_python_comparison.md\" rel=\"noopener\">findings/redist_python_comparison.md</a>. The Swing-Zone Allocation Test shows the contested boundary choices are partisan-skewed, but the tested question was whether the boundary shapes themselves — the lasso corridor, the park extension — are the direct cause of the seat swing; they are not. The seat effect comes from how redrawn Voting Area assignments shift vote efficiency across districts, not from the shapes per se. Both lanes flag the minority map; they reach it through independent instruments. Lane 2 is the central finding. Lane 1 corroborates without carrying."
		},
		impact: {
			...
			intro: "Lane 1 depends on which election results you score the maps against. Lane 2 does not. The structural evidence is in the maps themselves — drawn lines, split cities, where the boundaries do and don't follow administrative lines existing for other reasons. On these tests, the two maps are not close.",
			...
			rationales_p: "A separate finding — applied only to the minority because the minority is the map whose contested redraws are public (the majority did not publish a contested-redraw rationale list; the seven-rationale audit cannot be applied symmetrically and is reported as a single flag, not as additional rows in the structural-irregularity count) — <strong>five of six of the minority commissioners' published rationales fail under independent check</strong>. (A seventh redraw the audit had previously listed turned out to rest on a federal-boundary claim untraceable in the minority report; it has been removed rather than left as a weak claim.)",
			chair_appendix_p: 'The audit also tested the chair\'s separate, blanket assertion in Appendix C — the minority\'s seven contested hybrid configurations had <strong>no public support</strong> in the 1,140+ public submissions. A keyword search across the full submission archive (94% machine-parsed, 6% image-only and excluded; methodology and per-configuration evidence at <a href="https://github.com/Ixby/alberta-electoral-boundaries-audit/blob/master/findings/submission_search_findings.md" rel="noopener">findings/submission_search_findings.md</a>) returned a more nuanced picture than either the chair\'s blanket claim or its blanket dismissal: the chair was right on three of seven (Airdrie 4-way split, Calgary–Nolan-Hill–Cochrane hybrid, and the St. Albert minority alternative each lack any documented support), wrong on three of seven (Rocky Mountain House–Banff Park drew an explicit, detailed proposal from at least one Clearwater-area submission plus several aligned ones; Olds–Three-Hills–Didsbury was supported by Beiseker residents in writing; Chestermere drew multiple submissions opposing a Calgary merger, materially aligning with the minority\'s intent), and partially wrong on one (Red Deer hybrids drew a peri-Red-Deer hybrid proposal from a sitting Red Deer councillor, with directional but not configuration-exact alignment). The chair\'s Appendix C "no public support" sweep is therefore demonstrably overbroad — three of seven are demonstrably false — but it is not invented out of whole cloth, since three of seven do hold up. <strong>This finding cuts against the chair, not against the minority.</strong>',
		...
		cpd: {
			...
			airdrie_callout_p2: 'Splitting it across four provincial divisions — Calgary-Airdrie, Calgary-Foothills-Airdrie West, Calgary-Nolan Hill-Cochrane, and Airdrie East — each primarily identified with a different surrounding jurisdiction, removes Airdrie from the political map at the level of government drawing it. The city has 85,805 residents and zero seats in the legislature where a majority of voters call the place home.',
			airdrie_callout_p3: 'A four-way split is invisible to every partisan-fairness test except the one asking: can a voter find their MLA?',
		...
		litmus: {
			...
			fig_caption: 'Distribution of <button class="vocab-term" data-def="a measure of how lopsidedly votes are converted into seats — positive values favour the UCP, negative values favour the NDP" aria-expanded="false">efficiency gaps</button> across 250,000 neutral Alberta maps drawn from the same geography. Most neutral maps cluster near zero; the shaded right tail marks the top 10%. The minority proposal&rsquo;s +3.96% sits at the 94th <button class="vocab-term" data-def="the percentage of maps scoring lower — p94 means 94 out of 100 neutral maps were less partisan than this" aria-expanded="false">percentile</button> — a region fewer than 6 in 100 neutral maps ever reach. The majority proposal&rsquo;s +0.04% is indistinguishable from what a neutral process typically produces.',
			...
			table_r4_a: 'Borders following existing municipal lines',
		...
		commission_split: {
			...
			finding2: '<strong>Every measured difference moves in the same direction.</strong> Everywhere the two maps diverge — northwest Calgary, Airdrie, urban areas with clear city limits — the minority map draws boundaries spreading NDP votes thinner and letting UCP votes count more efficiently. The communities most reshaped by the minority map are the same communities where the NDP is strongest. The audit cannot determine intent. It can measure effect.',
		...
		november: {
			...
			context_body: ' — This section describes the process replacing the commission and the legal framework applying to it. It is not part of the statistical findings. The findings are in §3–§6 above.',
		...
		about_me: {
			...
			p2: "I have supported parties on all sides of the political spectrum depending on the election. I'm telling you this because my political history could affect how I look at this issue. The main protection against it is the method: I tested both maps the same way, wrote down my predictions before looking at the results, and put everything online so anyone can check my work. I paid for this research myself. If you find something I got wrong, I genuinely want to know.",
		...
		retractions: {
			...
			c3_what: "<em>What gets retracted:</em> The inference — the minority's boundary choices were drafting decisions rather than responses to documented community submissions.",
			c3_cond:
			...
			c4_what: "<em>What gets retracted:</em> The Lane 1 finding — the minority map's seats@50/50 score sits at the 99.99th percentile of 1,010,000 neutral draws.",
			c4_cond:
			...
			c5_what: '<em>What gets retracted:</em> The Section 7 procedural argument — the April 16 motion to replace the commission with the Lunty committee sits in the same constitutional class as Quebec\'s 2024 redistricting freeze.',
			c5_cond:
				"Background: on April 22, 2026 — six days after Alberta's April 16 motion — the Supreme Court of Canada upheld, 7–2 and from the bench, a Quebec Court of Appeal ruling: the Legault government's legislative freeze on its redistricting commission violated the Charter's s.3 democratic-representation guarantee. Condition: a court reviewing the Alberta motion finds it constitutionally distinct — for example, because reassigning the work to an MLA committee differs structurally from a legislative freeze, or because Alberta's effective-representation analysis under s.3 comes out differently than Quebec's.",
		...
		suggestions: {
			...
			census_p1:
				"<strong>Alberta should wait for the 2026 census before drawing the next map.</strong> Canada counts its population every ten years. The 2026 census enumeration happens in spring 2026; Statistics Canada releases usable sub-provincial data roughly two years later, in 2027 or 2028. The commission drawing the maps assessed in this audit had to use the 2021 census — already four years old when the maps were drawn, and potentially fourteen years old by the time those boundaries retire. Fast-growing cities like Airdrie and Chestermere will change by 40% or more over the window. Rural communities will shrink. The map will be wrong from the day it is used. A straightforward change to the <em>Electoral Boundaries Commission Act</em> could require any new commission to be appointed only after Statistics Canada releases the most current dissemination-area data from the preceding census. The result: maps reflecting where Albertans actually live, not where they lived a decade ago.",
			census_p2:
			...
			closing:
				"Neither of these is a finding about the current commission's maps. They are observations about a system making accurate electoral analysis harder than it needs to be. They are offered here as practical suggestions, not conclusions. Both are genuinely fixable, and fixing them would make every future commission — and every future audit — work from better ground."
		}
	},
	editorial_canada: {
		...
		p7: 'So when Canadian courts say "gerrymander" isn\'t their legal vocabulary, they are not saying the underlying concept does not apply here. They are saying the test is different — effective representation, not mathematical equality. Whether the minority proposal meets the test is exactly the question this audit has measured the geometry against, and exactly the question only a judge can answer definitively. The <em>Saskatchewan Reference</em> reasoning in full, the contrast with other provinces, the standing question, and the available reform pathways are treated in <a href="#references">the references section below</a>.'
	},
	chrome: {
