...
**One pre-registered pass (§5.3.5) — substrate-sensitive; canonical re-run pending.** The neighbour-drain adjacency test — which asks whether packed and cracked districts of the same party sit next to each other — finds **2 coupled chain signals under the minority map, compared to 6 under the majority and 5 under the 2019 enacted map** on the v0_2 DPG topology-cleaned substrate. The minority's coupled count (2) is 0.33× the majority's — well below the pre-registered pass threshold of 1.5×; the pre-registered PASS criterion is met. A v0_8 full-coverage substrate sensitivity check reported in §5.3.5 found minority=4, majority=3 (ratio 1.33× — still inside the 1.5× pre-registered threshold, but the direction reverses); the cross-substrate disagreement is documented honestly per the multi-method-reporting discipline. Neither substrate is canonical — the official Elections Alberta shapefiles arrived 2026-05-06 and the drain re-run on those shapefiles is not yet complete (see Open questions). §5.3.5 establishes the substantive explanation regardless of substrate: the minority achieves its partisan effect via **hybridization** (city-splitting that internalises packing and cracking within single EDs), not the adjacency-chain packing model this test measures.
...
| Coupled packing-cracking adjacencies (v0_2 DPG substrate; **canonical re-run pending**) | 6 | 2 | Pre-registered PASS for the minority on this test: 0.33× ratio, well below the 1.5× threshold. v0_8 substrate sensitivity check reversed the direction (majority 3, minority 4; ratio 1.33×, still inside threshold). Neither substrate is canonical; official EA shapefile re-run pending. §5.3.5 explains the hybridization mechanism, which is the substantive claim and is substrate-independent. |
...
**The pre-registered passes are reported as prominently as the findings.** The neighbour-drain test result — 2 coupled chain signals under the minority, 6 under the majority, 5 under the 2019 enacted map on the v0_2 DPG substrate (with a v0_8 cross-substrate check that reverses the direction inside the same pass threshold; canonical EA-shapefile re-run pending) — is reported as a §5.3.5 PASS (ratio 0.33×, threshold ≤ 1.5×), not buried in a supplementary table. The minority's low adjacency-chain count does not exonerate it; §5.3.5 establishes that the minority achieves its partisan effect via hybridization (city-splitting that internalises packing and cracking within single EDs), not via the classic adjacency-drain model this test measures. An audit that hides its non-findings — or hides the substrate-sensitivity of its non-findings — is not an audit.
...
**Neighbour-drain canonical re-run (§5.3.5).** The §5.3.5 pre-registered pass (minority 2 coupled chain signals vs majority 6 vs 2019 5; ratio 0.33×) was computed on the v0_2 DPG topology-cleaned substrate. A v0_8 full-coverage substrate sensitivity check (reported in §5.3.5) gave minority=4, majority=3 (ratio 1.33× — still inside the 1.5× pre-registered threshold, but the direction reverses). Neither substrate is the canonical Elections Alberta shapefile set received 2026-05-06; the canonical re-run of `analysis/scripts/neighbour_drain_adjacency.py` against `ea_majority_2026_eds.gpkg` and `ea_minority_2026_eds.gpkg` is not yet complete. Until it is, the §5.3.5 pass is reported with the substrate caveats above; the substantive hybridization explanation (§5.3.2) does not depend on which substrate the adjacency test runs on.
