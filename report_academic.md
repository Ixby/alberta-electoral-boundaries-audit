...
**What is narrowed:**
- §B partisan-bias *magnitude* claims. The point estimate of 0.58 pp is within Monte Carlo noise. The direction holds at 89.3% confidence across modelling uncertainty, which is a defensible directional claim but not classical 95% significance.
...
The minority-majority EG asymmetry is negative (minority more UCP-favorable) in 89.3% of 2,000 Monte Carlo samples across the parameter space (urban weight 0.55–0.85, rural baseline 0.26–0.36, per-hybrid jitter ±0.10). The 95% confidence interval is [−3.04, +0.76] pp and crosses zero. We report this as a directional observation at approximately 90% confidence and do not assert statistical significance at the conventional 95% threshold. The magnitude claim (specifically 0.5–1.6 pp) does not meet the 95% threshold. The 1-seat structural asymmetry does meet a replication test across distinct vote inputs: 338Canada April 2026 polling reallocated through the hybrid crosswalks produces the same 1-seat minority-vs-majority gap as 2023 Statement-of-Vote data (see §3.5 cross-validation paragraph below). If measured attribution from Phase 4C produces an asymmetry of opposite sign, or if the 1-seat cross-input replication fails on a third vote input, the directional claim is falsified.

**Cross-election contingency.** The asymmetry direction is stable across 2023 Statement-of-Vote data and April 2026 338Canada polling. It reverses sign when 2019 votes are used as input (asymmetry becomes +0.75 pp under 2019 votes). The direction is therefore stable across 2020s-era Alberta political geography but contingent on that geography persisting. A hostile reader who substitutes 2019 voter distributions for 2023 distributions recovers a result that contradicts the headline. The paper reports this contingency as a property of the finding, not a defect: the boundary effect is sensitive to which electorate is asked.
...
|---|---|---|---|
| RT1 — Monte Carlo 95% CI | Same-sign bounds for strong pass | [−3.04, +0.76] pp, crosses zero | Fails strong pass; 89.3% direction consistency is a separate direction claim |
...
**Pending release of the commission shapefiles**, the audit has drafted a request to Elections Alberta for research access (`analysis/v0_1_elections_alberta_shapefile_request.md`). A refinement pass using higher-DPI commission-map-page extraction, OpenStreetMap road-network snapping, and base-map overlay verification is catalogued in `analysis/v0_1_shape_refinement.md` (when complete); when that produces Tier C compactness scores with documented uncertainty intervals, this subsection will be updated accordingly.
