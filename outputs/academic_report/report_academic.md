...
#### 5.4.9 Canonical ensemble — official Elections Alberta shapefiles (250k, 4 chains, 2026-05-07–2026-05-08)
...
**SZAT Bootstrap Pre-registration Mismatch:** The initial SZAT p-value of 0.0044 was calculated using an additive-delta approximation of the bootstrap null on the election-day-only VA substrate. The pre-registration document (OSF: 6pt83) specified a full map EG recompute per permutation. The definitive SZAT run (§5.2.10 Post-registration substrate update, 2026-05-10) uses the full 2023 VA vote substrate and reports p=0.0024; this is the value used in the Fisher combination above (p = 6.87×10⁻⁸). Validation testing (`szat_validate.py`) confirms the delta approximation and full recompute yield functionally equivalent results; the minor methodological deviation from the pre-registration is acknowledged.
