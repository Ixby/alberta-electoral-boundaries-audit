...
- Dependence-robust joint upper bound across the two analytical channels: **p ≤ 2.80×10⁻⁶ (≈ 1 in 357,000)**, Bonferroni-corrected. The earlier Fisher figure of 6.87×10⁻⁸ is retired because Ch1 and Ch2 share the same vote-attribution substrate and overlap on the EG dimension — they are not independent.
...
```bash
# Verify the corrected joint statistic against the canonical chain CSVs
python -c "
...
real_min = np.array([0.040194, 0.010402, -0.077003, 0.516854])
...
"
# Expected: D = 5.7757; n_extreme = 0; bound = 1.98e-06 (~1 in 506,000)
# (The published 1-in-357,000 uses the parametric χ² Ch1 p = 1.40e-6 instead of the
#  empirical floor; both are defensible, the parametric is the more conservative.)
```

If that one-liner reproduces, the audit's central machinery is intact and the canonical chain CSVs are unmodified.
