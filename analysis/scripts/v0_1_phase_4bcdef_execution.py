...
def run_phase_4b() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def summarize(das_w: pd.DataFrame, names: list[str], sources: dict[str, str],
                  label: str) -> pd.DataFrame:
        das_w = das_w.copy()
        das_w["_assigned_norm"] = das_w["name_2026"].apply(
            lambda x: norm(x) if pd.notna(x) else None)
        agg = (das_w.dropna(subset=["_assigned_norm"])
               .groupby("_assigned_norm")["population_2021"]
               .sum().reset_index())
        agg.columns = ["_name_norm", "pop_4b_sum"]

        rows = []
        unassigned_total = float(das_w[das_w["_assigned_norm"].isna()]["population_2021"].sum())
        for name in names:
            nm = norm(name)
        ...
        print(f"  [{label}] total assigned pop: {out['pop_2021_from_das'].sum():,.0f}, "
...
def run_phase_4c() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def assign_and_summarize(polys: gpd.GeoDataFrame, xwalk: dict[str, str],
        ...
        joined = joined[~joined.index.duplicated(keep="first")]
        vas_["_assigned_poly"] = joined["name_2026"].apply(
            lambda x: norm(x) if pd.notna(x) else None).values

        vas_["_assigned_xwalk"] = vas_["_parent2019"].map(xwalk).fillna(vas_["_parent2019"])
        ...
        for name in names:
            ...
            if len(sub) == 0:
                rows.append({"ed_name": name, "ucp_2023": 0, "ndp_2023": 0,
                             "other_2023": 0, "polygon_source": sources.get(nm, "none"),
                             "total_votes": 0})
            else:
                ucp = int(sub["ucp"].iloc[0])
                ndp = int(sub["ndp"].iloc[0])
                oth = int(sub["other"].iloc[0])
                rows.append({"ed_name": name, "ucp_2023": ucp, "ndp_2023": ndp,
                             "other_2023": oth,
                             "polygon_source": sources.get(nm, "none"),
        ...
        print(f"  [{label}] sums: UCP={df['ucp_2023'].sum():,}, "
              f"NDP={df['ndp_2023'].sum():,}, Other={df['other_2023'].sum():,}, "
              f"two-party={df['ucp_2023'].sum() + df['ndp_2023'].sum():,}")
        return df

    maj_out = assign_and_summarize(maj_polys, maj_xwalk, maj_names, maj_sources, "majority")
    min_out = assign_and_summarize(min_polys, min_xwalk, min_names, min_sources, "minority")
...
def run_phase_4f(maj_4b: pd.DataFrame, min_4b: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  Implied growth factor: {growth_factor:.4f} ({100*(growth_factor-1):.2f}%)")
    ...
    def prep(df, pubs, label):
        ...
        d["delta_scaled_pct"] = 100.0 * d["delta_scaled"] / d["pop_commission"].replace({0: np.nan})
        d["map"] = label
        return d
    ...
    for label, df in [("majority", maj), ("minority", mino)]:
        ...
        print(f"    RAW delta: mean={d['delta_raw'].mean():.0f}, "
              f"median={d['delta_raw'].median():.0f}, "
              f"max_abs={d['delta_raw'].abs().max():.0f}, "
              f"RMS={np.sqrt((d['delta_raw']**2).mean()):.0f}")
        print(f"    SCALED delta: mean={d_nz['delta_scaled'].mean():.0f}, "
              f"median={d_nz['delta_scaled'].median():.0f}, "
              f"max_abs={d_nz['delta_scaled'].abs().max():.0f}, "
              f"RMS={np.sqrt((d_nz['delta_scaled']**2).mean()):.0f}")
        print(f"    warn 0.5% (scaled, nonzero): {int(d_nz['delta_scaled_pct'].abs().gt(0.5).sum())}")
        print(f"    hardstop 2% (scaled, nonzero): {int(d_nz['delta_scaled_pct'].abs().gt(2.0).sum())}")
...
def compute_summary(maj_4b, min_4b, maj_4c, min_4c, combined_4f) -> dict:
    ...
    for label, df in [("majority", maj_4c), ("minority", min_4c)]:
        ...
        summary["phase_4c"][label] = {
            "n_eds": int(len(df)),
            "ucp_total": ucp, "ndp_total": ndp, "other_total": oth,
            "two_party_total": ucp + ndp, "all_votes": ucp + ndp + oth,
        }
    ...
    summary["phase_4f"]["province_2021_from_das"] = float(da_pops["population_2021"].sum())
    summary["phase_4f"]["commission_total_tbf_jul2024"] = float(commission_maj)
    summary["phase_4f"]["growth_factor"] = float(commission_maj / da_pops["population_2021"].sum())
    for label in ["majority", "minority"]:
        ...
        summary["phase_4f"][label] = {
            ...
            "rms_delta_scaled_nonzero": float(np.sqrt((nz["delta_scaled"] ** 2).mean())),
            "max_abs_delta_scaled_pct_nonzero": float(nz["delta_scaled_pct"].abs().max()),
            "n_warn_0p5pct_scaled_nonzero": int(nz["delta_scaled_pct"].abs().gt(0.5).sum()),
            "n_hardstop_2pct_scaled_nonzero": int(nz["delta_scaled_pct"].abs().gt(2.0).sum()),
        }
