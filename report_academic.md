...
| H3 | Non-compact geometry (the chair-flagged lasso shapes, the urban-rural hybridizations, the Airdrie four-way split) is the load-bearing mechanism through which the minority commissioners reached their UCP `seats@50/50` advantage. Concretely: the SMC plans that reach the minority's 0.4831 value should have systematically lower mean Polsby-Popper compactness than the SMC plans that don't | **REJECTED** | Falsification test designed by the principal investigator before running, in keeping with pre-registration discipline. The test was: among the 5,000-plan R-`redist` SMC ensemble, compare mean per-district Polsby-Popper compactness for plans with `seats@50/50` ≥ 0.4831 (n=2,762) against the rest (n=2,238). Result: **mean PP for high-UCP-advantage plans = 0.2391; mean PP for other plans = 0.2339** (Welch t-test p = 7.7 × 10⁻²³⁴). The high-UCP-advantage plans are very slightly *more* compact, not less. The hypothesis fails decisively in the wrong direction. The empirical finding that R-SMC reaches the minority's value more often than Python-ReCom does is preserved; the *mechanistic* claim that compactness is what makes that gap is rejected | `analysis/reports/redist_python_comparison.md`; `analysis/scripts/compactness_for_verification_subset.py`; `data/v0_1_redist_crossvalidation_s50.csv` |

The audit's central directional claim (Lane 2 structural-irregularity scorecard fires 5/5 against the minority and 0/5 against the majority; corrected ReCom Lane-1 percentile of p98.6) survives all three rejections. Hypothesis H3's failure narrows the audit's scope of interpretation: the geometric mechanism that distinguishes ReCom-reachable maps from SMC-reachable maps is open and worth investigating (candidates: minimum per-district compactness, city-cracking patterns specifically, urban-rural hybridization configurations, cut-edge density), but the audit does not claim to have identified it.
