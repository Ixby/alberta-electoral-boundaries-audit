...
def load_leduc_beaumont_vas():
    ...
    total_votes = (
        lb_vas["va_ndp"] + lb_vas["va_ucp"] + lb_vas.get("va_other", 0)
    ).sum()
...
def buffer_to_target(city_poly, lb_vas_gdf, total_lb_votes, target_pop):
    ...
    for buf_m in range(0, 30_001, 500):
        ...
        if captured >= target_votes_proxy:
            ...
            log(
                f"  Buffer {buf_m}m: captured {int(captured):,} votes "
    ...
    log(
        f"  Final: captured {int(best_captured):,} votes, fraction={actual_frac:.3f}, "
...
def update_edmonton_beaumont(maj_gdf, new_poly):
    ...
    if not mask.any():
        ...
        log(
            "  Replacing existing Edmonton-Beaumont polygon (was: "
...
def redteam_validate(maj_gdf, min_gdf):
    ...
    for label, gdf, expected_n in [
        ...
        if gdf.crs is None or gdf.crs.to_epsg() != WORK_CRS:
            warnings_list.append(
                f"{label}: CRS is {gdf.crs} (expected EPSG:{WORK_CRS})"
            )
        else:
        ...
        if "canon_tier" in gdf.columns:
            ...
            for tier, cnt in sorted(tier_dist.items(), key=lambda x: -x[1]):
                conf = (
                    "high"
                    if tier in ("A", "B")
                    else "medium" if "pixel" in tier or "sweep" in tier else "lower"
                )
        ...
        if len(tiny) > 0:
            errors.append(
                f"{label}: {len(tiny)} near-zero area polygons: "
        ...
        n_overlaps = 0
        overlap_pairs = []
        valid_gdf = gdf[~gdf.geometry.is_empty & gdf.geometry.notna()].reset_index(
            drop=True
        )
        for i in range(len(valid_gdf)):
            for j in range(i + 1, len(valid_gdf)):
                try:
                    inter = valid_gdf.geometry.iloc[i].intersection(
                        valid_gdf.geometry.iloc[j]
                    )
    ...
    if "Edmonton-Beaumont" in min_gdf["name_2026"].values:
        log(
            "  [NOTE] Edmonton-Beaumont exists in minority file too "
    ...
    try:
        ...
        warnings_list.append(
            "Could not import packing_cracking_analysis for crosswalk check"
        )
...
def main():
    ...
    if beaumont_osm is not None:
        ...
        captured_votes = (
            in_eb["va_ndp"] + in_eb["va_ucp"] + in_eb.get("va_other", 0)
        ).sum()
        ...
        log(
            f"  Building ~15km radius buffer around Beaumont centre "
