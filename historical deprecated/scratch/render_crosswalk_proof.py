...
OUT_PATH = ROOT / "scratch" / "crosswalk_proof.svg"
...
def render_proof():
    ...
    eds_2026 = eds_2026[[name_col_2026, "geometry"]].rename(
    ...
    fig.suptitle(
        "The Anatomy of a Gerrymander: Cracking, Draining, and Packing",
        fontsize=28,
        color="black",
        y=1.05,
        weight="bold",
    ...
    cracked_clip.plot(
        ax=ax1, facecolor="none", edgecolor="black", linewidth=4, linestyle="--"
    )
    ...
    red_patch = mpatches.Patch(
        color="#ffcccc", ec="red", label="Original 2019 District"
    )
    dash_line = mlines.Line2D(
        ...
        linestyle="--",
        linewidth=3,
        label="New 2026 District Borders",
    )
    ...
    drain_clip.plot(
        ax=ax2, color="#ffcc99", edgecolor="orange", linewidth=4, hatch="///"
    )
    swallower_outline = gpd.clip(
        swallower_2026, drain_2019.buffer(5000)
    )  # show a bit outside
    swallower_outline.plot(
    ...
    blue_patch = mpatches.Patch(
        color="#cce5ff", ec="#005a9c", label="2019 Urban District"
    )
    orange_patch = mpatches.Patch(
        color="#ffcc99",
        ec="orange",
        hatch="///",
        label="Drained Slice (Attached to Rural)",
    )
    ...
    labels = [
        "From: Drayton Valley (Rural Base)",
        "From: Edmonton SW (Drained)",
        "From: Strathcona (Cracked)",
    ]
    patches = []

    for idx, (i, row) in enumerate(sink_clip.iterrows()):
        gpd.GeoSeries([row.geometry]).plot(
            ax=ax3, color=colors[idx % len(colors)], edgecolor="gray", linewidth=1
        )
        patches.append(
            mpatches.Patch(
                color=colors[idx % len(colors)], label=labels[idx % len(labels)]
            )
        )
    ...
    ax3.legend(
        handles=patches, loc="lower right", fontsize=12, title="2019 Donor Districts"
    )
