...
Forward: analysis/v0_1_mcmc_full_coverage_rescore_100k.py
Backward:
...
def build_majority_crosswalk() -> pd.DataFrame:
    ...
    for _, r in hyb.iterrows():
        ...
        if cur in eds_2019_set:
            tp = "hybrid" if "-" in prop and prop != cur else ("rename" if prop != cur else "identity")
    ...
    for src_2019, successors in rev.items():
        ...
        if len(successors) == 1:
            ...
            if identical:
                ...
                contains = [s for s in successors if src_2019.lower() in s.lower() or s.lower() in src_2019.lower()]
                if contains:
            ...
            candidates[src_2019].append((3, sel, f"reall_multi ({tp}, opts={len(successors)})"))
    ...
    # Based on analysis/v0_1_91_seat_preliminary.md F6: Calgary +2 (Nose Creek, Confluence, McKenzie; Peigan eliminated)
    # Peigan 2019 was absorbed; the 2026 area is split between Calgary-Peigan-adjacent EDs.
    manual = {
        # 2019 -> (2026 successor, note)
        "Calgary-Peigan": ("Calgary-McKenzie", "manual: Peigan eliminated in majority; McKenzie covers S. Calgary Peigan territory"),
        "Calgary-Foothills": ("Calgary-Nose Creek", "manual: Foothills restructured; Nose Creek absorbs north-Calgary territory"),
        "Calgary-McCall": ("Calgary-Bhullar-McCall", "manual: McCall renamed to Bhullar-McCall"),
        "Calgary-West": ("Calgary-West-Elbow Valley", "manual: West restructured with Elbow Valley addition"),
        "Calgary-Glenmore": ("Calgary-Glenmore-Tsuut'ina", "manual: Glenmore + Tsuut'ina addition"),
        "Calgary-Falconridge": ("Calgary-Falconridge-Conrich", "manual: Falconridge + Conrich addition"),
        "Banff-Kananaskis": ("Canmore-Banff", "manual: Banff-Kananaskis renamed/relocated under majority"),
        "Cardston-Siksika": ("High River-Vulcan-Siksika", "manual: Siksika territory absorbed into High River-Vulcan-Siksika"),
        "Highwood": ("Okotoks-Diamond Valley", "manual: Highwood restructured -> Okotoks-Diamond Valley"),
        "Drayton Valley-Devon": ("Stony Plain-Drayton Valley", "manual: Drayton Valley-Devon renamed Stony Plain-Drayton Valley"),
        "Edmonton-Glenora": ("Edmonton-Glenora-Riverview", "manual: Edmonton-Glenora merged with Riverview"),
        "Edmonton-Riverview": ("Edmonton-Glenora-Riverview", "manual: Edmonton-Riverview merged with Glenora"),
        "Edmonton-South West": ("Edmonton-Windermere", "manual: Edmonton-South West absorbed into Edmonton-Windermere"),
        "Innisfail-Sylvan Lake": ("Sylvan Lake-Innisfail", "manual: renamed"),
        "Lac Ste. Anne-Parkland": ("Edmonton-Enoch", "manual: LSA-Parkland absorbed into Edmonton-Enoch (majority)"),
        "Olds-Didsbury-Three Hills": ("Mountain View-Kneehill", "manual: Olds-Didsbury-Three Hills restructured as Mountain View-Kneehill (majority)"),
        "Rimbey-Rocky Mountain House-Sundre": ("Lacombe-Clearwater", "manual: RMH-Sundre restructured into Lacombe-Clearwater under majority"),
        "Taber-Warner": ("Taber-Cardston", "manual: Taber-Warner merged with Cardston portion to form Taber-Cardston (majority)"),
        "Athabasca-Barrhead-Westlock": ("Barrhead-Westlock-Athabasca", "manual: reorder rename"),
        "Airdrie-Cochrane": ("Airdrie-West", "manual: Airdrie-Cochrane restructured to Airdrie-West"),
    }
    ...
    for k in sorted(eds_2019_set):
        if not candidates[k]:
            rows.append({"current_2019": k, "proposed_2026": k, "type": "identity_fallback",
        ...
        tp = "identity" if sel == k else ("rename" if any(s in src for s in ["rename", "hybrid"]) else "split_primary")
        rows.append({"current_2019": k, "proposed_2026": sel, "type": tp, "source": src})
...
def build_minority_crosswalk() -> pd.DataFrame:
    ...
    for _, r in hyb.iterrows():
        cur = norm(r["current_2019"])
        prop = norm(r["proposed_2026"])
        if cur in eds_2019_set and prop and "MERGED" not in prop.upper() and "ABSORBED" not in prop.upper():
            tp = "hybrid" if r.get("is_hybrid") == "yes" else ("rename" if prop != cur else "identity")
            candidates[cur].append((2, prop, f"hybrid_xwalk ({tp})"))
    ...
    manual = {
        ...
        "Airdrie-Cochrane": ("Calgary-Nolan Hill-Cochrane", "manual: Airdrie-Cochrane absorbed -> Nolan Hill-Cochrane"),
        "Banff-Kananaskis": ("Canmore-Kananaskis", "manual: Banff-Kananaskis -> Canmore-Kananaskis (rename)"),
        "Calgary-Shaw": ("Calgary-South", "manual: Calgary-Shaw split between De Winton and South; South is primary"),
        "Cardston-Siksika": ("Lethbridge-Cardston", "manual: Cardston-Siksika absorbed -> Lethbridge-Cardston"),
        "Edmonton-Castle Downs": ("Edmonton-Castledowns", "manual: renamed"),
        "Edmonton-Riverview": ("Edmonton-Glenora-Riverview", "manual: merged into Glenora-Riverview"),
        "Edmonton-South West": ("Edmonton-Windermere", "manual: absorbed into Edmonton-Windermere"),
        "Lacombe-Ponoka": ("Red Deer-Lacombe", "manual: Lacombe-Ponoka absorbed -> Red Deer-Lacombe"),
        "Lethbridge-East": ("Lethbridge-Taber-Warner", "manual: Lethbridge-East absorbed -> Lethbridge-Taber-Warner (primary)"),
        "Lethbridge-West": ("Lethbridge-Fort MacLeod-Crowsnest Pass", "manual: Lethbridge-West absorbed -> Lethbridge-Ft MacLeod"),
        "Livingstone-Macleod": ("Lethbridge-Fort MacLeod-Crowsnest Pass", "manual: Livingstone-Macleod absorbed -> Lethbridge-Ft MacLeod"),
        "Maskwacis-Wetaskiwin": ("Wetaskawin-Ponoka-Maskwacis", "manual: renamed+consolidated"),
        "Okotoks-Sheep River": ("Highwood", "manual: Okotoks-Sheep River absorbed -> Highwood"),
        "Vermilion-Wainwright": ("Lloydminster-Wainwright", "manual: Vermilion-Wainwright absorbed -> Lloydminster-Wainwright"),
        # Also fill any 2019 EDs not in candidates yet
        "Rimbey-Rocky Mountain House-Sundre": ("Rocky Mountain House-Banff Park", "manual: RMH-Sundre -> RMH-Banff Park"),
        "Calgary-Foothills": ("Calgary-Foothills-Airdrie West", "manual: Foothills extends to Airdrie West"),
        "Calgary-Peigan": ("Calgary-Peigan-Chestermere", "manual: Peigan + Chestermere hybrid"),
        ...
        "Taber-Warner": ("Lethbridge-Taber-Warner", "manual: Taber-Warner absorbed into Lethbridge-Taber-Warner"),
        "Morinville-St. Albert": ("St. Albert-Sturgeon", "manual: renamed St. Albert-Sturgeon"),
        "Spruce Grove-Stony Plain": ("Edmonton-Spruce Grove", "manual: renamed Edmonton-Spruce Grove"),
        "Cold Lake-St. Paul": ("Cold Lake-Bonnyville-St. Paul", "manual: consolidated"),
        "Bonnyville-Cold Lake-St. Paul": ("Cold Lake-Bonnyville-St. Paul", "manual: consolidated (reorder)"),
        "Olds-Didsbury-Three Hills": ("Olds-Three Hills-Didsbury", "manual: reorder rename"),
        "Drayton Valley-Devon": ("Stony Plain-Drayton Valley", "manual: DV-Devon -> Stony Plain-DV"),
        "Leduc-Beaumont": ("Leduc", "manual: Leduc-Beaumont -> Leduc (Beaumont split to Edmonton-Beaumont)"),
        "Innisfail-Sylvan Lake": ("Red Deer-Sylvan Lake", "manual: renamed"),
        "Red Deer-North": ("Red Deer-Innisfail", "manual: split primary (also Red Deer-Lacombe); Red Deer-Innisfail primary"),
        ...
        "Vermilion-Lloydminster-Wainwright": ("Lloydminster-Wainwright", "manual: Vermilion split out"),
        "Athabasca-Barrhead-Westlock": ("Barrhead-Westlock-Athabasca", "manual: reorder rename"),
        "Airdrie-East": ("Airdrie East", "manual: Airdrie-East -> Airdrie East (no hyphen)"),
    }
    for k, (v, note) in manual.items():
        k_n = norm(k)
        v_n = norm(v)
    ...
    for k in sorted(eds_2019_set):
        if not candidates[k]:
            ...
            if k in min_2026_set:
                rows.append({"current_2019": k, "proposed_2026": k, "type": "identity",
                             "source": "identity_found_in_2026"})
            else:
                rows.append({"current_2019": k, "proposed_2026": k, "type": "identity_fallback_missing",
