...
ED_NAMES_2019 = [
    ...
    "Calgary-Cross", "Calgary-Currie", "Calgary-East", "Calgary-Edgemont",
    "Calgary-Elbow", "Calgary-Falconridge", "Calgary-Fish Creek",
    "Calgary-Foothills", "Calgary-Glenmore", "Calgary-Hays", "Calgary-Klein",
    "Calgary-Lougheed", "Calgary-Bhullar-McCall", "Calgary-Mountain View",
    "Calgary-North", "Calgary-North East", "Calgary-North West",
    "Calgary-Peigan", "Calgary-Shaw", "Calgary-South East",
    ...
    "Edmonton-Glenora", "Edmonton-Gold Bar", "Edmonton-Highlands-Norwood",
    "Edmonton-Manning", "Edmonton-McClung", "Edmonton-Meadows",
    "Edmonton-Mill Woods", "Edmonton-North West", "Edmonton-Riverview",
    "Edmonton-Rutherford", "Edmonton-South", "Edmonton-South West",
    "Edmonton-Strathcona", "Edmonton-West Henday", "Edmonton-Whitemud",
    # Rest of Alberta 47-87
    "Airdrie-Cochrane", "Airdrie-East", "Athabasca-Barrhead-Westlock",
    "Banff-Kananaskis", "Bonnyville-Cold Lake-St. Paul",
    "Brooks-Medicine Hat", "Camrose", "Cardston-Siksika",
    "Central Peace-Notley", "Chestermere-Strathmore",
    "Cypress-Medicine Hat", "Drayton Valley-Devon", "Drumheller-Stettler",
    "Fort McMurray-Lac La Biche", "Fort McMurray-Wood Buffalo",
    "Fort Saskatchewan-Vegreville", "Grande Prairie", "Grande Prairie-Wapiti",
    "Highwood", "Innisfail-Sylvan Lake", "Lac Ste. Anne-Parkland",
    "Lacombe-Ponoka", "Leduc-Beaumont", "Lesser Slave Lake",
    "Lethbridge-East", "Lethbridge-West", "Livingstone-Macleod",
    "Maskwacis-Wetaskiwin", "Morinville-St. Albert",
    "Olds-Didsbury-Three Hills", "Peace River", "Red Deer-North",
    "Red Deer-South", "Rimbey-Rocky Mountain House-Sundre", "Sherwood Park",
    "Spruce Grove-Stony Plain", "St. Albert", "Strathcona-Sherwood Park",
    "Taber-Warner", "Vermilion-Lloydminster-Wainwright", "West Yellowhead",
]
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if not row or row[0] is None:
                ...
                if row[1] and isinstance(row[1], str):
                    if "Subtotal" in row[1] or "Total" in row[1] or "Percentage" in row[1]:
            ...
            ballot_type = row[type_col] if type_col is not None and type_col < len(row) else None
            voting_areas = row[areas_col] if areas_col is not None and areas_col < len(row) else None
            ndp_votes = row[ndp_col] if ndp_col is not None and ndp_col < len(row) else 0
            ucp_votes = row[ucp_col] if ucp_col is not None and ucp_col < len(row) else 0
            valid_votes = row[valid_col] if valid_col is not None and valid_col < len(row) else 0
            ...
            polls.append({
                ...
                "poll_name": poll_name.strip() if isinstance(poll_name, str) else poll_name,
...
def assign_to_2026_eds_via_vision(polls: List[Dict], map_image_paths: Dict[str, str]) -> List[Dict]:
