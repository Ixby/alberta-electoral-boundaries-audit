...
> **Disclosure (April 26, 2026): two-million-map ensemble numbers are being recomputed.** An external code audit by an independent AI reviewer (Gemini, prompted with the audit's own [code-audit brief](analysis/methodology/v0_1_external_code_audit_brief.md)) surfaced nine fixable bugs in the audit pipeline — three of them statistically load-bearing. The most serious: the "2,000,000-map" Markov-chain ensemble was structurally a stack of 100 independent 20,000-step short-bursts (the chain state was being silently reset to the 2019 baseline at every chunk boundary), not the single deep continuous chain its diagnostics implied. All nine bugs are now fixed in source ([commit 73544a3](https://github.com/Ixby/alberta-electoral-boundaries-audit/commit/73544a3), [audit memo](analysis/methodology/v0_1_external_code_audit_findings_gemini_2026-04-26.md)) and three new regression tests cover the failure modes (12 of 12 tests passing). The corrected 2,000,000-step ensemble is currently running. **Until that re-run completes, every percentile placement and "out of distribution at p100" claim in this article is held in revision.** Lane 2 (structural-irregularity) findings, the rationale-failure pattern, the qualitative findings about community splits, the constitutional discussion, and the description of the April 16 process change are *not* affected — those rest on direct measurement and primary documents, not on the ensemble. A dated pre-registration amendment recording the deltas will be filed when the re-run finishes; if the corrected ensemble's `seats@50/50` ceiling rises above the minority map's value, the headline framing is retracted and replaced with the corrected number on the same day.
...
**Honest gaps in the audit, listed.** Three open exposures are worth naming explicitly. First, **external replication in a second language.** The audit is reproducible by design — open code, open data, deterministic seeds, pinned dependencies — but at time of writing has not been independently re-run in R or in any other research stack. A step-by-step plan for cross-validating the headline finding using the R `redist` package is at `analysis/methodology/v0_1_external_tool_validation_plan.md`. Until that R cross-validation completes, all quantitative claims should be read as "the audit's Python pipeline produces these values; reproduction in a second language is the next defensibility step." Second, **code-bug risk that the test suite caught.** As of April 26, 2026, an external code audit by an independent AI reviewer (Gemini) surfaced nine fixable bugs in the pipeline — three of them statistically load-bearing — and all nine are now fixed in source with three new regression tests (12 of 12 tests passing). The most serious bug had the headline 2,000,000-map MCMC ensemble silently restarting from the 2019 baseline at every 20,000-step chunk boundary, meaning the deep continuous chain its diagnostics implied was structurally a stack of 100 short bursts. The corrected ensemble is being re-run; until it completes, every percentile placement in this article is held in revision (see the disclosure notice at the top). The full audit memo with each finding, severity, and fix is at `analysis/methodology/v0_1_external_code_audit_findings_gemini_2026-04-26.md`. Third, **bug-class risk that no test suite catches.** Standard scientific-software bugs (off-by-one errors, coordinate-system mismatches, floating-point edge cases) almost certainly exist somewhere at low magnitude and would not be caught by re-running the same Python pipeline. The April 26 audit is one snapshot of an adversarial review by one tool; that snapshot is *not* a guarantee that no other bugs exist. The high-value mitigation remains independent reproduction in R; the supplementary mitigations are continued external code audits and spot-checking by another developer.
