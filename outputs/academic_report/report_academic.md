...
**Archived for Provenance (Superseded May 6, 2026).** Prior to Elections Alberta's release of official 2026 shapefiles, the audit constructed a provisional three-tier geometry framework (Tier A/B/C) to execute compactness testing. This framework used the 2019 enacted shapefile (Tier A), merges of constituent 2019 polygons (Tier B), and visual transcription from commission map images (Tier C). Across multiple refinement passes (v0_1 through v0_10), the audit produced compactness estimates for 64–79 % of each map's districts. The Polsby-Popper metric showed inconsistent results across substrate generations, reversing direction twice (v0_1 → v0_7 → v0_9); the Reock metric (which penalizes elongation rather than perimeter wiggle) proved more discriminating and identified the chair-named "lasso" corridors at 2.58× minority/majority asymmetry (31/89 vs 12/89 districts below the 0.30 threshold). Three districts (Calgary-De Winton, Calgary-South, Edmonton-Windermere) remained unscored due to insufficient shapefile data.

**All findings in this section have been officially superseded by the canonical measurements reported in §5.4.9 and the main body (§5) using Elections Alberta's official shapefiles received 2026-05-06.** The provisional compactness scores are preserved here for revision-trail transparency; the authoritative structural findings on compactness rest on the post-shapefile analysis. Full technical methodology and version history at `analysis/methodology/commission_reference_shapes.md` and `analysis/reports/approximate_shape_analysis.md`.
