...
- Top 3 priority items:
  1. Dict-iteration order bug in targeted gerrymander scoring (`targeted_gerrymander_burst.py`).
  2. Missing spatial-join deduplication in fuzzing script (`fuzz_missing_eds.py`).
  3. PRNG sequence continuity breakage via per-chunk reseeding in the 250k ensemble (`mcmc_ensemble_250k_v0_8.py`).
...
- File: `analysis/scripts/mcmc_ensemble_250k_v0_8.py`
- Lines: 121-123
...
- File: `analysis/scripts/mcmc_ensemble.py`
- Lines: 198-200
...
1. `score()` in `targeted_gerrymander_burst.py`: The dict iteration bug would have been caught if this function had a unit test injecting unaligned mock dictionaries.
2. `fuzz_missing_eds.py`'s manual aggregation logic: Needs a test to verify it handles polygon overlap slivers identically to the main pipeline.
3. `rural_protection_test.py`'s `classify()` function: A simple parameterized test checking that all 87 specific ED names map to the expected urban/rural/hybrid category would prevent silent classification failures if name spellings drift.
