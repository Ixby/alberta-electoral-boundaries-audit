...
Inputs:
  data/alberta_2023_results.csv      — 87 EDs, candidate-level 2023
  data/majority_2026_populations.csv — 89 majority EDs
  data/minority_2026_populations.csv — 89 minority EDs
...
MAJORITY_2026_MAPPING = {
    ...
    "Edmonton-Glenora-Riverview": (
        "merge",
        ["Edmonton-Glenora", "Edmonton-Riverview"],
        [1.0, 0.4],
    ),
    # Edmonton hybrids
    ...
    # applicable. See data/majority_hybrid_crosswalk.csv for the
...
MINORITY_2026_MAPPING = {
    ...
    "Red Deer-Sylvan Lake": (
        "merge",
        ["Red Deer-South", "Innisfail-Sylvan Lake"],
        [0.5, 0.3],
    ),
}
...
def estimate_2026(
    dists_2019: List[Dict],
    mapping: Dict[str, tuple],
    rural_ndp_share: float,
    urban_weight: float = URBAN_WEIGHT_DEFAULT,
) -> List[Dict]:
    ...
    # inside main(), but monte_carlo_ci.py calls estimate_2026 in a
    ...
    if missing:
        ...
        raise KeyError(
            "estimate_2026: mapping rows could not be resolved: " + "; ".join(missing)
        )
    return out
...
def main():
    ...
    if m_maj and m_min:
        ...
        print(
            f"  Actual seats NDP/UCP | {m_2019['ndp_seats']}/{m_2019['ucp_seats']}   | {m_maj['ndp_seats']}/{m_maj['ucp_seats']}    | {m_min['ndp_seats']}/{m_min['ucp_seats']}"
        )
        print(
        ...
        for w in [0.60, 0.70, 0.80, 0.85, 0.90]:
            ...
            override_maj = {
                k: (v[0], v[1], w) if v[0] == "blend" else v
            ...
            print(
                f"  {w:.2f}         | {mm_maj['eg']*100:+7.2f}%    | {mm_min['eg']*100:+7.2f}%    | {(mm_min['eg']-mm_maj['eg'])*100:+5.2f}pp"
            )
    ...
    if m_maj and m_min:
        ...
        if abs(delta_min - delta_maj) < 0.005:
            ...
            print(
                f"  VERDICT: minority shifts {(delta_min-delta_maj)*100:+.2f} pp relative to majority."
            )
