...
def plot_va_dots(ax, boundary_poly, va_pts):
    ...
    if not pts_in.empty:
        ...
        pts_in.plot(
            ax=ax, color=pts_in["color"], markersize=sizes, alpha=0.5, edgecolor="none"
        )
    return pts_in
...
def render_xray_event(event, eds_2019, eds_2026, va_pts):
    ...
    # ---------------------------------------------
    ax1.set_title(
        "Panel 1: The Source (2019 Baseline)", fontsize=22, weight="bold", pad=15
    )
    ...
    if etype in ["Cracking", "Draining"]:
        ...
        if etype == "Cracking":
            ...
            gpd.clip(cutters, target_shape).plot(
                ax=ax1,
                facecolor="none",
            ...
            gpd.clip(swallower, target_shape).plot(
                ax=ax1,
                facecolor="none",
                edgecolor=border_color,
                linewidth=5,
    ...
    # ---------------------------------------------
    ax2.set_title(
        f"Panel 2: The Washout (2026 {mlabel})", fontsize=22, weight="bold", pad=15
    )
    ...
    fig.suptitle(
        f"Partisan X-Ray: {etype} of {title_target}", fontsize=30, weight="bold", y=1.05
    )
    ...
    fig.text(
        ...
        weight="bold",
        bbox=dict(
            facecolor="white", edgecolor="black", boxstyle="round,pad=1", linewidth=2
        ),
    )
    ...
    border_line = plt.Line2D(
        [0],
        [0],
        color=border_color,
        linewidth=4,
        linestyle="--",
        label=f"2026 {mlabel} Map Cut-Lines",
    )
    fig.legend(
        handles=[ndp_patch, ucp_patch, border_line],
        loc="upper right",
        fontsize=14,
