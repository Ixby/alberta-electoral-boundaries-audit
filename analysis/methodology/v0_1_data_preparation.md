...
python3 analysis/scripts/parse_2015_results.py
```
...
with pdfplumber.open('.temp/commission_report.pdf') as pdf:
    ...
    # Emit CSV from tables[0] (see analysis/scripts/parse_2015_results.py for table extraction pattern)
"
```
...
- `analysis/scripts/submission_search.py` — the reproducible pipeline
- `analysis/reports/submission_search_findings.md` — written verdict on the chair's "no public support" claim
...
python3 analysis/scripts/submission_search.py
```
