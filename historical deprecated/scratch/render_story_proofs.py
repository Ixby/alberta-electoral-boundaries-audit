...
def load_data():
    ...
    va_pts["color"] = va_pts.apply(
        lambda r: NDP_COLOR if r["va_ndp"] > r["va_ucp"] else UCP_COLOR, axis=1
    )
        ...
        if highlight_poly is not None:
            ...
            if not pts_fade.empty:
                pts_fade.plot(
                    ax=ax,
                    color="#e2e8f0",
                    markersize=sizes.loc[pts_fade.index],
                    alpha=0.3,
                    edgecolor="none",
                )
            if not pts_focus.empty:
                pts_focus.plot(
                    ax=ax,
                    color=pts_focus["color"],
                    markersize=sizes.loc[pts_focus.index] * 1.2,
                    alpha=0.9,
                    edgecolor="none",
                )
        else:
...
def render_story_event(event, eds_2019, eds_2026, va_pts, index, total_events):
    ...
    for ax in [ax_map1, ax_map2, ax_bar1, ax_bar2]:
        ax.axis("off")
    ...
    if etype == "Cracking":
        ...
        target_shape.plot(
            ax=ax_map1, facecolor="none", edgecolor="#64748b", linewidth=4
        )

        all_intersections = []
        for i, piece in enumerate(event["pieces"]):
            intersect = get_intersection(
                eds_2019, event["target_2019"], eds_2026, piece
            )
            c = HIGHLIGHT_COLORS[i % len(HIGHLIGHT_COLORS)]
            intersect.plot(
                ax=ax_map1, facecolor="none", edgecolor=c, linewidth=4, hatch="///"
            )
            all_intersections.append(intersect.geometry.iloc[0])
            carveout_patches.append(
                mpatches.Patch(
                    facecolor="none",
                    edgecolor=c,
                    hatch="///",
                    label=f"Transferred to {piece}",
                )
            )
        ...
        intersect = get_intersection(
            eds_2019, event["target_2019"], eds_2026, event["recipient_2026"]
        )
        ...
        carveout_patches.append(
            mpatches.Patch(
                facecolor="none", edgecolor=c, hatch="///", label=f"Transferred Area"
            )
        )

        plot_va_dots(
            ax_map1, target_poly, va_pts, highlight_poly=intersect.geometry.iloc[0]
        )
        pts1_frames = [va_pts[va_pts.geometry.within(target_poly)]]
        ...
        for i, donor in enumerate(event["donors"]):
            ...
            donor_shape.plot(
                ax=ax_map1, facecolor="none", edgecolor="#64748b", linewidth=2
            )
    ...
    if etype == "Cracking":
        for i, piece in enumerate(event["pieces"]):
            ...
            recipient.plot(
                ax=ax_map2, facecolor="none", edgecolor=border_color, linewidth=2
            )
            ...
            c = HIGHLIGHT_COLORS[i % len(HIGHLIGHT_COLORS)]
            intersect.plot(
                ax=ax_map2, facecolor="none", edgecolor=c, linewidth=4, hatch="///"
            )
            pts2_frames.append(
                va_pts[va_pts.geometry.within(recipient.geometry.iloc[0])]
            )

    elif etype == "Draining":
        swallower = eds_2026[eds_2026["name"] == event["recipient_2026"]]
        swallower.plot(
            ax=ax_map2, facecolor="none", edgecolor=border_color, linewidth=4
        )

        cx, cy = (
            swallower.geometry.iloc[0].centroid.x,
            swallower.geometry.iloc[0].centroid.y,
        )
        ax_map2.text(
            cx,
            cy,
            event["recipient_2026"].upper(),
            color="#000000",
            fontsize=20,
            weight="bold",
            ha="center",
    ...
    fig.text(
        0.5,
        0.48,
        f"Voters Displaced: {int(displaced_voters):,}",
        ha="center",
        va="center",
        fontsize=16,
        weight="bold",
        color="white",
        bbox=dict(
            facecolor="#0f172a", edgecolor="none", pad=8, boxstyle="round,pad=0.5"
        ),
    )
    ...
    if abs(net_swing) > 0.5:
        ...
        fig.text(
            0.5,
            0.17,
            f"NET SWING: {abs(net_swing):.1f}-Point {swing_party} Shift",
            ha="center",
            va="center",
            fontsize=20,
            weight="bold",
    ...
    fig.text(
        0.5,
        0.02,
        f"Forensic Evidence: Event {index} of {total_events} documented boundary manipulations.",
        ha="center",
        fontsize=14,
        color="#64748b",
        style="italic",
