...
def load_data():
    ...
    maj_map = gpd.read_file(
        DATA_DIR / "derived" / "v0_10_topological_majority_2026_eds.gpkg"
    ).to_crs(epsg=3401)
    maj_map = maj_map.cx[bbox[0] : bbox[2], bbox[1] : bbox[3]]
    min_map = gpd.read_file(
        DATA_DIR / "derived" / "v0_10_topological_minority_2026_eds.gpkg"
    ).to_crs(epsg=3401)
...
def plot_teardown(airdrie, major_hwys, maj_map, min_map, bbox):
    ...
    plt.suptitle(
        "Airdrie Partition: Majority (2 EDs) vs Minority (4 EDs)",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )

    for idx, (ax, ed_map, title) in enumerate(
        zip(
            axes,
            [maj_map, min_map],
            [
                "Majority Recommendation (2-way split)",
                "Minority Recommendation (4-way split)",
            ],
        )
    ):
