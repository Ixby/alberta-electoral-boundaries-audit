...
**Absolute-level Chen-Rodden decomposition (2026-04-24).** The pairwise-gap decomposition above collapses to "100 % drawing" by construction because both 2026 proposals share Alberta's voter geography and the same neutral ensemble. The *absolute-level* decomposition answers a separate and more substantively revealing question: how much of each map's partisan lean on each metric is natural geography (what the ensemble median produces under neutral drawing) versus specific drawing choices? Using the 100k-plan MCMC ensemble as the geography baseline and reading in the script-native convention (positive EG = NDP wastes more, per §5.4 line 540), the ensemble-median EG is **+1.49 %** — Alberta's natural geography produces this slight UCP-wastage tilt on any neutrally-drawn map (p5-p95 bracket [−0.97 %, +3.94 %]). Per-map drawing components (actual − geography median):

| Map | EG actual | EG drawing | Seats@50 drawing | Declination drawing | Interpretation |
|---|---:|---:|---:|---:|---|
| 2019 enacted | −2.64 % | **−4.12 pp** | +2.30 pp NDP | +0.054 NDP-packing | 4 pp UCP-favouring drawing on top of geography |
...
The **headline claim** the decomposition supports: *Alberta's natural geography produces a structural +1.5 % EG under neutral drawing. The 2019 map and the majority 2026 proposal both add ≈ 4 pp of additional UCP-favouring drawing on top of that floor. The minority 2026 proposal pulls the EG back to near-zero on that metric — a ~4 pp drawing displacement toward NDP relative to neutral, and **adds +5.75 percentage points of NDP-favouring responsiveness at the 50/50 vote scenario through asymmetric drawing** (strong NDP-packing declination signal paired with near-zero EG drawing).* This is the audit's cleanest drawing-attribution claim: rather than framing any single metric as decisive, it names **what specific draws produce what specific departure from Alberta's natural floor**, making the minority map's drawing signature visible as a distinct asymmetric-packing pattern rather than a symmetric pro-one-party tilt. Full decomposition table in `data/v0_1_chen_rodden_absolute_decomposition.json`; methodology and criticism / defense in `analysis/methodology/v0_1_test_apparatus_defense.md` §2.5.
