...
forward_dependencies:
  - analysis/methodology/v0_1_methodological_defenses.md §1.2 "The Centroid Fallacy (Ecological MAUP)" — quantified-insignificance line for insertion
backward_dependencies:
  - analysis/scripts/v0_9_va_attribution_area_weighted.py — area-weighted apportionment by intersection_area / VA_area
  ...
  - analysis/scripts/phase_4c_va_attribution.py — original centroid-based attribution
  - analysis/scripts/phase_4c_va_attribution_maup.py — prior MAUP work (full-vote variant)
  ...
  - data/v0_9_maup_centroid_sensitivity.json — full output
---
...
`analysis/scripts/v0_9_va_attribution_area_weighted.py` performs the overlay (`gpd.overlay` with `how="intersection"`), normalises per-VA weights to sum to 1.0, and apportions the per-VA UCP/NDP/other vote totals into per-ED CSVs. A nearest-ED fallback is applied to the 5 VAs whose polygons fall entirely outside the v0_9 ED union (lakes, edge-of-province slivers) — this matches the centroid pipeline's own `nearest_ed` fallback in `phase_4c_va_attribution.py`, ensuring like-for-like vote conservation. `analysis/scripts/_v0_9_maup_compare.py` then re-runs `seat_results()` (corrected two-party math, commit 5cc7e5c) on both attribution flavours and emits the comparison.
...
The centroid scores reproduce `data/v0_1_mcmc_real_map_scores_250k_v0_9.json` to all reported decimals — confirming the `seat_results()` baseline. The area-weighted scores diverge only at the 7th–10th decimal place. This is floating-point noise, not signal.
...
## One-liner for `v0_1_methodological_defenses.md` §1.2

> **Empirical refutation (2026-04-26):** `v0_9_va_attribution_area_weighted.py` re-attributes votes by `intersection_area / VA_area` instead of centroid-in-polygon; v0_9 majority and minority `seats@50/50` change by exactly +0.0000 pp under area-weighted attribution (other Lane 1 metrics shift by ≤1e-5 pp). Diagnostic: only 7 VAs have a >0.01 % area sliver in a second 2026 ED. The v0_9 topological shapefiles snap ED boundaries to VA boundaries, so MAUP is structurally nullified. See `analysis/reports/v0_9_maup_centroid_sensitivity.md` and `data/v0_9_maup_centroid_sensitivity.json`.
