...
def main():
    print(
        f"[{_ts()}] short-bursts NDP-maximization start (symmetric mirror of UCP run)",
        flush=True,
    )
    print(
    ...
    print(
        f"  initial seats@50/50 = {cur_score:.4f}  "
        f"(ucp_seats={cur_metrics['ucp_seats']}/{cur_metrics['n_districts']})",
        flush=True,
    )
