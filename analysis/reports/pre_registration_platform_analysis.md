...
forward_dependencies:
  - analysis/reports/pre_registration_draft.md (the submission-ready document itself)
...
**Scope:** Platform survey only. The pre-registration document itself is drafted in `analysis/reports/pre_registration_draft.md`; actual submission is reserved for the PO.
...
The pre-registration draft is `analysis/reports/pre_registration_draft.md`. The PO retains submission authority. The steps below assume OSF as the chosen platform per the recommendation above. Estimated wall time for a first-time user: 30–45 minutes.

### Pre-submission checklist

- [ ] Confirm the draft document (`analysis/reports/pre_registration_draft.md`) is final. Any edit after OSF submission creates a new version; the original timestamp is preserved but subsequent edits are visible.
