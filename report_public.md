...
*Killed by:* a falsification test designed before the data came back. R-SMC plans that reach the corrected 0.4831 value have mean Polsby-Popper compactness of 0.2391 — *more* compact than the 0.2339 of plans that don't reach it (p = 7.7 × 10⁻²³⁴). Compactness, the most obvious mechanistic suspect, is not the mechanism. Some other geometric feature is, and the audit doesn't yet know which one. (A second unrelated SMC issue — the sampler's tail probabilities swung from 5% to 58% across runs that differed only in PRNG-state consumption — separately disqualifies SMC as a co-equal source of truth here. ReCom converges to publication-grade Gelman-Rubin R̂ < 1.05; SMC, on this geometry, did not.)
