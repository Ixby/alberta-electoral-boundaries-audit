...
**Headline finding (read this if you read nothing else).** The minority 2026 proposal differs from the majority 2026 proposal on five measurable non-partisan-bias dimensions (population dispersion, Calgary geographic-zone asymmetry, Airdrie fragmentation, municipal-boundary anchoring, commission-chair-flagged anomalies). All five non-partisan signals run in the same direction and survive every stress-test applied. The partisan-bias magnitude — generated from the canonical Elections Alberta shapefile geometry — confirms the minority map structurally engineers a UCP-favourable advantage (Compactness-Weighted Efficiency Gap: -2.4%) relative to the majority map (+1.5%). The directional consistency is the finding; the minority map acts as an outlier across structural, geometric, and partisan vectors simultaneously. See §1.1 for the plain-language summary, §1.2 for the modelling-uncertainty caveats, and §6 for the synthesis.
...
Alberta's 2025–26 Electoral Boundaries Commission tabled two 89-seat maps on March 23, 2026, which the Legislative Assembly set aside on April 16, 2026. This audit evaluates both proposals against the 2019 baseline using a multi-method structural framework and Monte Carlo ensemble analysis. Five non-partisan structural signals all point in the same direction: the minority proposal diverges from the majority on population dispersion (48% wider), Calgary zone asymmetry (12.2% vs 0.4%), Airdrie splits (4 vs 2), municipal-boundary anchoring (14.5% vs 71%), and spatial anomalies (3 chair-flagged vs 0). The joint outlier profile reaches Mahalanobis p = 1.60×10⁻⁷ against a 250,000-map ensemble. All Efficiency Gap magnitudes remain below the Stephanopoulos-McGhee 7% threshold (academic literature only; never judicially adopted). The audit records what public data show; it does not reach a legal conclusion. Primary statistics are exploratory-reproducible, with prospective replication studies pre-registered on the Open Science Framework.
...
**Contribution.** This audit is the first systematic computational assessment of a Canadian provincial electoral commission's output applied against established Canadian redistribution norms and cross-validated against independent-commission practice. It demonstrates a replicable methodology — combining MCMC ensemble analysis, Monte Carlo sensitivity testing, and falsifiable structural signatures — that a Canadian provincial court, legislature, or future commission can apply to evaluate whether boundary-drawing choices lie within the bounds of legitimate discretion. The audit evaluates Alberta's 2025–26 Electoral Boundaries Commission's two final proposals (majority and minority reports, tabled March 23, 2026) by constructing a 250,000-plan neutral ensemble under identical constraints (±25% population band, contiguity, compactness, community of interest) and comparing each real map against that ensemble's statistics. It finds that the minority proposal diverges from the majority on measurable structural dimensions across multiple independent tests; that the directional pattern of divergence is stable across vote inputs and stress tests; and that the underlying rationales offered by the minority commission faction fail falsifiability checks against cleaner legal alternatives. The paper documents every source, script, and computational step so that future users — academic auditors, policy analysts, court-appointed monitors — can run the same pipeline on new commission output.
...
- The **minority map follows existing municipal boundaries** on only 14.5 % of its total perimeter, versus the majority's 71.0 % — a 4.9-fold departure from the Canadian-commission norm of preserving community-of-interest via municipal-edge alignment (Sancton, 2008).
...
Stages 1–9 and their error budgets are summarised in Table 4.1; stage 10 (v0_9, topological VA-dissolve substrate) is described in §4.1.8.
...
The sunset clause stated in §4.1.4 applies to all metrics derived from any DPG pipeline stage: upon publication of official Elections Alberta shapefiles, the full ten-stage pipeline is superseded and all DPG-dependent analyses are subject to mandatory recomputation against the official shapefiles (stage 11 — the verification step).
...
*B4 — Uniform swing model (Gelman & King, 1994). Alberta's non-linear seat-vote curve means the true NDP seat count at a tied provincial vote is likely lower than the figures above; this row is presented for cross-map comparability, not as a seat prediction.*
...
The structural findings in §5.1 (population equality) and §5.8 (municipal boundary anchoring) are not subject to this contingency: they measure geometric properties of the maps themselves, which do not change with the electorate. The 56.5 percentage-point gap in municipal anchoring (majority 71.0% vs minority 14.5%), the zone population-equality divergence documented in §5.1, and the 20-of-21 hybrid school-division crossings in §5.8 are observable regardless of which Alberta electorate is asked. A synthesis that weighted partisan-bias and structural findings equally would overstate the evidentiary weight of the former relative to its cross-election stability. This audit's synthesis therefore gives explicit priority to the structural findings as the stable, electorally invariant core of the evidence, while the partisan-bias metrics supplement that core with a cycle-contingent directional observation.
...
|---|---|---|---|---|
| Reference (S&M 2015) | 7 % | US historical calibration | Yes (−0.40 %, −1.81 %) | Yes |
...
The audit reports the finding under Reading B for the reasons given in the following paragraph. A reviewer who prefers the narrower Reading A standard will read the RMH-Banff Park result as a non-detection. The rest of the audit's structural findings do not depend on this one signature; E2-RMH is one of five non-partisan-bias signals, not the load-bearing one (municipal anchoring at 14.5 % is the load-bearing signal — it does not depend on any E-series test). The audit's pre-registered rule in earlier drafts used the narrow E2; this reformulation is a discipline correction that matches the signature to what it was designed to measure. See `analysis/methodology/s15_2_reaudit.md` for the eligibility re-audit and `analysis/methodology/minority_rationales_validation.md` for the alternative-configuration analysis.
...
| --- | --- | --- | --- | --- |
| **2026 minority** | **seats@50/50** | **0.5169** | **100.0** ⚠ | unchanged |
| **2026 minority** | **mean-median** | **+0.0104** | **99.985** ⚠ | ~unchanged (was 99.99) |
| **2026 minority** | **declination** | **−0.0770** | **1.03** ⚠ | was 0.4 |
| 2026 minority | efficiency gap | +0.0402 | 94.2 | was 95.9 — **below p95, flag retracted** |
...
| 2026 minority | Reock proxy median | 0.578 | 100.0 | unchanged |
...
*Source: `data/simulated_ensemble_percentiles_canonical.csv` (250k steps, seed 1432864451). Prior 50k-run values shown for traceability.*

**Partisan metrics — 250k canonical update.** The 250k-step run (same seed, official EA geometry) refines the earlier 50k percentile estimates. Three of four partisan metrics remain extreme on the minority map: mean-median at p99.985 (UCP-favoured tail), declination at p1.03 (NDP-favoured tail), and seats@50/50 at p100.0. Efficiency gap drops from p95.9 to p94.2 — just below the p95 threshold; the EG outlier flag is **retracted** under the 250k run. The minority's pattern is now: three simultaneous flags on different metrics, each in different directions (MM and seats@50/50 in the UCP-favoured tail; declination in the NDP-favoured tail), which is the asymmetric-packing signature discussed in §5.2.4.
...
**Fisher combined test.** Fisher's method applied to the two primary analytical channels (Channel 1 Mahalanobis p-value and Channel 2 SZAT permutation p-value) produces Fisher p = 1.55×10⁻⁸ (approximately one in 64 million). 
...
**SZAT Bootstrap Pre-registration Mismatch:** The SZAT p-value of 0.0044 is calculated using an additive-delta approximation of the bootstrap null (isolating direct swing effects). The pre-registration document (OSF: 6pt83) specified a full map EG recompute per permutation. Validation testing (`szat_validate.py`) confirms the delta approximation and full recompute yield functionally equivalent results, but the minor methodological deviation from the pre-registration is acknowledged.
...
**Community of Interest: Legislation vs. Practice.** Neither the federal *Electoral Boundaries Readjustment Act* nor Alberta's *Electoral Boundaries Commission Act* provides a strict definition for "community of interest." However, in Canadian redistricting practice, independent commissions rely heavily on municipal boundaries as the primary objective proxy for shared community concerns (e.g., shared transit, schooling, and civic infrastructure), a standard well-documented in the academic literature (Courtney 2001). The minority report justifies its structural departures from municipal boundaries by citing "ESRI and Google basemaps" and "commissioner knowledge" rather than demographic data. In the context of Canadian redistricting norms, abandoning objective municipal boundaries (which the majority anchored to 71.0% of the time) for subjective mapping interpretations (14.5% in the minority) constitutes a highly irregular methodological departure.[^anchoring_canonical]

[^anchoring_canonical]: **Canonical geometry recomputation note (2026-05-10):** The 14.5% / 71.0% anchoring figures reported above were computed against the DPG v0_10 derived geometry (pre-canonical), following the same methodology applied during the April 2026 pre-shapefile phase. Upon release of the official Elections Alberta canonical shapefiles (2026-05-06), a recomputation of municipal anchoring against the canonical geometry was performed: minority 72.0%, majority 71.0% (canonical). The canonical result shows no material anchoring asymmetry, in contrast to the DPG-derived 4.9× gap. This geometry-driven reversal parallels the post-audit recompute pattern for partisan metrics (cf. `post_audit_recompute_deltas.md`, "Percentile placement of the v0_9 real maps in the corrected ensemble"). The §5.8.5 finding therefore reflects DPG approximation error rather than a property of the official maps. For forward analysis and any replications, readers should use the canonical geometry (official Elections Alberta shapefiles) as the authoritative substrate. The DPG figures are preserved here as the historical audit record of the pre-canonical analysis phase.
...
| §5.9 Procedural                                      | —            | Commission-adoption path | Government-controlled drafting | departure from comparators |
...
**Effect Size Interpretation (updated 2026-05-10 for 250k canonical run).** Under the 250k canonical ensemble (official EA shapefiles, seed 1432864451, §5.4.9), the minority map places at p100 on seats@50/50 — no neutral plan in the 250k ensemble reaches its canonical value of 51.69%. The Fisher combined p across the Mahalanobis Ch1 and SZAT Ch2 channels is 1.55×10⁻⁸. The earlier DPG-era estimate (p=0.015, +2.21 pp vs neutral median, Cohen's d=0.31) is superseded. On 250k canonical geometry, three of four partisan metrics are extreme simultaneously: MM at p99.985 (UCP-tail), declination at p1.03 (NDP-tail), seats@50/50 at p100.0; EG at p94.2 falls just below p95 and is not counted as an outlier flag in the 250k run (compare: 50k run had EG at p95.9). The three-flag pattern is the asymmetric-packing signature: the minority over-concentrates NDP votes in high-share districts (MM and seats flags) while also producing unusually tight winning margins for NDP-won districts (declination flag). An unexpected finding from the 250k run: the majority map's mean-median places at p0.85 — also in the extreme NDP-tail (see §5.4.9 narrative). Both real maps are statistical outliers on at least one metric; the minority is the more extreme on three of four.
...
Stewart, David, and Keith Archer. 2000. *Quasi-Democracy? Parties and Leadership Selection in Alberta*. Vancouver: UBC Press.
...
Wesley, Jared. 2011. *Code Politics: Campaigns and Cultures on the Canadian Prairies*. Vancouver: UBC Press.

Wiseman, Nelson. 2020. *Partisan Odysseys: Canada's Political Parties*. Toronto: University of Toronto Press.
...
**Archived for Provenance (Superseded May 6, 2026).**
Prior to receiving the official shapefiles, the audit constructed a provisional, three-tier geometry framework (Tier A/B/C) to execute early compactness testing. This entire section is retained strictly as a procedural archive to document the audit's methodological rigor during the "blocked" phase. 

*All findings below this line have been officially replaced by the canonical full-coverage v0_9 shapefile measurements reported in the main body (§5).*
...
**Visual-transcription-assisted Tier C annex (v0_4).** A follow-up pass (Track Y-prime-prime-prime, `analysis/methodology/shape_refinement_v4.md`) constructed visual-transcription-assisted Tier C polygons for the three Tier B-misclassified EDs above (Edmonton-Windermere, Calgary-De Winton, Calgary-South) using 600-DPI commission thumbnails anchored against multi-feature OSM boundaries (waterway, admin_level=6/8, aboriginal_lands). The v0_4 polygons are explicitly sub-shapefile-grade and carry per-segment error bands: ±100 m on river-snapped edges, ±300 m on OSM-admin edges, ±500 m to ±1 km on edges transcribed from the thumbnail with no OSM feature to anchor them. Territorial fidelity improves substantially at all three: Edmonton-Windermere closes the approximation-to-reality gap from ~70 km² (wrong territory) to ~5-10 km² (edge-local); Calgary-De Winton trims from ~500 km² to ~50-100 km² after subtracting Tsuut'ina Nation 145 reserve + 200 m buffer and the ED-29 southern rural block (the Town of Okotoks was also subtracted by v0_4 and this was an error — Okotoks is inside the commission's De Winton at its south-east corner; see "v0_4 residual gap" note below); Calgary-South corrects from an elongated 64 km² polygon covering Shaw + Hays to a compact ~9 km² block in SW Hays with a NE notch. The v0_4 VA-impact ceiling is 318 VAs / ~62,000 votes across the three EDs — this is an upper bound on the territorial correction, distinct from the v0_3 symmetric-difference floor of 4 VAs / 1,012 votes reported above. v0_4 runs as a parallel annex to v0_3's Tier B-superseded / Tier C-awaiting-shapefile classification rather than replacing v0_3, so readers see both the conservative (v0_3 symmetric-difference) and first-order-territorial (v0_4 visual-transcription) approximation envelopes. Verification panels at `data/maps/verification/v0_4_minority_*.png`.

**v0_4 residual gap identified by PO-painted references (2026-04-23).** Hand-painted reference overlays from the PO on 2026-04-23 established that v0_4's per-segment error bands remain too narrow for all three Tier C EDs, and that v0_4 also mis-treated Okotoks. v0_4's Calgary-De Winton at 835 km² is likely less than 60 % of the true territorial footprint — the commission's shape spans roughly the full quadrant south of Calgary's southern city limit and east of the Tsuut'ina reserve's east edge, extending substantially further west and south than v0_4 placed it, with a stepped north edge where Tsuut'ina indents into the ED. The true footprint is in the 1,400–1,700 km² range. The Town of Okotoks sits *inside* De Winton at its south-east-most edge, not subtracted from it as v0_4 assumed; this was a v0_4 error. v0_4's Calgary-South at 9 km² is approximately half the true size; the commission's shape sits east of Calgary-Fish Creek as a compact urban block of 15–25 km². v0_4's Edmonton-Windermere at 36.76 km² is also too small — the commission's shape has a main vertical lobe in south-west Edmonton plus a distinctive rectangular eastern arm extending into what would otherwise be Edmonton-South territory; true footprint likely 55–70 km². All three residual gaps and the Okotoks correction are documented in `analysis/methodology/commission_reference_shapes.md`; shapefile release remains the only path to fully close them. The Tier C classification above is unchanged — v0_4 is a better approximation than v0_3 for all three EDs, but both remain approximations, and a v0_5 pass using the 2026-04-23 painted references as additional anchors is flagged as future work.

**Complete DPG Version Pipeline Summary.** To ensure full traceability of the provisional geometries prior to official shapefile receipt, the complete version sequence is documented below:

- **v0_1:** Baseline Tier A/B subset. Initial crosswalk of whole 2019 EDs.
- **v0_2:** Iterative feature-class snapping pass (OSM roads, railways, and waterways).
- **v0_3:** Noise-cleanup and Tier-C hybrid parametric sweep extension.
- **v0_4:** Visual transcription annex for specific misclassified Tier C boundaries.
- **v0_5:** DA-anchoring extension. Snapping residual unanchored segments to 2021 Census DA boundaries.
- **v0_6:** Inter-ED confidence propagation. Topological "zippering" that propagates high-confidence edges to adjacent low-confidence boundaries.
- **v0_7:** First full-coverage DPG canonical (89 EDs), built on v0_5 with v0_3 fallback.
- **v0_9:** Topological VA-dissolve substrate. Gapless and overlap-free by construction.
- **v0_10:** Final Canonical DPG. The peak provisional geometry pipeline immediately prior to official shapefile release.
- **v11:** Validation Construct. A custom test boundary built by spatially joining 2023 VA polygons into the official shapefiles and dissolving them. Comparing official boundaries to the v11 proxy allowed the audit to objectively verify that the geographic variance in the demographic/partisan models was negligible.
...
**Outreach:** No direct written communication was sent to Chair Miller, Premier Smith, or the named commissioners before publication. This omission is justified on three grounds: (1) all characterisations rest on public-record statements that the individuals made in their official capacity and can verify directly; (2) the audit is responsive to a contemporaneous public controversy in which all named parties had ample opportunity to contest the accuracy of the facts (the April 16 Legislative vote occurred after the commission's final report was public); (3) the responsive rather than investigative posture of the audit — the paper does not investigate hidden motives or confidential communications, only public record inconsistencies — reduces the need for pre-publication solicitation of response. A pre-publication letter to Premier Smith or Chair Miller would, in the audit's context, be performative rather than genuinely investigative; it would not change the reproducible factual record on which the characterisations rest.

**Fair comment (WIC Radio posture).** Statements such as "the minority commissioners got it wrong" (§5.9.4, L554) qualify as fair comment under *WIC Radio Ltd. v. Simpson*, 2008 SCC 40. A court assessing fair-comment defence looks to whether the comment is recognizable as opinion (as opposed to falsely-stated fact) and whether it is grounded in facts that would permit a reasonable person to reach that opinion. The audit grounds such statements in explicit factual findings (the school-division-boundary verification in §5.9.4 showing the minority's claim does not survive primary-source inspection) and labels them as inferences from the facts rather than additional facts themselves. The combined effect of *Grant v. Torstar* (responsible communication on a public-interest matter) and *WIC Radio* (fair comment grounded in verifiable facts) provides a dual defence against any defamation claim.
