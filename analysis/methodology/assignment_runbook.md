...
## Inputs available (produced by `analysis/scripts/assignment_prep.py`)
...
| `analysis/reports/va_spatial_integrity_report.md` | Gate S3b (centroid-in-ED, 99.20%) and Gate S3c (vote conservation, 0.0000% diff) results. |
...
Output: `analysis/assignment_va_to_2026_assignments.csv` -
...
Output: `analysis/assignment_2026_synthetic_totals.csv` (one row per 2026 ED
in each map, with synthetic 2023 EDay two-party totals).

## Stage 6 - Packing & cracking re-run

Re-execute `analysis/scripts/packing_cracking_analysis.py` with the
...
Output: `analysis/reports/assignment_gerrymander_comparison.md` with v0.2 vs 4C
metric deltas and significance.

## Stage 7 - Monte Carlo CIs

Re-run `analysis/scripts/monte_carlo_ci.py` on the VA-resolution data to get
