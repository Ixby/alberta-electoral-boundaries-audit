...
<script lang="ts">
	...
	// Form state (Svelte 5 runes). Name and email are OPTIONAL — collected only so
	// the author can reply to the submitter. The author's own contact details are
	// never shown on this form.
	let name = $state('');
	let email = $state('');
	let message = $state('');
