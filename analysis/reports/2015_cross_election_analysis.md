...
(`alberta_2019_results.csv`) and the 2026 hybrid crosswalks.

Coverage (see `data/2015_to_2019_crosswalk.csv`):
...
Pre-2017 ED vote totals (from `data/alberta_2015_results.csv`)
...
`compute_metrics()` from `analysis/scripts/packing_cracking_analysis.py`
is invoked for:
...
`analysis/scripts/monte_carlo_ci.py` prints "Samples with minority more
...
> final report crosswalk (2015_to_2019_crosswalk.csv, 87/87
...
Ancillary correction: audit scripts in analysis/scripts/monte_carlo_ci.py
...
- `data/2015_to_2019_crosswalk.csv` (127 rows, 87 2015 EDs,
  87 2019 EDs, 58 high + 69 medium confidence).
- `data/2015_cross_election_summary.csv` (per-map B1-B6 results
  under 2015 votes).
- `data/cross_election_asymmetry_3way.csv` (three-election
...
`analysis/scripts/monte_carlo_ci.py` lines 158-159 print:
...
packing_cracking_analysis.py. Under that formula:
