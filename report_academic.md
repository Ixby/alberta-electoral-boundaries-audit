...
**Absolute-level Chen-Rodden decomposition (2026-04-24, narrowed under pre-committed exoneration framework).** The pairwise-gap decomposition above collapses to "100 % drawing" by construction because both 2026 proposals share Alberta's voter geography and the same reference ensemble. The *absolute-level* decomposition answers a separate and more substantively revealing question: how much of each map's partisan lean on each metric is natural geography (what the 100k-plan MCMC **geometric baseline** produces under ±25 %-population-constrained neutral drawing) versus specific drawing choices? Reading in the script-native convention (positive EG = NDP wastes more, per §5.4), the geometric baseline (ensemble median) EG is **+1.49 %** with a 5–95 band of [−0.97 %, +3.94 %]. The "geometric baseline" terminology replaces "ensemble median" in this section and §5.4 to acknowledge honestly that the MCMC ensemble does not represent the *human trade-offs* of a public hearing (community-of-interest preservation, Indigenous consultation, Chair Recommendation 5 conditions); a residual drawing component of ≈ 0.5 pp may reflect the *cost of community cohesion*, not partisan intent. Per-map drawing components (actual − geometric-baseline-median):

| Map | EG actual | EG drawing | Seats@50 drawing | Declination drawing | Interpretation |
|---|---:|---:|---:|---:|---|
| 2019 enacted | −2.64 % | **−4.12 pp** | +2.30 pp NDP | +0.054 NDP-packing | 4 pp UCP-favouring drawing on top of geometric baseline |
...
**Pre-committed exoneration check** (per `analysis/methodology/v0_1_null_hypothesis_and_exoneration_criteria.md` §2.5): a minority drawing component is *exonerating* (within what neutral drawing on the geometric baseline could produce) if it falls inside the ensemble 5–95 band. Checking each minority metric against its band:
