"""
Wuff voice and readability checker.

Wuff voice rules (from project styles):
...
# Patterns that violate Wuff voice. Each is (regex, description).
...
def check_file(path: Path, target_grade: float | None = None) -> tuple[bool, list[str]]:
    ...
    # Wuff voice violations are fatal; informational lines are not
    fatal = any(
