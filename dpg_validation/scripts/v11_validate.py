...
V010_BASELINE = {
    "majority": {
        "mean_iou": 37.78, "min_iou": 0.0,
        "t1_misassign_pct": 28.71, "eg_pct": 0.636,
        "anchoring_pct": 71.0, "nw_excess_pct": 2.8, "airdrie_count": 2,
        "t3_mean_hausdorff_m": 19070, "t3_max_hausdorff_m": 94993,
        "t5_extra_edges": 68, "t5_missing_edges": 60,
    },
    "minority": {
        "mean_iou": 36.32, "min_iou": 0.0,
        "t1_misassign_pct": 32.91, "eg_pct": 0.111,
        "anchoring_pct": 14.5, "nw_excess_pct": 11.5, "airdrie_count": 4,
        "t3_mean_hausdorff_m": 22564, "t3_max_hausdorff_m": 136657,
        "t5_extra_edges": 136, "t5_missing_edges": 103,
    },
}

# Official geometry results (from T4 rerun on official) for reference
OFFICIAL_BASELINE = {
    "majority": {"eg_pct": 0.083, "anchoring_pct": 48.8, "nw_excess_pct": 0.8, "airdrie_count": 2},
    "minority": {"eg_pct": 3.999, "anchoring_pct": 41.1, "nw_excess_pct": 6.3, "airdrie_count": 4},
}
...
def compute_iou(v11_gdf, off_gdf, label):
    ...
    for _, off_row in off_gdf.iterrows():
        ...
        rows.append({"map": label, "official_ed": off_name, "v11_ed": v11_name,
...
def t1_misassignment(va_gdf, v11_gdf, off_gdf, label):
    ...
    v11_join = gpd.sjoin(centroids[["geometry"]], v11_gdf[["geometry","EDName2025"]],
                         how="left", predicate="within")
    off_join = gpd.sjoin(centroids[["geometry"]], off_gdf[["geometry","EDName2025"]],
                         how="left", predicate="within")
    ...
    print(f"  {label} T1: {n_mismatch}/{n_total} VA centroids misassigned in v11 ({pct:.2f}%)")
...
def t2_area(v11_gdf, off_gdf, label):
    ...
    for _, off_row in off_gdf.iterrows():
        ...
        rows.append({"ed": name, "off_area_km2": round(off_area/1e6, 2),
                     "v11_area_km2": round(v11_area/1e6, 2), "abs_err_pct": round(err_pct, 2)})
    df = pd.DataFrame(rows)
    mean_err = df["abs_err_pct"].mean()
    max_err  = df["abs_err_pct"].max()
    n_over3  = (df["abs_err_pct"] > 3).sum()
    print(f"  {label} T2: mean area error {mean_err:.1f}%, max {max_err:.1f}%, {n_over3} EDs >3%")
...
def t3_hausdorff(v11_gdf, off_gdf, label):
    ...
    for _, off_row in off_gdf.iterrows():
        ...
        if name in v11_gdf["EDName2025"].values:
            ...
            hd = np.nan
        rows.append({"ed": name, "hausdorff_m": round(hd, 1) if not np.isnan(hd) else None})
    df = pd.DataFrame(rows).dropna()
    mean_hd = df["hausdorff_m"].mean()
    max_hd  = df["hausdorff_m"].max()
    n_over2k = (df["hausdorff_m"] > 2000).sum()
    print(f"  {label} T3: mean Hausdorff {mean_hd:.0f} m, max {max_hd:.0f} m, {n_over2k} EDs >2 km")
...
def agg_votes(va_gdf, ed_gdf):
    ...
    joined = gpd.sjoin(centroids[["geometry","va_ndp","va_ucp"]],
                       ed_gdf[["geometry","EDName2025"]], how="left", predicate="within")
    missed = joined[joined["EDName2025"].isna()].index
    if len(missed):
        nearest = gpd.sjoin_nearest(centroids.loc[missed, ["geometry","va_ndp","va_ucp"]],
                                    ed_gdf[["geometry","EDName2025"]], how="left")
...
def nw_calgary_excess(vote_df):
    nw_eds = [e for e in vote_df["ed"] if "Calgary-North West" in e
              or "Calgary-Symons" in e or "Calgary-Edgemont" in e
              or "Calgary-Nose Creek" in e or "Calgary-North" in e
...
def t4_metrics(va_gdf, v11_gdf, off_gdf, csd_gdf, label):
    ...
    print(f"  {label} T4: EG={eg:+.3f}%  anchoring={anchor:.1f}%  NW_excess={nw:+.1f}%  Airdrie={airdrie}")
    return {"eg_pct": round(eg, 3), "anchoring_pct": round(anchor, 1),
            "nw_excess_pct": round(nw, 1), "airdrie_count": airdrie}
...
def t5_adjacency(v11_gdf, off_gdf, label):
    ...
    print(f"  {label} T5: v11={len(v11_adj)} official={len(off_adj)} "
...
def main():
    ...
    for label, v11_path, off_path in [
        ...
        row = {
            "map": label,
            # IoU
            "v010_mean_iou": b["mean_iou"], "v11_mean_iou": round(mean_iou, 2),
            "v010_min_iou":  b["min_iou"],  "v11_min_iou":  round(min_iou, 2),
            # T1
            "v010_t1_misassign_pct": b["t1_misassign_pct"], "v11_t1_misassign_pct": round(t1_pct, 2),
            # T2
            "v11_t2_mean_area_err_pct": round(t2_mean, 1), "v11_t2_n_over3pct": t2_n,
            # T3
            "v010_t3_mean_hd_m": b["t3_mean_hausdorff_m"], "v11_t3_mean_hd_m": round(t3_mean, 0),
            "v010_t3_max_hd_m":  b["t3_max_hausdorff_m"],  "v11_t3_max_hd_m":  round(t3_max, 0),
            # T4
            "v010_eg":        b["eg_pct"],       "v11_eg":        t4.get("eg_pct"),
            "official_eg":    o["eg_pct"],
            "v010_anchor":    b["anchoring_pct"], "v11_anchor":    t4.get("anchoring_pct"),
            "official_anchor":o["anchoring_pct"],
            "v010_nw":        b["nw_excess_pct"], "v11_nw":        t4.get("nw_excess_pct"),
            "official_nw":    o["nw_excess_pct"],
            "v010_airdrie":   b["airdrie_count"], "v11_airdrie":   t4.get("airdrie_count"),
            # T5
            "v010_extra_edges":   b["t5_extra_edges"],   "v11_extra_edges":   extra,
            "v010_missing_edges": b["t5_missing_edges"], "v11_missing_edges": missing,
            "off_edges": off_edges,
        }
        sum_rows.append(row)
        if t4:
