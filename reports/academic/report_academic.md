...
*Severity-audit disclosure (T1.17 closed 2026-06-12 per T1.7 R1 Ref #16 D25).* The "commission convention" auxiliary above absorbs the majority's MM outlier (p0.92) and the Reock / CSD-anchoring p100 placements on both maps. The same auxiliary is *not* applied to the minority's MM (p99.98), declination (p98.79), or s50 (p99.99) tails, which are drawn from the same unconstrained ensemble. Mayo's severity discipline (Mayo 2018, SIST Excursion 2) requires the auxiliary be applied symmetrically — every metric whose neutral-ensemble distribution is shaped by an unenforced commission convention should be discounted, not just the metrics where the discount weakens the audit's preferred conclusion. The audit's response: **rename the null "ReCom-typical plan" everywhere in the §6.2 verdict text** rather than "neutral commission drawing," because the actual ensemble samples ReCom-typical plans, not constraint-enforcing-commission-typical plans. The constraint-enforcing ensemble queued as T1.4 is the methodologically complete answer (it would discount all four direction-aligned minority metrics by the same factor); until that lands, the audit's headline reads under the "ReCom-typical" framing and treats the minority's three tail flags as outliers relative to the *unconstrained sampling distribution* — not relative to the legal map space. The Bonferroni headline (p ≤ 2.80×10⁻⁶) holds under the ReCom-typical interpretation; under the constraint-enforcing interpretation, the bound is conservative.
