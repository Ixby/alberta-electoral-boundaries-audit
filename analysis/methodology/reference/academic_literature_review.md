...
- **McCartan, Cory, and Kosuke Imai. 2023.** "Sequential Monte Carlo for Sampling Balanced and Compact Redistricting Plans." *Annals of Applied Statistics* 17(4): 3300–3323. — the foundational SMC method underlying the R `redist` package's `redist_smc()` sampler. The audit uses this sampler for algorithm-independent corroboration of the Python GerryChain ReCom headline finding in `analysis/scripts/redist_crossvalidation.R` (cited at monograph §5.4.9). Should be cited wherever the SMC cross-validation is described.
- **Cannon, Sarah, Ari Goldbloom-Helzner, Varun Gupta, JN Matthews, and Bhushan Suwal. 2023.** "Voting Rights, Markov Chains, and Optimization by Short Bursts." *Methodology and Computing in Applied Probability* 25(36). (arXiv preprint 2022:2011.02288.) — introduces the short-bursts hill-climbing procedure used in monograph §5.4.8 to test whether a non-neutral procedure can reach the minority's seats@50/50 reading under EBCA constraints. The audit cites the journal version (2023), not the preprint year.
