...
> **Empirical refutation (2026-04-26):** `va_attribution_area_weighted.py` re-attributes votes by `intersection_area / VA_area` instead of centroid-in-polygon; v0_9 majority and minority `seats@50/50` change by exactly +0.0000 pp under area-weighted attribution (other Lane 1 metrics shift by ≤1e-5 pp). Diagnostic: only 7 VAs have a >0.01 % area sliver in a second 2026 ED. The v0_9 topological shapefiles snap ED boundaries to VA boundaries, so MAUP is structurally nullified. See `analysis/reports/maup_centroid_sensitivity.md` and `data/maup_centroid_sensitivity.json`.
