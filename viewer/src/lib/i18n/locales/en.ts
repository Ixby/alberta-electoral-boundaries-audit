...
export default {
	...
	body: {
		...
		clean: {
			...
			prereg_body: "Pre-registration means writing down the exact tests, thresholds, and predicted directions before looking at any data, and locking those commitments into a public time-stamped record. The Open Science Framework (OSF) is the public repository where this audit's commitments are filed. It prevents retrofitting: if a result doesn't emerge cleanly, the threshold cannot be changed after the fact and then presented as always having been the test. The audit's random seeds were committed to a public, time-stamped beacon (Cloudflare's drand) before the official Elections Alberta shapefiles were released, so the simulations could not be re-run toward a chosen result. The neighbour-drain test was fully pre-registered on OSF — its threshold and predicted direction fixed before measurement. The remaining tests are reported as exploratory: their seeds were pre-committed, but the specific test names and the method of combining them were not registered on OSF before they were run, and the report labels each finding accordingly.",
