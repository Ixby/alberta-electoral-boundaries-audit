...
| `hardcoded_values_audit.md` | PREP COMPLETE 2026-05-24, NOT AUTHORIZED | Inventory of every hardcoded literal in the Phase B scorecard pipeline, classified by necessity. Flags one real inconsistency: `phase_b_scorecard.py` maintains two separately-defined city lists (10 cities in mo1's drain check, 8 in mo2's lasso-urban-share denominator) with no cross-reference. Drafts a runtime consistency check; resolution requires a PI methodology decision (intentional asymmetry or drift bug) before the check can land. |
