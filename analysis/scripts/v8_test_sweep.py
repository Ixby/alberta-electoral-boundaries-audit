...
def run_one(script: str, args: list[str], label: str) -> dict:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"label": label, "script": script, "ok": True,
                    "elapsed": elapsed, "stdout_tail": result.stdout[-400:]}
        else:
            print(f"  ✗ FAILED  {label:<30s}  exit={result.returncode}  ({elapsed:.0f}s)",
                  flush=True)
            return {"label": label, "script": script, "ok": False,
        ...
        return {"label": label, "script": script, "ok": False,
                "elapsed": elapsed, "exit_code": "timeout"}
    except Exception as e:
        elapsed = time.time() - t0
        print(f"  ! ERROR   {label:<30s}  {e}", flush=True)
        return {"label": label, "script": script, "ok": False,
                "elapsed": elapsed, "exit_code": str(e)}


def main() -> int:
    n_workers = min(8, len(TESTS), os.cpu_count() or 4)
    print(f"[v0_8 test sweep] launching {len(TESTS)} test scripts with "
