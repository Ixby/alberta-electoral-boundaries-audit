...
Forward:
  findings/partisan_bias_summary_canonical.json — machine-written each run (added
    2026-07-12; this script previously wrote nothing, stdout only, despite this
    docstring claiming it produced the two files below, so anything citing them
    was necessarily hand-transcribed from console output)
  findings/partisan_bias_summary.md, findings/extended_partisan_metrics.md — older,
    hand-maintained narrative writeups; not generated by this script; cross-checked
    2026-07-12 against a fresh run and found to match (EG +3.96%/+0.04%, MM
    +1.03pp/-3.64pp, seats@50/50 43/48, asymmetry +3.92pp) — treat the JSON above
    as the reproducible source, these as prose built on top of it
"""
...
def main():
    ...
    # Persist — this script previously wrote nothing (stdout only) despite
    # its own docstring's "Forward:" section claiming it produced
    # findings/partisan_bias_summary.md and findings/extended_partisan_metrics.md;
    # anything citing those files was necessarily hand-transcribed from
    # console output. Fixed 2026-07-12.
    if m_map_a and m_map_b:
        import json
        out_path = Path(__file__).resolve().parent.parent.parent / "findings" / "partisan_bias_summary_canonical.json"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "method": "Phase 4C exact VA-level spatial attribution "
                              "(packing_cracking_analysis.py v0.3)",
                    "2019_enacted": m_2019,
                    "majority_2026": m_map_a,
                    "minority_2026": m_map_b,
                    "delta_from_2019_eg_majority_pp": round(delta_map_a * 100, 2),
                    "delta_from_2019_eg_minority_pp": round(delta_map_b * 100, 2),
                    "minority_majority_asymmetry_pp": round((delta_map_b - delta_map_a) * 100, 2),
                },
                f,
                indent=2,
            )
