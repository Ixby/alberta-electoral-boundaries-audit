...
and `plain_language_defense_summary.md` (six-objection quick reference).
The summary file is removed; this document is the canonical record.*

---

## Quick Reference — Six Common Objections
...
The simulation does not draw arbitrary fake maps. It draws random maps subject to the same legal rules the commission was required to follow: districts must be connected, roughly equal in population, and satisfy the same geographic constraints. The result is a reference band — the range of outcomes a fair, unbiased process would normally produce. When the minority map's scores fall outside that band, it means they are more extreme than nearly every one of the million randomly drawn neutral maps. The randomness itself is auditable: all random seeds come from a public cryptographic timestamp service (operated by Cloudflare), committed to git before the analysis ran. No seed was chosen after the results were seen.

### 4. "You picked a threshold that makes the minority map fail."

The threshold — 4.10% efficiency gap — was not chosen by the researcher. It is the value that 95% of the million randomly drawn neutral Alberta maps fall below, computed from the same 2023 election that produced these maps. It is Alberta's own ceiling for what a neutral process normally produces, not a number imported from another context. The same computation was run using vote data from the 2019 and 2015 elections; the minority map falls below the ceiling under 2019 and 2023 conditions. The majority map falls below the ceiling in all three. The US academic reference threshold of 7% sits in the middle of that range; both maps pass under that standard too.
...
**Answer:** The 70/30 blend was a modeling assumption used when precise vote attribution was not possible: 70% of a hybrid district's vote estimate came from applying the result to one zone, 30% from another. It was a placeholder for the measured value that would come from a complete geographic overlay of every polling station with the new district shapes. **Phase 4C — the canonical VA-polygon attribution step — has now been executed** (script: `analysis/scripts/phase4c_canonical_attribution.py`; output: `data/outputs/phase4c_canonical_results.json`). It produces, for canonical EA shapefiles + 2023 vote data: minority EG +4.02%, majority EG +0.10% (asymmetry +3.92pp); minority seats@50/50 = 0.5169 (UCP fraction; equivalently 0.4831 NDP fraction per Phase 4C's own convention note). These are the values the monograph and public report now cite as canonical. The earlier "uncertainty from modeling choice" framing was correct for the pre-canonical era and is now superseded: the Phase 4C measurement *converged* to a specific value rather than reducing the blend's range. The Monte Carlo interval reported in §5.2.2 is a separate (correctly disclosed) source of uncertainty — the ensemble's distribution under different neutral-map draws — not the blend-substrate uncertainty. (Earlier draft of this entry contradicted the §1706 entry on the same page; this revision aligns them.)
...
**Answer:** *This entry has been superseded by canonical execution of Phase 4C (2026-06-10).* The "+0.0000 pp" shift cited in earlier drafts referred to the v0_9-era **MAUP test** (centroid-in-polygon vs area-weighted attribution on the v0_9 topological shapefiles) — not to Phase 4C VA-polygon attribution itself. The MAUP test showed the v0_9 substrate's snap-to-VA-boundaries construction made attribution-method choice immaterial on that substrate. Phase 4C is now the canonical-substrate VA-polygon attribution step and *has* been executed (`analysis/scripts/phase4c_canonical_attribution.py`); see the preceding entry on this page for the canonical Phase 4C numbers (minority EG +4.02%, seats@50/50 0.5169 UCP fraction). The earlier "intentionally bypassed" framing was an artifact of pre-canonical sequencing and is no longer accurate.
