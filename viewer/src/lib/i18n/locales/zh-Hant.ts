...
export default {
	...
	editorial_intro: {
		...
		p3: '那是標準程序。這一次,標準程序產生了不尋常的結果。委員會的五名成員就地圖應是什麼樣子分裂成了3比2,與其達成一項建議,他們提出了兩份:一份<em>多數派提議</em>(由三位委員支持)和一份<em>少數派提議</em>(由兩位支持)。兩份都擺在桌上——但立法機構已將兩份都擱置。根據政府第37號議案(2026年4月21日),一個由Brandon Lunty主持的獨立MLA委員會——由省長為此特定決定任命——現監督一個由四人組成的獨立諮詢小組,負責制定一份全新的91席選區地圖。立法機構必須在2026年11月之前批准該委員會提交的方案。',
