...
**Pipeline:** `analysis/assignment_va_attribution_maup.py`
**Companion (not modified):** `analysis/scripts/assignment_va_attribution.py`, `analysis/scripts/phase_4bcdef_execution.py`
...
The existing Phase 4C pipeline (`assignment_va_attribution.py` + the corresponding VA-polygon branch of `phase_4bcdef_execution.py`) assigns each of 4,765 Voting Area polygons **wholesale to a single 2026 ED** based on the VA polygon's `representative_point()` (its interior centroid). When a VA polygon straddles a DPG ED boundary — i.e., when part of it lies inside ED_A and part inside ED_B — 100 % of the VA's 2023 votes are attributed to whichever ED the centroid happens to fall in. If a VA is split 60/40 across two EDs, the ED on the 60 % side gets all of it and the other ED gets nothing.
...
(Centroid-pipeline seat totals for the same maps are reported in `data/pipeline_summary.json`; the MAUP totals differ modestly because of margin shifts at tipping EDs.)
...
- `C:\Users\email\Documents\Claude\Projects\Electoral Boundary Analysis\alberta_audit\analysis\assignment_va_attribution_maup.py`
- `C:\Users\email\Documents\Claude\Projects\Electoral Boundary Analysis\alberta_audit\data\v0_1_votes_2023_majority_maup.csv`
- `C:\Users\email\Documents\Claude\Projects\Electoral Boundary Analysis\alberta_audit\data\v0_1_votes_2023_minority_maup.csv`
- `C:\Users\email\Documents\Claude\Projects\Electoral Boundary Analysis\alberta_audit\analysis\assignment_va_to_2026_assignments_maup.csv` (15,723 rows)
...
- `C:\Users\email\Documents\Claude\Projects\Electoral Boundary Analysis\alberta_audit\analysis\maup_area_weighted_analysis.md` (this file)
...
PYTHONIOENCODING=utf-8 python analysis/assignment_va_attribution_maup.py
# smoke test (50 VAs):
PYTHONIOENCODING=utf-8 python analysis/assignment_va_attribution_maup.py --smoke 50
```
