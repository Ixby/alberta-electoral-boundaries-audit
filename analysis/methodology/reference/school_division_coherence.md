...
**Scope.** Every minority hybrid (any ED with `-hybrid` or `-merged` in the `region_type` column of `data/minority_2026_populations.csv`). This yields 21 EDs across Calgary, Edmonton, Red Deer, Lethbridge, and St. Albert regions.
...
**What we are not doing.** We are not re-running the population math (closed — see `justification_tests_findings.md`). We are not re-running the commute tests (done for Cochrane — see `cochrane_journey_to_work.md`). We are testing a narrow claim: does the school-division geography *permit* the minority's community-of-interest framing?
