...
def load_data():
    ...
    va_pts["party"] = va_pts.apply(
        lambda r: "NDP" if r["va_ndp"] > r["va_ucp"] else "UCP", axis=1
    )

    va_pts_proj = gpd.GeoDataFrame(
        va_pts.drop(columns="geometry"),
        geometry=va.geometry.centroid.to_crs(3401),
        crs=3401,
    )
...
def get_events(eds_2019_proj, eds_2026_proj, va_pts_proj, map_label):
    crosswalk = gpd.overlay(
        eds_2019_proj.rename(columns={"name": "name_2019"}),
        eds_2026_proj.rename(columns={"name": "name_2026"}),
        how="intersection",
    )
    ...
    cw_area = crosswalk.merge(
        eds_2026_proj[["name", "area_2026"]].rename(columns={"name": "name_2026"}),
        on="name_2026",
    )
...
if __name__ == "__main__":
    ...
    for e in unique_events:
        ...
        if etype in ["Cracking", "Draining"]:
            target_poly = eds_2019[eds_2019["name"] == e["target_2019"]].geometry.iloc[
                0
            ]
        else:
