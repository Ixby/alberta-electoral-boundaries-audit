...
3. **Novel combined tests in `v0_1_test_selection_rationale.md` §6** (the five this document expands). These are proposed but not yet executed. Each is worth executing or dropping explicitly; leaving them as "planned but never run" creates a soft-proliferation trap.
...
**Implementation.** `geopandas.sjoin(predicate='touches')` for adjacency; per-ED vote totals from 2023 Statement of Vote (already in the repo at `data/v0_1_alberta_2023_results.csv`); threshold loop + chain-count aggregation. ~200 lines of Python. Runtime negligible. **Effort: 2 days** including writeup + sensitivity table.
...
**Implementation.** Reuse `v0_2_packing_cracking_analysis.py` machinery; loop over 11 scenarios. **Effort: 3 days** (most code exists).
...
- **Predictive / criterion validity.** The November 2026 committee's 91-seat map is the out-of-sample test the audit explicitly pre-registers against (Track C in `migration.md` / private `v0_1_live_tasks.md`). Until that map is tabled, criterion validity is pending.
...
| 7 | Retract unscoped-and-unexecuted tests from §6 of `v0_1_test_selection_rationale.md` | 0.5 day | Remove soft-proliferation risk |
