...
Status: v0.1 — revisable. This is a companion analysis to §C4 in `v0_1_section_C_geographic_coherence.md`. It supplies verifiable per-CSD counts for the community-of-interest claim that the §C4 narrative rests on.
...
- `data/v0_1_majority_hybrid_crosswalk.csv` — 19 rows from Appendix C, covering the majority commission's hybrid and renaming changes.
- `data/v0_1_minority_hybrid_crosswalk.csv` — 103 rows, heuristically assembled from Appendix E with a `jaccard` match score; includes 14 rows where a 2019 ED has no clean 2026 successor and is recorded as `(MERGED/ABSORBED)`.
...
- Script: `analysis/v0_1_csd_community_splits.py`
- Per-CSD detail: `data/v0_1_csd_splits_summary.csv`
- Section under revision: `analysis/v0_1_section_C_geographic_coherence.md` §C4
- Academic report section potentially affected: `report_academic.md` §3 (community of interest) — flag, do not edit.

## Dependencies header

Backward: `alberta_2021_csds.gpkg`, `alberta_2021_csd_populations.csv`, `alberta_2019_eds/*.shp`, `v0_1_majority_hybrid_crosswalk.csv`, `v0_1_minority_hybrid_crosswalk.csv`.
