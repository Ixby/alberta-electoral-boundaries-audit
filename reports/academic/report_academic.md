...
**Policy contribution: Elections Alberta should capture home-VA identifiers for advance voters.** In the 2023 provincial election, 395,472 NDP and UCP votes cast advance, mobile, or special carry no Voting Area identifier in EA's published data (§3.2 item 5). Because advance voters are drawn from the same geographic communities as election-day voters, their spatial distribution is not meaningfully different from the election-day pattern — but it cannot currently be verified, measured at the VA level, or used in geographic partisan-bias analysis. Alberta voters are already identified by electoral division at the advance poll (to verify eligibility and prevent double-voting); recording the voter's home VA at that point requires no additional voter-facing interaction and imposes minimal administrative burden. The change would bring Alberta's result-reporting to the geographic granularity that systematic electoral analysis requires, allow complete-coverage VA-level vote attribution in future audits, and provide a more accurate basis for community-of-interest assessments that depend on knowing where voters live rather than where they voted. The audit offers this as a policy contribution, not a finding.
