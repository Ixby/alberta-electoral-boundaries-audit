...
- `analysis/v0_1_submission_ocr.py` — OCR run script.
- `analysis/v0_1_submission_ocr_analyze.py` — standalone analyzer that reads the OCR page files and rebuilds submission text + runs the keyword search (safe to re-run if more OCR pages arrive).
...
python analysis/v0_1_submission_ocr.py          # OCR the 73 planned pages (~45-60 min CPU)
python analysis/v0_1_submission_ocr_analyze.py  # Stitch + keyword search (~10 s)
```
