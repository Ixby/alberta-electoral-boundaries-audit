...
backward_dependencies:
  ...
  - analysis/v0_1_chair_recommendation_5_analysis.md — B5 background
  - analysis/submission_search_findings.md — B4 text-layer evidence
  - analysis/v0_1_submission_ocr_log.md — B4 OCR recovery evidence
  - analysis/v0_1_claim_significance_analysis.md — B4 tiered refutation structure
  - v1_2_gerrymander_audit_prompt.md — RT1–RT6 stress-test gates referenced in B1
  - analysis/v0_1_majority_symmetry_counter_test.py — new B3 counter-test script
...
Two of these attacks (B1 methodology, B3 test-symmetry) carry forward work already begun in the A-fortification. One (B4 public-support) has new evidence from Track D OCR that shifts the framing slightly. Three (B2 declination, B5 chair R5, B6 AI authorship) are first-pass fortifications. B3 required a new script (`analysis/v0_1_majority_symmetry_counter_test.py`) to execute a symmetry-of-test-selection counter-test; its results are reported below.
...
**Defence 2 — The B3 counter-test was executed in `analysis/v0_1_majority_symmetry_counter_test.py` (Track Q, 2026-04-22).** Results in `data/v0_1_majority_symmetry_counter_test.csv`. Two tests:
...
The paper's §5.2 should retain the existing three-layer finding (form, conditions, intent, per `analysis/v0_1_chair_recommendation_5_analysis.md`), and add an explicit defence of why R5's form-match does not cure the other two layers:
...
**F13 — New §3.12 (Symmetry-of-test-selection audit).** Insert the B3(iv) counter-test paragraph reporting the Edmonton and city-wide counter-test results. Reference `analysis/v0_1_majority_symmetry_counter_test.py` and `data/v0_1_majority_symmetry_counter_test.csv`. Purpose: establish test-selection symmetry as an auditable discipline, contribute the Lethbridge and Red Deer findings.
...
**F17 — `analysis/v0_1_majority_symmetry_counter_test.py` — authored and runnable.** Produces `data/v0_1_majority_symmetry_counter_test.csv`. Purpose: execute the B3 counter-test; contribute two new cracking-candidate findings (Lethbridge, Red Deer 4-way splits). Parent session commits.
...
- Chair R5 close reading: `analysis/v0_1_chair_recommendation_5_analysis.md`
- Public-support evidence: `analysis/submission_search_findings.md`, `analysis/v0_1_submission_ocr_log.md`, `analysis/v0_1_claim_significance_analysis.md`
- Pre-registration prompt: `v1_2_gerrymander_audit_prompt.md` (RT1–RT6 thresholds verbatim)
- Symmetry counter-test: `analysis/v0_1_majority_symmetry_counter_test.py` (this track) and `data/v0_1_majority_symmetry_counter_test.csv`
- Monte Carlo CI (referenced in B1 defence): `analysis/v0_3_monte_carlo_ci.py`
- Declination computation (referenced in B2 defence): `analysis/v0_2_packing_cracking_analysis.py`
