...
**Substrate.** `ea_majority_2026_eds.gpkg` and `ea_minority_2026_eds.gpkg` (official Elections Alberta files, EPSG:3400, 89 EDs each). Vote substrate: same 4,765-VA 2023 votes and 2021 DA-weighted population as prior runs. Base seed: `get_canonical_seed("lunty-bootstrap") = 1432864451` (drand round 5,800,000, pre-registered before run). Chain: 4 chains × 62,500 steps = 250,000 total plans (2026-05-07–2026-05-08). Combined n_eff: 379–452 (partisan metrics EG/MM/Decl/seats@50/50); 838–1,613 (population MAD and Reock proxy). Integrated autocorrelation time τ: 552–659 (partisan metrics), 155–299 (population/compactness). Partisan ESS is sufficient for reliable percentile estimation through p99.9 but below publication-grade for individual tail claims; see ESS caveat below. Convergence: rho_lag1 = 0.968–0.991. **Chain note (initial 50k run, 2026-05-06, superseded).** A first canonical run used only 2 chains — chains 2 and 3 encountered geometrically pathological starting partitions (bipartition time 17–23 s/step vs. 15 ms/step for chains 0 and 1); that run produced n_eff 115–343 and is superseded. The 250k update reinitialised all 4 chains from the canonical checkpoint; the pathological initialisation did not recur. All results below derive from the 4-chain 250k run. **1,000,000-step canonical run in progress (launched 2026-05-11).** A 1M-step run (4 chains × 250,000 steps, same seed and canonical shapefiles) is running; expected completion ~2026-05-12. Partisan-metric ESS will scale approximately 4× to ~1,500–1,800, achieving publication-grade for individual percentile claims. Percentile placements are not expected to shift materially from the 250k values.
...
| --- | --- | --- | --- | --- |
| 2026 minority | seats@50/50 | 0.5169 | >99.9 empirical; ESS-adjusted lower bound p89.72 — **individual flag retracted** (n_eff≈452 puts ESS-corrected threshold below p95; see ESS note) | unchanged |
...
**Partisan metrics — 250k canonical update.** The 250k-step run (same seed, official EA geometry) refines the earlier 50k percentile estimates. Two of four partisan metrics survive individual flagging on the minority map: mean-median at p99.985 (UCP-favoured tail) and declination at p1.03 (NDP-favoured tail). Efficiency gap drops from p95.9 to p94.2 — just below the p95 threshold; the EG outlier flag is withdrawn. Seats@50/50 reads p>99.9 empirically (0 of 250,000 neutral plans reach the minority value) but the ESS-adjusted lower bound is p89.72 with n_eff≈452, which falls below the pre-registered p95 threshold; the individual flag is retracted (the joint Mahalanobis result is unaffected). The minority's surviving pattern: two flags in structurally opposite directions — MM in the UCP-favoured tail, declination in the NDP-favoured tail — which is the asymmetric-packing signature discussed in §5.2.4.
...
*Methodological Caveat on Autocorrelation:* The Mahalanobis degrees of freedom uses 4 (number of metrics) but does not formally discount for autocorrelation within the MCMC chain. Effective sample sizes from the 250k canonical run: EG n_eff=421, MM n_eff=379, Decl n_eff=443, seats@50/50 n_eff=452 (integrated autocorrelation times τ=552–659). The $p = 1.60 \times 10^{-7}$ value is mathematically extreme enough to survive an ESS correction: even at the minimum n_eff=379, the ESS-adjusted joint-tail probability from a chi-squared₄ distribution remains below 10⁻⁵. A 1M canonical run in progress (2026-05-11) will raise ESS to ~1,500–1,800, eliminating this caveat for the 1M publication run.
