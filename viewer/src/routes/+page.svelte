  ...
  onMount(() => {
    // Pre-fetch all three hover datasets so cross-map comparison is available
      ...
      (function () {
        ...
        // ── Map selector ──────────────────────────────────────────────────────────
        const _mapSvgUrls = {
          minority: `${base}/images/cover_art_minority_hires.svg`,
          majority: `${base}/images/cover_art_majority_hires.svg`,
          '2019':   `${base}/images/cover_art_2019_hires.svg`,
        };
        const _mapJsonUrls = {
        ...
        stage.addEventListener('pointerup', e => {
          ...
          if (!_dragMoved && e.pointerType === 'touch' && mode === 'viewbox') {
...
<header>
  <div class="header-inner">
    <div class="header-text">
      ...
      <p class="cover-note">The map at right is the minority commission's 2026 proposal &mdash; the one that sits furthest from the neutral ensemble in statistical testing. Each riding is coloured by its 2023 vote: blue for UCP-leaning districts, orange for NDP-leaning. Colour intensity reflects where people actually live: bright and saturated in dense urban cores, pale and faded in sparse rural areas.</p>
    </div>
...
<div class="container">
  ...
  <section id="section-7">
    ...
    <div class="callout">
      <p><strong>THE PLAIN READING.</strong> Two commission proposals were drawn in the same room, by five commissioners working from the same rules and the same data. Neither is law. The majority proposal is the kind of map a neutral procedure routinely produces: clean on every one of the audit's pre-registered structural tests, comfortably inside the simulated partisan-fairness distribution. The minority proposal is something else. It crosses four of five structural-irregularity tests — chair-flagged lasso corridors (3 confirmed of 7 configurations criticized), the four-way Airdrie split, the national-park extension, and the 48%-wider population spread — and on 1,010,000 computer-simulated neutral maps drawn from the official Elections Alberta shapefiles, its <code>seats@50/50</code> value is reached by fewer than 100 neutral maps. (Municipal anchoring is the fifth pre-registered test; it is retracted — both proposals fall within the 70–85% Canadian norm on official shapefiles. See the correction note below.) All four partisan-fairness measures simultaneously place it in the statistical tail; the joint probability of that combination under a neutral drawing process is roughly one in 15 million. The audit tested whether the unusual geometry is the specific mechanism that produces the seat advantage; that claim did not survive the falsification. What does survive: same room, same rules, same data, two very different proposals, and only one of them has the structural pattern that researchers flag for further inquiry. Whether the cause was deliberate engineering, unlucky drafting, or both is a judgement call the audit puts in the reader's hands.</p>
      <p>We measured the effects. We can't read minds.</p>
    </div>
