...
Tier-1 item T1.1 in `TODO_REMEDIATION.md` is partially closed by this finding. The remaining open question is whether a 1M-plan regional-swing ensemble (which requires archiving the per-VA assignments of the canonical 1M run, an LFS data debt) would extend the tail enough to demote the minority below p99. The directionally-honest expectation: no, it would not — the gap between the canonical minority's regional s50 (0.4607) and the 10k regional-swing maximum (0.4598) is wider than the ~0.01 tail extension a 100× sample increase typically produces.
