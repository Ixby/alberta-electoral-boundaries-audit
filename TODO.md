...
- **DRAIN-CANONICAL-01 — Re-run §5.3.5 neighbour-drain adjacency test on canonical Elections Alberta shapefiles.** **STATUS: PENDING.** The pre-registered §5.3.5 PASS (minority 2 coupled chain signals vs majority 6 vs 2019 5; ratio 0.33×) was computed on the v0_2 DPG topology-cleaned substrate. A v0_8 full-coverage sensitivity check returned minority=4 / majority=3 (1.33×, direction reversed, still passes the 1.5× threshold). Neither v0_2 nor v0_8 is canonical; the official Elections Alberta shapefiles (`ea_majority_2026_eds.gpkg`, `ea_minority_2026_eds.gpkg`, received 2026-05-06 from Raymond Mok at EA) have not been used as the drain-test input substrate. To execute: run `analysis/scripts/v0_1_neighbour_drain_adjacency.py` (or `analysis/scripts/neighbour_drain_adjacency.py`) with inputs pointed at the canonical EA gpkgs and the 2023 election-day VA vote substrate; write canonical output to `data/outputs/neighbour_drain_canonical.csv` / `.json`; update `findings/neighbour_drain_analysis.md` to lead with the canonical numbers; update README §"What the audit finds" §5.3.5 and the structural-cost table accordingly. Estimated clock: 1–2 hours of script work + writeup. Low compute (the adjacency test is fast). **This is not a substantive open methodology question — it's an overdue housekeeping recompute. Run when convenient; until then, the README is honest that the substrate is non-canonical.**
...
Stage 3 superseded by official shapefiles. Stages 3–7 complete for canonical spatial join.
...
| **CLR-05** | `findings/joint_outlier_score.json` | Anchoring values 14.5% / 71.0% in `structural_pending` | DPG-era values, retracted finding. | **RESOLVED 2026-05-12** — updated to 72% / 80% with RETRACTED status label. |
| **CLR-06** | `findings/joint_outlier_score.json` | `caveats: "Ensemble is 100k plans"` | D² and Fisher p from 100k covariance. | **RESOLVED 2026-05-12** — recomputed from 1M covariance: minority D²=32.67 (p=1.40e-06), majority D²=7.85 (p=0.097), enacted D²=12.75 (p=0.013); Fisher p=6.87e-08 (T=39.03). |
