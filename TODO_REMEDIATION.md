...
**Status: 🟡 SCRIPT VERIFIED PRESENT 2026-06-13; canonical rerun pending (same compute blocker as T1.5).**

Script-verification blocker cleared: both `analysis/scripts/targeted_gerrymander_burst.py` (UCP objective) and `analysis/scripts/targeted_gerrymander_burst_ndp.py` (NDP mirror) exist and **have already been run on the v0_9 substrate** — results are in the report H9 row (UCP-targeted reaches 52.87 % s50; NDP-targeted 37.93 %; both beyond the neutral 39.08–50.57 % envelope) and `findings/burst_symmetry_analysis.md`. The only remaining work is the **canonical-substrate rerun**, which needs gerrychain + canonical EA shapefiles + a dedicated compute window — i.e. the same blocker as T1.5, not a script gap. The earlier 52.87 % s50 maximum is v0_9 and should be replaced with a canonical value when T1.5/T1.5b run together.
...
**Status: 🟢 8 of 9 RESOLVED 2026-06-13 (web/CanLII-verified + deterministic fixes); only #5 (exact SCR page pinpoints) remains counsel/SCR-report-dependent. Full verification logged in `analysis/methodology/reference/citation_verification.md` Step 12.**
...
3. ✅ **EBCA census basis** → **RESOLVED 2026-06-13.** Section reference corrected from **s. 14(1)(a)** (the boundary-drawing *factors* section) to **s. 12** (the population provisions; §12(3) census basis, §12(5) supplementation) — confirmed against RSA 2000 c E-3, which **defines "decennial census" at s. 12(2)** ("the most recent decennial census of population referred to in s. 19(3) of the *Statistics Act* (Canada)"). So "decennial census" is the Act's own defined term and is **correct** (the referee's "the scheme is quinquennial" note is the imprecise part — the Act specifically invokes the decennial census). Term retained, section fixed. Narrow residual (counsel, non-blocking): whether the 2026 quinquennial census qualifies under the Act's "decennial census" definition (which points to 2021, next decennial 2031); the report's practical conclusion — use 2021 data — holds regardless.
