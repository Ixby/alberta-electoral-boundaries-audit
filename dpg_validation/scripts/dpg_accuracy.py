...
def run_map(label, dpg_path, off_path, enacted_2019):
    ...
    if enacted_2019 is not None:
        e_name_col = next((c for c in enacted_2019.columns if "name" in c.lower()), None)
        if e_name_col:
            enacted_idx = {normalize_name(r[e_name_col]): i for i, r in enacted_2019.iterrows()}
    ...
    for _, dpg_row in dpg.iterrows():
        ...
        rows.append({
            ...
            "iou_vs_2019_pct":     round(iou_2019, 2) if not np.isnan(iou_2019) else None,
    ...
    worst_ed   = reconstructed.loc[reconstructed["iou_vs_official_pct"].idxmin(), "dpg_name"] if n_recon > 0 else "—"
    ...
    print(f"  2019-inherited (excl): {n_inh}  (IoU >= {INHERITED_THRESHOLD}% vs enacted)")
    ...
    if under95 > 0:
        ...
        sub = reconstructed[reconstructed["iou_vs_official_pct"] < 95][cols].sort_values("iou_vs_official_pct")
    ...
    return df, {
        ...
        "mean_iou_reconstructed": round(mean_recon, 2) if not np.isnan(mean_recon) else None,
        "min_iou_reconstructed": round(min_recon, 2) if not np.isnan(min_recon) else None,
        "worst_ed": worst_ed,
...
def write_summary(summaries, out_path):
    ...
    lines.append(f"*Metric: Jaccard IoU (intersection / union × 100). "
                 f"2019-inherited EDs excluded from reconstructed aggregate.*")
    lines.append("")
    for s in summaries:
        lines.append(f"## {s['map'].title()}")
        lines.append(f"- EDs total: {s['n_total']}  |  2019-inherited (excluded): {s['n_inherited']}  |  Reconstructed: {s['n_reconstructed']}")
        lines.append(f"- **Mean IoU (reconstructed): {s['mean_iou_reconstructed']}%**")
        lines.append(f"- Min IoU: {s['min_iou_reconstructed']}%  ({s['worst_ed']})")
        lines.append(f"- Below 99%: {s['n_below_99pct']}  |  Below 95%: {s['n_below_95pct']}  |  Below 90%: {s['n_below_90pct']}")
        lines.append("")
    out_path.write_text("\n".join(lines))
    print(f"\nSummary -> {out_path}")
...
def main():
    ...
    if enacted_2019 is None:
        print("WARNING: 2019 enacted shapefile not found — inheritance detection disabled")
