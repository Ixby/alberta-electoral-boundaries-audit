...
export default {
	...
	top_callouts: {
		...
		tldr_p2: 'This audit tested both commission maps the same way, using 1,010,000 computer-drawn neutral maps built from the official Elections Alberta shapefiles as a reference point. The majority proposal sits within the neutral range on every pre-registered test. The minority proposal crosses four of five structural tests, and its partisan-fairness seat split at a 50/50 vote is reached by roughly 66 of those 1,010,000 neutral maps \u2014 a dependence-robust joint upper bound of roughly 1 in 350,000 under that reference distribution. (The earlier framing of \u201c1 in 15 million\u201d combined two channels that share underlying data and overstated the joint significance; the figure above is the corrected, defensible bound.) The audit\u2019s efficiency-gap metric for the minority sits at the 94th percentile \u2014 <em>near, but below</em>, the audit\u2019s own 95th-percentile threshold.',
	...
	boundary: {
		...
		can_2:
			'Separately, when election data is brought in, the minority map sits at the 99.99th percentile of 1.01 million algorithmically-drawn comparison maps held to the same statutory rules. Roughly 66 of those 1.01 million neutral maps reach the same seats@50/50 value. The dependence-robust joint upper bound across the two analytical channels is approximately 1 in 350,000 under that reference distribution — a corrected figure that replaces an earlier "1 in 14.5 million" Fisher-method estimate that assumed channel independence those channels do not have. Two analytical instruments — geometric and statistical — reach the same conclusion in the same direction; the joint bound is the dependence-aware summary of that convergence, not four independent tests reading in agreement.',
		can_3:
		...
		cant_4:
			"The audit does <strong>not</strong> claim the earlier 1-in-14.5-million Fisher estimate (now corrected to the 1-in-350,000 dependence-robust bound) is the probability that a gerrymander occurred. That number is the tail probability of the minority's geometry under a neutral algorithmic null. It is evidence the neutral null is implausible. It is not a posterior probability of partisan intent — that question requires evidence the geometry cannot supply.",
		cant_5:
	...
	body: {
		...
		litmus: {
			...
			fig_alt: 'Histogram showing the distribution of efficiency gaps across 1,010,000 neutral Alberta maps. Most maps cluster near zero. The minority commission map (purple line) sits at the 94th percentile (+3.96%), in the shaded right tail. The majority map (teal line) sits at +0.04%, well within the normal range.',
			fig_caption: 'Distribution of <button class="vocab-term" data-def="a measure of how lopsidedly votes are converted into seats — positive values favour the UCP, negative values favour the NDP" aria-expanded="false">efficiency gaps</button> across 1,010,000 neutral Alberta maps drawn from the same geography. Most neutral maps cluster near zero; the shaded right tail marks the top 10%. The minority proposal&rsquo;s +3.96% sits at the 94th <button class="vocab-term" data-def="the percentage of maps that scored lower — p94 means 94 out of 100 neutral maps were less partisan than this" aria-expanded="false">percentile</button> — a region fewer than 6 in 100 neutral maps ever reach. The majority proposal&rsquo;s +0.04% is indistinguishable from what a neutral process typically produces.',
