...
def run_crosswalk():
    ...
    eds_min = eds_min[[name_col_2026, 'geometry']].rename(columns={name_col_2026: 'name_2026'})
    ...
    slice_votes = joined.groupby('slice_id').agg(
        ucp_votes=('va_ucp', 'sum'),
        ndp_votes=('va_ndp', 'sum')
    ).reset_index()
    ...
    for district in cracked_candidates:
        slices = crosswalk[crosswalk['name_2019'] == district].sort_values('total_votes', ascending=False)
        ...
        for _, row in slices.iterrows():
            ...
            print(f"  -> {row['total_votes']:,.0f} votes sent to [2026] {row['name_2026']} (Leans {winner})")
    ...
    districts_2026['ucp_share'] = districts_2026['ucp_votes'] / districts_2026['total_votes']
    districts_2026['ndp_share'] = districts_2026['ndp_votes'] / districts_2026['total_votes']
    
    # Filter for packed districts assembled from multiple (>=2) meaningful sources
    packed = districts_2026[
        (districts_2026['num_2019_sources'] >= 2) & 
        ((districts_2026['ucp_share'] > 0.62) | (districts_2026['ndp_share'] > 0.62)) &
        (districts_2026['total_votes'] > 15000)
    ].sort_values('num_2019_sources', ascending=False).head(5)
    ...
    for _, row in packed.iterrows():
        ...
        print(f"  -> It was assembled by harvesting {row['num_2019_sources']} different 2019 districts:")
        
        pieces = crosswalk[crosswalk['name_2026'] == row['name_2026']].sort_values('total_votes', ascending=False)
        for _, piece in pieces.iterrows():
            if piece['total_votes'] > 1000: # Only show meaningful chunks
                print(f"     - Pulled {piece['total_votes']:,.0f} votes from [2019] {piece['name_2019']}")
