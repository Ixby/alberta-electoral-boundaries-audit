...
def extract_landing_series(html: str) -> Dict:
    ...
    if len(parsed) < 7:
        raise RuntimeError(
            f"landing: expected >=7 full-length blocks, got {len(parsed)}"
        )
...
def wayback_closest(url: str, yyyymmdd: str) -> Optional[str]:
    ...
    """
    api = (
        f"http://archive.org/wayback/available?url={urllib.request.quote(url)}"
...
def wayback_cdx_last_before(
    url: str, yyyymmdd_from: str, yyyymmdd_to: str
) -> Optional[str]:
...
def wayback_pull_aggregate(date_iso: str) -> Optional[Dict]:
    ...
    if html.startswith("__FETCH_ERROR__"):
        return {"wayback_url": wb_url, "actual_date": actual_date, "error": html}
    try:
    ...
    return {
        ...
        "ucp_share": (
            round(s.get("UCP_share", [float("nan")])[-1], 3)
            if "UCP_share" in s
            else None
        ),
        "ndp_share": (
            round(s.get("NDP_share", [float("nan")])[-1], 3)
            if "NDP_share" in s
            else None
        ),
        "ucp_seats": (
            round(s.get("UCP_seats", [float("nan")])[-1], 1)
            if "UCP_seats" in s
            else None
        ),
        "ndp_seats": (
            round(s.get("NDP_seats", [float("nan")])[-1], 1)
            if "NDP_seats" in s
            else None
        ),
        "ucp_maj_prob": (
            round(s.get("UCPMAJ", [float("nan")])[-1], 2) if "UCPMAJ" in s else None
        ),
        "ndp_maj_prob": (
            round(s.get("NDPMAJ", [float("nan")])[-1], 2) if "NDPMAJ" in s else None
        ),
    }


def wayback_pull_per_riding_window(
    ridings_index: List[Dict], window_from: str, window_to: str, max_ridings: int = 87
) -> List[Dict]:
    ...
    for i, r in enumerate(ridings_index[:max_ridings]):
        ...
        if os.path.exists(cache) and os.path.getsize(cache) > 1000:
            ...
            if (i + 1) % 15 == 0:
                print(
                    f"  wayback (cache) {i+1}/{len(ridings_index[:max_ridings])} "
                    f"(hit={hit}, miss={miss})",
                    file=sys.stderr,
                )
            continue
        ...
        if (i + 1) % 15 == 0:
            print(
                f"  wayback window {window_from}-{window_to}: "
...
def _extract_projection_date(html: str) -> str:
    ...
    if not m:
        m = re.search(
            r"338Canada Popular vote projection\s*\|\s*([A-Za-z]+)\s+(\d+),?\s+(\d{4})",
            html,
        )
    if m:
...
def _parse_riding_html(html: str) -> Dict[str, Dict[str, float]]:
    ...
    entries = re.findall(r'<text[^>]*fill="(#[0-9A-Fa-f]{6})"[^>]*>([^<]+)</text>', svg)
...
def main():
    ...
    if pre_idx is None:
        ...
        print(
            f"  338 vote share (two-party):  UCP {u_tp_338:.2f}%  NDP {n_tp_338:.2f}%"
        )
        print(
            f"  2023 actual (two-party):     UCP {actual['ucp_actual_tp']:.2f}%  "
            f"NDP {actual['ndp_actual_tp']:.2f}%"
        )
        print(
            f"  error (338 - actual):        UCP {u_tp_338 - actual['ucp_actual_tp']:+.2f} pp  "
            f"NDP {n_tp_338 - actual['ndp_actual_tp']:+.2f} pp"
        )
        if ucp_seats_pre is not None:
            print(
                f"  338 seat projection:         UCP {ucp_seats_pre:.1f}  NDP {ndp_seats_pre:.1f}"
            )
            print(
                f"  2023 actual seats:           UCP {seats_actual['ucp_seats']}  "
    ...
    with open(
        os.path.join(HIST_DIR, "per_riding_wayback.json"), "w", encoding="utf-8"
    ) as f:
    ...
    if pre_hits:
        ...
        if v.get("n", 0) > 0:
            ...
            print(
                f"  mean bias:     {v['bias_338_minus_actual']:+.2f} pp (338 - actual)"
            )
            for reg, vs in v["regional"].items():
                print(
                    f"  region={reg}: n={vs['n']} mae={vs['mae']:.2f} "
                    f"bias={vs['bias']:+.2f}"
                )
            print(f"  top-5 errors:")
            for p in v["top_errors"][:5]:
                print(
                    f"    {p['ed']:45s} 338={p['ucp_338_tp']:6.2f}  "
                    f"actual={p['ucp_actual_tp']:6.2f}  err={p['error']:+6.2f}"
                )
            with open(
                os.path.join(HIST_DIR, "pre2023_per_riding_validation.json"),
                "w",
                encoding="utf-8",
            ) as f:
    ...
    rural_ucp_cur = statistics.mean(
        current_t338[n]["ucp_share"] for n in rural_names if n in current_t338
    )
    rural_ndp_cur = statistics.mean(
        current_t338[n]["ndp_share"] for n in rural_names if n in current_t338
    )
    print(
        f"\n  current (2026-04-12) rural baseline: "
        f"UCP {rural_ucp_cur:.2f}% NDP {rural_ndp_cur:.2f}%"
    )
    maj_cur = reallocate_snapshot(current_t338, MAJ, pops, rural_ucp_cur, rural_ndp_cur)
    min_cur = reallocate_snapshot(current_t338, MIN, pops, rural_ucp_cur, rural_ndp_cur)
    ...
    print(
        f"  current: majority UCP/NDP = {ucp_maj_cur}/{ndp_maj_cur}; "
        f"minority = {ucp_min_cur}/{ndp_min_cur}"
    )
    print(
        f"  current min-vs-maj asymmetry: "
    ...
    stability_table = [
        {
            "snapshot_date": "2026-04-12 (current, full 87)",
            "maj_ucp": ucp_maj_cur,
            "maj_ndp": ndp_maj_cur,
            "min_ucp": ucp_min_cur,
            "min_ndp": ndp_min_cur,
    ...
    if n_obs >= 80:
        pre23_t338 = {
            h["district"]: {"ucp_share": h["ucp_share"], "ndp_share": h["ndp_share"]}
            for h in pre23_hits
        }
        ...
        rural_ucp_pre = statistics.mean(
            pre23_t338[n]["ucp_share"] for n in rural_names if n in pre23_t338
        )
        rural_ndp_pre = statistics.mean(
            pre23_t338[n]["ndp_share"] for n in rural_names if n in pre23_t338
        )
        maj_pre = reallocate_snapshot(
            pre23_t338, MAJ, pops, rural_ucp_pre, rural_ndp_pre
        )
        min_pre = reallocate_snapshot(
            pre23_t338, MIN, pops, rural_ucp_pre, rural_ndp_pre
        )
    ...
    for i, d in enumerate(dates):
        ...
        rural_u = statistics.mean(
            sim_t338[n]["ucp_share"] for n in rural_names if n in sim_t338
        )
        rural_n = statistics.mean(
            sim_t338[n]["ndp_share"] for n in rural_names if n in sim_t338
        )
    ...
    for lo, hi in bands:
        rows = [r for r in sweep_rows if lo <= r["ucp_share_two_party"] < hi]
        if not rows:
            continue
