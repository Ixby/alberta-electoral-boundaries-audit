...
- "Your region" addresses a real obstacle the earlier draft of this section missed: a rural reader who already feels under-represented by the current system will read a "fair-maps" argument as urban-coded — "they're trying to strip the cities of the structural advantages they finally have." If the section doesn't acknowledge that perception directly, it loses the rural reader inside the first sentence. The rewrite opens by naming the perception ("political conversations about boundaries always seem to centre the cities — that's a fair complaint") and then makes the rural case from the rural side rather than the urban side.
...
- "Your province" used to lead with "the lines shape who governs" — i.e., the partisan/legislature frame the section's opening paragraph names as *the parties' frame*. The rewritten rung instead presents the legislature as the **sum** of every district's answers to the three questions; the partisan-majority question is "downstream of that," and the supermajority question is "downstream of *that*." Same observations (structural extremity at 50/50, supermajority threshold at 58 seats, honest caveat about the untested vote-share range), but staged so the lived-reality questions are the cause and the legislature shape is the consequence. That ordering matches what the audit actually measures (district-level geometry) and what it has not (vote-share-conditional seat behaviour).
- The supermajority point still appears, because dropping it entirely would create a gap with Q3 of the verdict. But it's no longer the punchline of the rung — it's named as one of several downstream consequences and explicitly flagged as a question the audit has not yet answered at the relevant vote shares.
