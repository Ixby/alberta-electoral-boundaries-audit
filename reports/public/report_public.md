...
**What we found.** The same zone of Calgary, drawn by the same commission under the same constraints, comes out a quarter larger on one map than on the other. In the city's north-east and central districts — where thirteen of seventeen seats went NDP in 2023 — the minority map's divisions average 11.5% above the province-wide population, versus 2.8% on the majority. The pattern meets the audit's pre-registered packing-signature test (academic report §5.3.1).
...
**What we found.** The minority map spreads its district populations markedly wider than the majority's. On the audit's official-shapefile measurements, its spread (3,938 vs the majority's 2,827) sits at the 99th percentile of the canonical neutral ensemble — only about 1 in 100 neutral maps produces a worse spread. The commission's own population tables tell the same story: a Population Mean Absolute Deviation (MAD) of 4,707 against the majority's 3,180 — 48% wider. (The two measurements agree; both are reported in [`findings/population_equality.md`](findings/population_equality.md).)
...
*(Figure regenerated 2026-07-08 against canonical values; the prior version's five defects are itemized in the academic report's corrections ledger.)*
...
The full neutral distribution: median 44.8%, 95th percentile 47.1%, 99th percentile 48.4%, maximum 51.72% — and 69 of the 1,010,000 plans exceed the minority map's 51.69%. The other fairness measures repeat the story: on the mean-median gap the same map lands deeper in the tail than 99.97 per cent of the neutral maps, and on declination deeper than 98.8 per cent — all leaning the same way, toward the UCP. What does that mean in seats? On the actual 2023 voting patterns, the minority map's geometry awards the UCP 60 of 89 seats, against 55 on the majority map.

To calibrate what "aiming" can achieve, the audit also ran a targeted hill-climbing procedure in both directions — the standard tool for exploring biased-but-legal maps (Cannon et al. 2022) — 40,000 steps each, same constraints, same geometry:
...
**What we found.** The fourth partisan-fairness metric, the *efficiency gap* — a single number measuring how lopsidedly a party's votes translate into seats — came in at +4.0% for the minority map: the 94th percentile. The audit's pre-registered outlier line is the 95th percentile (about +4.1% on Alberta's simulated distribution). It missed. Leaning the UCP's way, but below the line, and reported that way in every table in this report. The majority map's efficiency gap is +0.1% (16th percentile).

**What it could show.** Taken alone, almost nothing — which is exactly why it is in the log.

**What the experts say.** Near-misses are the natural behaviour of an honest test battery: a report whose every test fires should make a reader more suspicious of the auditor than of the map. This one missed and stays a miss. (The efficiency gap's proposed US threshold, about 7%, is an academic proposal no court has adopted.)
...
**What we found.** Canada spent roughly sixty years building independent boundary commissions — the institutional answer, since the 1960s federal reform era, to the once-routine practice of governing parties drawing their own ridings (Courtney 2001). The process now promoting a government-controlled committee mid-cycle is **without precedent among the Canadian redistribution cycles this audit reviewed** — an assessment political scientist Duane Bratt (Mount Royal University) shared in correspondence with the author. None of the reviewed provinces lets a governing-party majority hand redistricting to a committee its own party controls partway through a cycle. Most provinces either require the legislature to debate the commissioners' map first, or give the commission's map automatic effect unless overridden. Alberta does neither.
...
The horizontal axis is Act II's headline number (the efficiency gap — further right, more UCP-favoured). The vertical axis is Act I's count of discriminating structural tests failed. The 2019 map and the 2026 majority sit in the quiet corner: near-zero skew, zero structural flags. The minority sits alone: four of four structural flags, efficiency gap just under the line the audit set in advance, and — off this chart's axes — the tied-vote seat share only 69 of 1,010,000 neutral maps exceed, the mean-median gap deeper in the tail than 99.97 per cent of them, and the 60-of-89 seat outcome. The two lanes converge on the same map and the same communities: the urban-hybridization pattern Act I found is the structural pattern consistent with the seat advantage Act II measured — even though the specific geometry-is-the-mechanism claim failed its test (Casing 9), both lanes still agree on who benefits.

| What was measured | Majority map | Minority map |
|---|:--|:--|
| **Structural tests failed** (five pre-registered; the retracted fifth counts against neither map) | **0 of 5** | **4 of 5**, each by a wide margin |
...
| **Efficiency gap** | +0.1% — near dead centre (16th percentile) | +4.0% — the 94.5th percentile: near, but below, the line set in advance. A reported miss |
| **The combined profile, under neutral drawing** | Not flagged | At most about **1 in 568,000** — a deliberately cautious ceiling |

Act I carries the case. Its five tests were pre-registered on April 24, 2026, before the final simulation results were compiled; its measurements are geometric and do not depend on any sampler or vote attribution. Act II corroborates without carrying. (An earlier, stronger combined figure was retired on 2026-06-10 because its two channels were not independent — the story is Casing 8; the full entry is in the academic ledger.)
...
>
> The minority map is something else. It crosses four of five structural tests — the chair-flagged corridors (3 confirmed of the 7 configurations he criticized), the four-way Airdrie split, the national-park extension, and the 48%-wider population spread. *(Municipal anchoring is the fifth test; it is retracted — both maps fall within the 70–85% Canadian norm on official shapefiles; the story is Casing 5.)* And against 1,010,000 computer-simulated neutral maps drawn from the official Elections Alberta shapefiles, fewer than 100 match its seat share at a tied vote. Three of its four fairness measures land in the far tail; the fourth — the efficiency gap, at the 94.4th percentile — leans the same way but stays below the line; all four agree on who they favour. The chance of that combined profile arising under a neutral drawing process is at most one in 568,000, a deliberately cautious ceiling.
>
...
> **CORRECTIONS.** This audit corrects in public and deletes nothing. The retracted structural test is told in full at Casing 5; the two retired statistical tests at Casings 8 and 10. The complete ledger of every correction — what changed, when, and why — is kept in the [academic report's DOCUMENTED CORRECTIONS section](https://github.com/Ixby/alberta-electoral-boundaries-audit/blob/master/reports/academic/report_academic.md).
