...
- **Substrate-stale numbers (retracted here).** The published Phase B numbers (`findings/drain_label_shuffle_null.md`: majority drain = 0.000179, minority = 0.006176; majority z = −2.915 "anomalously low") and the joint-outlier-score reference to "majority anomalously clean drain" were computed against a DPG-era / blended-vote substrate. On the canonical substrate the *direction* of the continuous score reverses (majority > minority, matching the coupled-count table above). Until a canonical-substrate label-shuffle null is re-run, the "majority drain anomalously low" reading in `findings/joint_outlier_score.json` is treated as substrate-stale and should not be cited as evidence. The §5.3.5 coupled-count finding (majority 6 / minority 2 / 2019 5) is canonical and stands. Channel 3 remains excluded from the Bonferroni headline combination (p ≤ 2.80×10⁻⁶), so this retraction does not affect the audit's headline.
