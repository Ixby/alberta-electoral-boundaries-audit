...
**MCMC neutral-ensemble outlier test (§5.4, Ch1).** The minority map's four partisan metrics jointly produce Mahalanobis D = 5.72 against the canonical ensemble covariance (p = 1.40×10⁻⁶ parametric χ²₄; see referee #1 finding on the ESS-bounded empirical alternative). Against the 1 million-plan canonical ensemble (n_eff 1,429–1,682), three of four metrics individually exceed the 95th percentile: mean-median at p99.98, **declination at p98.79 (UCP-tail under the Amendment 10 sign-convention correction, 2026-06-12)**, and seats@50/50 at p99.99 (ESS-adjusted lower bound ≈p98, above p95 — flag reinstated). Efficiency gap at p94.4 remains below the threshold and is not flagged. The joint Mahalanobis result stands; the corrected declination sign means the four marginals now agree on direction. The majority sits at p0.92 mean-median (NDP-tail) and within the neutral null on the other three metrics.

**Combined-channel result (§5.4, headline = Bonferroni 2026-06-10).** Ch1 (Mahalanobis, p = 1.40×10⁻⁶) and Ch2 (SZAT swing-zone bootstrap, p = 0.0024) are combined under the **Bonferroni dependence-robust upper bound p ≤ 2.80×10⁻⁶ (≈ 1 in 357,000)** — valid under arbitrary dependence between the channels. The earlier Fisher combination (p = 6.87×10⁻⁸) is anti-conservative under positive dependence (Brown 1975) and is retired as the headline figure; it appears in the methods section as the dependence-naive value for completeness. Both channels are seeded from publicly-verifiable drand League of Entropy beacon rounds predating the shapefile release. (Updated 2026-07-08: Ch2's p = 0.0024 is the i.i.d.-flip value and does not survive a contiguity-respecting block-permutation null — p = 0.1947, variance inflation 5.79×; see `findings/szat_block_permutation.md`. SZAT is retired as a confirmatory channel; the joint headline rests on Ch1 alone. The Bonferroni bound p ≤ 2.80×10⁻⁶ is numerically unchanged because Ch1 was always the binding term.)
...
*Note on why the 1 million-plan Ch1 p-value is less extreme than the 100k run:* as the ensemble grew from 100k to 1 million plans (n_eff 379 → 1,429 for the minimum metric), the Mahalanobis covariance matrix was estimated more precisely. The better-calibrated covariance shows the minority map is somewhat less extreme than the noisier 100k estimate suggested — Ch1 p moved from 1.60×10⁻⁷ to 1.40×10⁻⁶. This is a calibration effect, not a weakening of the finding. The dependence-naive Fisher combined p (6.87×10⁻⁸) is retained for methodology completeness but is not the headline; the Bonferroni-bounded headline (p ≤ 2.80×10⁻⁶) is the operative value. Individual metric percentiles (mean-median p99.98, seats@50/50 p99.99) are stronger, not weaker, in the 1 million-plan run.
