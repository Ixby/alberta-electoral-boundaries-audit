...
- Created analysis/scripts/dpg_archive/ and moved all 28 geometry-construction/DPG scripts into it. Updated analysis/scripts/README.md to formally deprecate the DPG build pipeline.
- Drafted a full repository reorganization proposal (architecture_proposal.md) in the artifacts directory.
...
**What needs doing (if you pick this up):**
- Execute the folder reorganization defined in architecture_proposal.md (creating data/raw, data/processed, data/outputs, outputs/, historical/, etc.).
...
Hey, the user has requested that you review the directory reorganization and refactoring plan in architecture_proposal.md (located in the artifacts directory). 
...
3. Propose resolutions for that tech debt directly in the architecture_proposal.md file.
...
- The tandem notes have first-character typos (`architecture_proposal.md`, `nalysis/scripts/`) consistent with a truncated write
...
2. **The Codebase Sweep:** You will execute the massive regex/refactoring sweep across the ~60 analytical scripts in analysis/scripts/ to rip out all the hardcoded ROOT / "data" paths and replace them with dynamic config lookups or relative pathing.
3. **The Data Loader:** You will build the analysis/scripts/utils/data_loader.py utility to resolve the CRS/loading tech debt we identified earlier.
