...
Samples:
  ...
  - rural_ndp_share ~ Uniform(0.26, 0.36)  (empirical range of rural
    ...
    See analysis/scripts/v0_1_cross_election_rural_baseline.py.)
...
def jittered_mapping(
    mapping: dict, base_weight: float, jitter: float, rng: random.Random
) -> dict:
...
def summarize(label: str, values: list):
    ...
    print(
        f"  {label:<30s} : mean={mn:+.3f}  median={p50:+.3f}  "
...
def main():
    ...
    print(
        f"Samples collected: {len(results['asymmetry'])} of {n_requested} requested"
        f" (skipped: {n_skipped})"
    )
    print()
    ...
    print("=" * 70)
    print(
        f"  Samples with minority more UCP-favorable (negative): {neg}/{len(asym)} ({neg/len(asym)*100:.1f}%)"
    )
    print(
        f"  Samples with minority less UCP-favorable (positive): {pos}/{len(asym)} ({pos/len(asym)*100:.1f}%)"
    )
    print(
        f"  Samples within +/- 0.05 pp of zero:                  {zero_cross}/{len(asym)} ({zero_cross/len(asym)*100:.1f}%)"
    )
    if asym:
        ...
        if ci_lo < 0 and ci_hi < 0:
            print(
                f"  VERDICT: 95% CI [{ci_lo:+.2f}, {ci_hi:+.2f}] pp is entirely negative."
            )
            print(
                f"           Directional claim DEFENSIBLE under modeling uncertainty."
            )
        elif ci_lo > 0 and ci_hi > 0:
            print(
                f"  VERDICT: 95% CI [{ci_lo:+.2f}, {ci_hi:+.2f}] pp is entirely positive."
            )
...
def cross_check_2019_votes():
    ...
    rural_ndp_2019 = sum(d["ndp"] for d in rural) / sum(
        d["ndp"] + d["ucp"] for d in rural
    )
    print(f"  Rural NDP share in 2019: {rural_ndp_2019*100:.1f}%")
    print(
        f"  (2023 rural NDP share was 33.5% — 2019 likely higher given NDP won in 2015)"
    )
    print()

    # Estimate 2026 maps using 2019 votes
    from packing_cracking_analysis import estimate_2026, compute_metrics
    ...
    print(
        f"    2019 map (actual):  NDP {m_2019['ndp_seats']}/UCP {m_2019['ucp_seats']}, EG {m_2019['eg']*100:+.2f}%, MM {m_2019['mm_gap']*100:+.2f}pp"
    )
    print(
        f"    Majority 2026 (est): NDP {m_maj['ndp_seats']}/UCP {m_maj['ucp_seats']}, EG {m_maj['eg']*100:+.2f}%, MM {m_maj['mm_gap']*100:+.2f}pp"
    )
    print(
        f"    Minority 2026 (est): NDP {m_min['ndp_seats']}/UCP {m_min['ucp_seats']}, EG {m_min['eg']*100:+.2f}%, MM {m_min['mm_gap']*100:+.2f}pp"
    )
