...
(a) **Publication-grade MCMC.** A 1,000,000-sample ReCom run with thinning to ≈ 5,000 effectively-independent draws, per MGGG lawsuit-grade practice, seeded on the commission's final 2026 shapefile rather than the 2019 map. The current 250,000-sample run gives n_eff ≈ 150 per metric (§5.4) — sufficient for a policy-comparison finding but not for a peer-review-grade statistical-significance claim at either tail. The 1M run would resolve whether the minority's mean-median p98.8 + declination p1.6 pattern holds at higher effective precision.
