...
The audit's central Lane-1 finding is that the minority's seats@50/50 = 0.5169 sits at p99.99 against the 1.01M-plan canonical ensemble *under uniform partisan swing*. When the same metric was recomputed under regional swing (Calgary +11.5 pp NDP, rural +8.1 pp, Edmonton +4.3 pp from 2019→2023) on the v0_9 10k verification subset, the minority dropped to p50.7 and the majority moved to p99.5 in the opposite direction (see `findings/regional_swing_robustness.md`). This is the single most likely Lane-1 falsifier.

The canonical 1.01M run stores only district-level summaries (the four partisan metrics + ucp_seats + n_districts + ucp_vote_share). Regional-swing recomputation requires per-VA assignments, which only live in `data/outputs/simulated_ensemble_raw_samples_canonical.csv` (Git LFS, ~170 MB). On this branch the LFS payload is the pointer file, not the binary.

**Resolution path:**
1. `git lfs install && git lfs pull --include="data/outputs/simulated_ensemble_raw_samples_canonical.csv"` (or pull the v0_9-style verification subset if it's smaller).
2. Run `python analysis/scripts/seats_at_50_50_regional.py --canonical --output data/outputs/regional_swing_canonical_ensemble.json` — but extend the script to *also rescore the ensemble*, not just the three real maps (current `main()` only scores `MAJ_CANONICAL`, `MIN_CANONICAL`, `2019_enacted`).
3. The script extension is roughly: load the per-VA assignment npz alongside the ensemble percentiles CSV, apply the regional-swing delta per VA, re-aggregate to districts per plan, recompute s50, then report new percentiles.

**Acceptance criterion:** A new file `findings/regional_swing_canonical_robustness.md` with minority + majority + 2019 s50 *under regional swing* against the canonical 1.01M ensemble, and a single headline sentence: either the p99.99 holds within ±2 percentile points (Lane 1 survives), or it collapses (Lane 1 is coverage-sensitive on swing assumption, in which case §6.2.4/6.2.5/6.2.8 verdict sentences need a second round of edits).
