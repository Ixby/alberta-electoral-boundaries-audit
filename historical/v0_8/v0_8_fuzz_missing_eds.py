...
def main():
    ...
    minority = gpd.read_file(
        DERIVED / "v0_8_full_refined_minority_2026_eds.gpkg"
    ).to_crs(va.crs)
    joined = gpd.sjoin(
    ...
    agg19 = (
        joined19.dropna(subset=["EDName2017"])
        .groupby("EDName2017")
        .agg(ucp=("va_ucp", "sum"), ndp=("va_ndp", "sum"))
        .reset_index()
    )
...
def seats_at_50_50(ucp_arr, ndp_arr):
    ...
    return (
        float((shifted > 0.5).sum() / len(ucp_arr)),
        int((shifted > 0.5).sum()),
        len(ucp_arr),
    )
...
def add_rows(base_df, rows):
    ...
    print("=" * 78)
    print(
        "FUZZING SCENARIOS — minority map seats@50/50 under different fill strategies"
    )
    print("=" * 78)
    print()
    print(f"Missing EDs (6): {missing_eds}")
    print(
        "Simulation 2M ceiling = 51.72% UCP seats at 50/50 (the out-of-distribution line)"
    )
    ...
    scenarios.append(
        ("A. Inherited 2019 polygons (most defensible)", s50, seats, n, eg)
    )

    # B: Worst case for finding (all 6 missing → strongly NDP)
    median_total = (agg["ucp"].values + agg["ndp"].values).mean()
    rows_b = [
        {"name_2026": ed, "ucp": median_total * 0.40, "ndp": median_total * 0.60}
    ...
    scenarios.append(
        ("B. All 6 missing assumed strongly NDP-leaning (40/60)", s50, seats, n, eg)
    )

    # C: Best case for finding (all 6 missing → strongly UCP)
    rows_c = [
        {"name_2026": ed, "ucp": median_total * 0.60, "ndp": median_total * 0.40}
        for ed in missing_eds
    ]
    ...
    scenarios.append(
        ("C. All 6 missing assumed strongly UCP-leaning (60/40)", s50, seats, n, eg)
    )

    # D: Regional-median attribution
    calgary_eds = [
        e
        for e in agg["name_2026"]
        if "Calgary" in e or "Chestermere" in e or "Airdrie" in e
    ]
    ...
    scenarios.append(
        (
            "D. Regional-median attribution (Calgary -> Calgary share, Edmonton -> Edmonton share)",
            s50,
            seats,
            n,
            eg,
    ...
    np.random.seed(42)
    existing_shares = agg["ucp"].values / (agg["ucp"].values + agg["ndp"].values).clip(
        min=1
    )
    ...
    print(
        f"   mean s@50 = {results_e.mean()*100:.2f}%   p5 = {np.percentile(results_e, 5)*100:.2f}%   p50 = {np.percentile(results_e, 50)*100:.2f}%   p95 = {np.percentile(results_e, 95)*100:.2f}%"
    )
    print(f"   min = {results_e.min()*100:.2f}%   max = {results_e.max()*100:.2f}%")
    print(
        f"   trials above 51.72% ceiling: {(results_e > 0.5172).sum():,} of 10,000 = {(results_e > 0.5172).mean()*100:.2f}%"
    )
    print(
        f"   trials above 50%: {(results_e > 0.50).sum():,} of 10,000 = {(results_e > 0.50).mean()*100:.2f}%"
    )
    print(
        f"   mean EG = {egs_e.mean()*100:+.2f}%   p5 = {np.percentile(egs_e, 5)*100:+.2f}%   p95 = {np.percentile(egs_e, 95)*100:+.2f}%"
    )
    print()
    print(f"Simulation ceiling (2M ensemble max): 51.72% UCP seats at 50/50")
    print()
    print(
        f"Of the 4 explicit scenarios above the 51.72% ceiling: {sum(1 for _,s,_,_,_ in scenarios if s > 0.5172)}/{len(scenarios)}"
    )
