...
**Acknowledgements and tools used.** No traditional statistical software (R, Stata, SPSS), no GIS desktop software (QGIS, ArcGIS), no commercial election-analytics platforms, and no paid datasets were used; all inputs are public. Integrity tools: falsifiability gates G0–G5 built into the pipeline; bias self-audit at `analysis/reports/v0_1_bias_audit.md`; uncertainty analysis at `analysis/methodology/v0_1_uncertainty_and_shapefile_impact.md`.
