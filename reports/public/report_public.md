...
| Seats at 50/50 votes (percentile in 1,010,000-map simulation) | 46.1% — p83 (normal range) | **51.7% — p99.99 (fewer than 100 of 1,010,000 reach this)** | **UCP** |
...
The minority map sits in the extreme tail of the 1,010,000-plan ensemble on three of four partisan-fairness metrics, with the fourth (efficiency gap) at the 94th percentile — *near, but below*, the audit's pre-registered Alberta-calibrated threshold of the 95th. **The honest joint reading is not a single one-in-fifteen-million number.** That earlier figure combined two channels (the four-metric Mahalanobis joint score and the Swing-Zone Allocation Test) that share the same underlying efficiency-gap data — they are not statistically independent, so multiplying their p-values overstates the joint significance. The defensible upper bound from a dependence-robust combination is roughly **one in 350,000 (Bonferroni-corrected; p ≈ 2.8×10⁻⁶)**. That is still an extreme result and an order of magnitude beyond conventional significance thresholds, but it is reported here as a bound, not as four independent instruments reading in agreement.

**Two questions, two results.** The 1,010,000-map simulation asks: *is this map extreme compared to neutral maps drawn on the same Alberta geography?* The Swing-Zone Allocation Test asks a different question: *are the specific lines on the map partisan-neutral, given the lines both maps shared?* Both questions return statistically significant answers in the same direction. Because the two tests use overlapping vote data, the audit reports each separately and combines them under the dependence-robust bound above rather than treating them as fully independent.
