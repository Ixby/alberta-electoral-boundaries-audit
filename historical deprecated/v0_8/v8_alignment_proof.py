...
Outputs:
  data/v0_1_topology_alignment_proof.json
  data/v0_1_topology_alignment_proof.md  (human-readable summary)
...
def write_markdown(report: dict, path: Path) -> None:
    ...
    for plan in ("majority", "minority"):
        ...
        lines.append(
            f"- ED count: **{topo['n_eds']}** (expected {topo['expected_n']}) — {'✓' if topo['n_match'] else '✗'}"
        )
        lines.append(
            f"- EDs with geometry (area ≥ 0.1 km²): **{topo['n_with_geometry']} / {topo['n_eds']}**"
        )
        if topo["n_empty_eds"]:
            lines.append(
                f"- **Empty EDs (area < 0.1 km²): {topo['n_empty_eds']}** — these EDs were not assembled from source data and remain unfilled. This is a known data-completeness limitation inherited from v0_7."
            )
            lines.append("")
        ...
        lines.append(
            f"- Residual overlaps > 1 m²: **{topo['overlap_pairs_gt_1m2']}** "
            f"({topo['overlap_total_km2']:.6f} km² total)"
        )
        if topo["province_reference_km2"]:
            lines.append(
                f"- Coverage: **{topo['coverage_pct']:.4f}%** of provincial area "
                f"({topo['coverage_km2']:.1f} / {topo['province_reference_km2']:.1f} km²)"
            )
        else:
            lines.append(
                f"- Coverage area: {topo['coverage_km2']:.1f} km² (no provincial reference available)"
            )
        lines.append("")
        ...
        if misses:
            lines.append(
                f"**{len(misses)} landmark check(s) failed** — likely misalignment or ED-naming mismatch."
            )
        else:
    ...
    if cross.get("available"):
        ...
        lines.append(
            f"- Absolute diff: {cross['abs_diff_km2']:.4f} km² ({cross['rel_diff_pct']:.4f}%)"
        )
        lines.append("")
...
def main() -> int:
    ...
    for plan in ("majority", "minority"):
        ...
        print(f"\n[{_ts()}] === {plan} ({src}) ===")
        print(
            f"  EDs={topo['n_eds']} (expected {topo['expected_n']})  "
        ...
        print(
            f"  landmark check: {len(landmarks) - len(misses)}/{len(landmarks)} cities matched"
        )
        for r in misses:
        ...
        print(
            f"[{_ts()}] [{plan}] alignment-proof block done in "
            f"{time.time()-t_plan:.2f}s",
            flush=True,
        )
    ...
    json_path = DATA / "v0_1_topology_alignment_proof.json"
    md_path = DATA / "v0_1_topology_alignment_proof.md"
    json_path.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    write_markdown(report, md_path)

    print(f"\nWrote {json_path.name} and {md_path.name}")
    print(
        f"[{_ts()}] [v0_8 alignment proof] DONE — total {time.time()-t_start:.2f}s",
        flush=True,
    )
    return 0
