...
**Three-tier boundary-confidence classification.** For each Tier B boundary the refinement pass computed a voter-assignment-impact score — the number of 2023 Voting Areas whose assignment would flip between the v1 and v2 boundary rendering and the 2023 votes those VAs contain. Boundaries with zero sensitive VAs are marked **orange-accepted**: the residual geometric uncertainty does not affect voter assignment and further refinement would be effort without return. Lethbridge-Little Bow and Wetaskawin-Ponoka-Maskwacis are orange-accepted. Three boundaries remain **refinement-significant** with documented residual voter impact: Calgary-De Winton and Calgary-South share a single sensitive VA (216 votes); Edmonton-Windermere has three sensitive VAs (796 votes) on the North-Saskatchewan-River bank where left / right assignment cannot be determined from the commission PNG without the actual shapefile. Total residual voter impact across all Tier B boundaries after v2: **1,012 votes across 4 unique VAs** — approximately 0.06 % of 2023 total valid votes. No boundary is categorically unresolvable; all three refinement-significant residuals would be resolved by commission shapefile release. Per-boundary impact table at `data/v0_1_boundary_refinement_impact.csv`; verification overlays with the three-tier green / orange / red convention at `maps/verification/v0_2_*.png`.
