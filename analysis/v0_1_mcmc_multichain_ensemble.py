#!/usr/bin/env python
"""
v0_1_mcmc_multichain_ensemble.py
=================================
...
Outputs:
    data/v0_1_mcmc_multichain_samples.csv        # per-chain per-step samples (post-burnin)
    data/v0_1_mcmc_multichain_pooled.csv         # thinned + pooled
    data/v0_1_mcmc_multichain_rhat.json          # per-metric R-hat + ESS
    data/v0_1_mcmc_multichain_summary.md         # human-readable summary

CLI:
    python v0_1_mcmc_multichain_ensemble.py --seeds 42,101,2024 --steps 30000
    python v0_1_mcmc_multichain_ensemble.py --seeds 42,101,2024 --steps 100000  # publication grade
...
Backward:
  ...
  analysis/v0_1_mcmc_ensemble.py (build_va_graph, run_ensemble, seat_results)
  analysis/v0_1_mcmc_full_coverage_rescore_100k.py (efficiency_gap, mean_median,
...
from v0_1_mcmc_ensemble import (  # noqa: E402
    build_va_graph,
...
from v0_1_mcmc_full_coverage_rescore_100k import (  # noqa: E402, F401
    efficiency_gap,
    mean_median,
    declination,
    seats_at_50_50,
...
SAMPLES_CSV = DATA / "v0_1_mcmc_multichain_samples.csv"
POOLED_CSV = DATA / "v0_1_mcmc_multichain_pooled.csv"
RHAT_JSON = DATA / "v0_1_mcmc_multichain_rhat.json"
SUMMARY_MD = DATA / "v0_1_mcmc_multichain_summary.md"
...
def run_one_chain(graph, assignment, seed: int, n_steps: int,
    ...
    # deterministic per (seed, initial-state, graph).
    np.random.seed(seed)
    _random.seed(seed)
...
def write_summary_md(out_path: Path, seeds: list[int], steps_per_chain: int,
    ...
    lines.append(f"**Script:** `analysis/v0_1_mcmc_multichain_ensemble.py`\n")
    ...
    if overall_converged:
        lines.append(
            "The pooled ensemble in `data/v0_1_mcmc_multichain_pooled.csv` "
    ...
    lines.append(
        f"python analysis/v0_1_mcmc_multichain_ensemble.py "
...
def main(argv: list[str] | None = None) -> int:
    ...
    rhat_payload = {
        ...
        "script_version": "v0_1_mcmc_multichain_ensemble.py",
    }
