...
**Policy contribution: Elections Alberta should publish home-VA vote totals for advance ballots.** In the 2023 provincial election, 395,472 NDP and UCP votes cast advance, mobile, or special carry no Voting Area identifier in EA's published data (§3.2 item 5). This is not a technical limitation. Every advance voter is verified against the provincial voters list before receiving a ballot; that list links each registered voter to a specific Voting Area. The home-VA identifier exists at the point of voting. What does not exist is a policy, procedure, or legislative requirement for EA to retain or publish that link in its results reporting. The barrier is administrative, not technical. Publishing VA-level advance vote totals alongside the existing election-day VA breakdowns requires no change to the voting process, no additional interaction with the voter, and no new data collection — only a change to what EA retains and reports from data it already holds. The change would allow complete-coverage VA-level vote attribution in future audits, provide a more accurate basis for community-of-interest assessments that depend on knowing where voters live rather than where they voted, and bring Alberta's results reporting to the geographic granularity that systematic electoral analysis requires. The audit offers this as a policy contribution, not a finding.
