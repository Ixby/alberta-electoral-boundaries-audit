...
| 7 | Efficiency-gap "bright-line" (US reference) | 7 % | §3.3 (B2), §8.1 | **LIT** | Stephanopoulos & McGhee (2015); cited in Gill v. Whitford (2018); not judicially adopted. See §B.2.1.A–C for three Alberta-calibrated alternatives (#39–#41) |
...
| 16 | Monte Carlo urban-weight sampling | Uniform(0.55, 0.85) | RT1 (monte_carlo_ci.py) | **MOD** | ±15 pp about central 0.70 (wider than point-sensitivity range) |
...
| 39 | EG assembly-size sensitivity floor | ≈ 2.2 % (2/89 seats) | §5.2.8 Option A | **FP** | First-principles scaling of S&M (2015) 2-seat-in-20-seat logic to 89-seat assembly |
| 40 | EG EBCA statutory-proportional threshold | 5 % (one-fifth of ±25 % band) | §5.2.8 Option B | **FP / STAT** | EBCA § 14 proportional anchoring; same logic as P1 packing threshold (#10) |
| 41 | EG Alberta historical-swing threshold | 1.01 % / 4.10 % / 9.71 % (2019 / 2023 / 2015) | §5.2.8 Option C | **FP / OBS** | Jurisdiction-normed p95 EG range across Alberta's three electoral contexts (100k-plan ReCom, seed 3562959107; 2023 = canonical 1M). Floor 1.01 %, centre 4.10 %, ceiling 9.71 %. Completed 2026-05-12. |
| 42 | EG MCMC ensemble 95th percentile | **4.10 %** | §5.2.8 Option D | **MOD** | 1,010,000-plan ReCom canonical ensemble, official EA shapefiles, seed 1432864451, ±25 %; `data/simulated_ensemble_percentiles_canonical.csv` p95 EG = +0.0410 |

Counts by provenance class: **STAT**: 6 · **LIT**: 6 · **FP**: 16 · **MOD**: 7 · **AUD**: 5 · **LIT/FP composite**: 1 · **FP/STAT composite**: 1 · **FP/OBS composite**: 1. Total catalogued thresholds: 42 (with #22 classified as an observed value rather than a threshold; #41 classified as computation-in-progress). Four new entries (#39–#42) document Alberta-calibrated EG threshold alternatives; see §B.2.1.A–D and report_academic.md §5.2.8.
...
**B.2.1 7 % efficiency-gap bright-line (Threshold #7).** Stephanopoulos and McGhee (2015) proposed a 7 percentage-point efficiency-gap threshold as a presumptive indicator of partisan gerrymandering at the Congressional-district level. The threshold was prominently discussed in *Gill v. Whitford*, 585 U.S. ___ (2018), although the Supreme Court ultimately disposed of the case on standing grounds and did not adopt the 7 % figure as a constitutional test. The audit uses the figure as a reporting reference in `compute_metrics()` verbose output (`packing_cracking_analysis.py` line 186) — EGs within 7 % are marked "within" and those exceeding 7 % are marked "EXCEEDS" — but does not claim that either 2026 Alberta map crosses the threshold (observed EGs are −0.40 % and −1.81 %, both well within). **Defence.** The 7 % figure is the most widely-cited operational threshold in the gerrymandering literature. Using it as a reporting reference connects the audit's output to a shared vocabulary without over-claiming. **Sensitivity.** Not applicable in this audit because observed EGs are an order of magnitude smaller than the threshold; a reviewer who substitutes 5 % or 10 % for the 7 % reference changes the verbose-print language but no substantive finding. **Residual vulnerability.** The 7 % figure has drawn academic criticism (see Stephanopoulos & McGhee 2018 for the authors' own response to critics; Warrington 2019 for a comparison of metrics). The audit does not hinge on whether 7 % is the "correct" bright-line, only on its role as a recognised reference point.

**B.2.1.A Option A — Assembly-size sensitivity threshold (≈ 2.2 %; Threshold #39).** Stephanopoulos and McGhee (2015) derived the 7 % EG threshold partly from a 2-seat-in-a-20-seat-Congressional-delegation sensitivity — the minimum asymmetry at which a 50/50 vote split reliably produces a detectable seat-share gap. For an 89-seat legislature, scaling the same logic gives 2 seats / 89 = **2.25 %**; 1 seat gives 1.12 %. Option A is the *minimum-meaningful-signal floor* for Alberta's assembly size: EG variation below ≈ 2.2 % is within the integer rounding noise of seat assignment in a legislature of this size. It is not a gerrymander threshold — it is the detection floor below which directional claims cannot be distinguished from rounding. Observed minority EG (−1.81 %) is now below the 2-seat floor; observed majority EG (−0.40 %) is below both floors; the inter-map asymmetry (1.41 pp) also approaches but does not clearly exceed the 2.2 % floor. **Defence.** Derived by scaling S&M §III.B's delegation-size sensitivity logic to Alberta. **Residual vulnerability.** S&M's 2-in-20 figure itself is an approximation; scaling it to a single-province legislature applies a model developed for multi-delegation comparison. The floor should be interpreted as an order-of-magnitude guide, not a sharp threshold.

**B.2.1.B Option B — EBCA statutory-proportional threshold (5 %; Threshold #40).** One-fifth of the Act's ±25 % statutory population-deviation band, applied to EG by the same proportional-anchoring logic used for the P1 packing threshold (see B.3.1 above). The derivation: if the Commission may draw populations within ±25 %, then a corresponding EG departure of one-fifth of that band (5 %) represents a meaningful structural departure in vote-to-seat translation, calibrated entirely within Alberta's own legislative framework and importing no US-derived figure. **Defence.** This is the most conservative Alberta-anchored alternative: it is based purely on the Act and the same first-principles proportionality argument that defends P1. Observed absolute EGs (−0.40 % and −1.81 %) are both below 5 %; the inter-map asymmetry (1.41 pp) is also below 5 %. The audit's sub-threshold characterisation is robust against Option B. **Residual vulnerability.** The "one-fifth" fraction is a choice; one-quarter (6.25 %) or one-third (8.3 %) are equally principled. However, any fraction from one-tenth (2.5 %) to one-half (12.5 %) of the band produces the same result (all observed values below threshold); the finding is insensitive to the fraction choice within a wide range.

**B.2.1.C Option C — Alberta historical-swing threshold (computation underway; Threshold #41).** A gerrymander-candidate threshold calibrated to the 95th percentile of Alberta's own historical EG distribution under prior-cycle boundaries would be the most jurisdiction-specific reference available. Issue #16 computation is now underway (2026-05-12): 100k-plan ReCom ensembles (seed 3562959107, 2 chains × 50k plans) are running under 2019 and 2015 vote inputs, area-proportionally attributed to the same 4,765-VA canonical substrate via `parent_ed_2019` crosswalk and a population-weighted 2015→2019 ED projection (`data/outputs/2015_to_2019_crosswalk.csv`). The 2023 canonical p95 (Option D, 4.10 %) serves as the third data point. Once the 2015 and 2019 runs complete, Option C will report the jurisdiction-normed p95 EG threshold range across Alberta's three measured electoral contexts. Because EG is intrinsically sensitive to vote-share distribution, the same geographic substrate under different elections produces a different neutral-ensemble 95th percentile; Option C's purpose is to quantify that range — floor, ceiling, and centre — rather than to test whether 4.10 % is election-invariant. **Residual vulnerability.** Area-proportional VA attribution approximates direct ballot assignment; deviations from proportionality within an ED are systematic only if population density and partisanship are correlated within EDs (plausible in large rural EDs with one population node). The approximation is appropriate for this jurisdiction-norming analysis rather than for point-estimate reporting of any single map's EG. **Write-up pre-commitment (documented 2026-05-12, before Option C results arrived).** If either the 2015 or 2019 ensemble run produces a p95 EG below +0.0402 (the minority map's canonical EG), the minority map will sit above that run's threshold. That outcome will be reported as-is. Option C results will not be used to retroactively select a lower threshold as the primary Alberta standard, nor will the write-up treat the lowest p95 across the three electoral contexts as the operative threshold for assessing the real maps. Option D (4.10 %, derived from the 2023 electoral context, Threshold #42) remains the primary Alberta-calibrated threshold; Option C establishes a range envelope. All three p95 values — 2015, 2019, and 2023 — will be reported in full regardless of their relationship to the observed map EGs. This pre-commitment is in the audit record before results were available so that the subsequent write-up cannot be characterised as specification searching.

**Option C results (completed 2026-05-12).** All three p95 EG values:

| Context | NDP two-party | Ensemble p50 EG | p95 EG | Notes |
|---|---|---|---|---|
| 2015 (NDP wave) | 44.0 % | +8.10 % | **+9.71 %** | Minority EG +10.54 % at p99.45 — over threshold; majority +7.58 % at p30.9 — sub-threshold |
| 2023 (competitive) | 45.7 % | +1.66 % | **+4.10 %** | Option D canonical. Minority p94.4 sub-threshold; majority p15.5 sub-threshold |
| 2019 (UCP landslide) | 37.3 % | −1.25 % | **+1.01 %** | Minority −0.49 % at p70.4 sub-threshold; majority −1.32 % at p48.3 sub-threshold |

Jurisdiction-normed range: **floor 1.01 %, centre 4.10 %, ceiling 9.71 %**. The 9 pp spread is the empirical demonstration that EG is vote-share-dependent and a single-election threshold would be misleading. Option D (4.10 %, 2023 context) is the operative threshold for 2026 assessment; the 2015 ceiling reflects a historically anomalous electoral outcome and is not calibrated to plausible 2026 conditions. The minority map's over-threshold result under 2015 votes is reported as required by the pre-commitment and does not alter the primary Option D verdict.

**B.2.1.D Option D — MCMC ensemble 95th percentile (4.10 %; Threshold #42).** The 1,010,000-plan canonical ReCom ensemble on official Elections Alberta shapefiles (seed 1432864451, ±25 % population constraint, 4 chains × 252,500 plans) produces a 95th-percentile EG of **+0.0410** (4.10 %; `data/simulated_ensemble_percentiles_canonical.csv`). This is the most empirically grounded Alberta-specific threshold: it measures where genuinely neutral redistricting would place the 95th-percentile UCP-favoured plan under the same geographic and legal constraints applied to the real maps. The minority map's canonical EG of +0.0402 (4.02 %) places it at p94.4 — just below this threshold; the majority at +0.0010 (0.10 %) is well inside. An earlier 250k DPG run produced 4.37 %; the canonical 1M official-boundary run supersedes it (downward revision by 0.27 pp, reflecting the difference between traced provisional geometries and official shapefiles). **Defence.** This is the only threshold derived entirely from Alberta's own geographic and legal substrate, requiring no US calibration or proportional-anchoring assumption. The 1M sample size achieves publication-grade ESS (n_eff 1,429–1,682 on partisan metrics). **Residual vulnerability.** The ensemble imposes uniform ±25 % on all 89 EDs and cannot generate maps where any ED falls below −25 %; the three s.15(2) EDs that legally reach down to −50 % are outside the ensemble's distribution. This introduces a small conservative bias in the threshold (the ensemble undershoots the full legal constraint space), but the effect on aggregate EG is expected to be negligible because s.15(2) EDs are small, remote, and have low partisan leverage.

**B.2.2 3 pp mean-median flag (Threshold #8).** McDonald and Best (2015) use a ~3 percentage-point mean-median gap as an indicator of partisan-biased districting. The audit uses this as a verbose-output flag in the same style as the 7 % EG reference (see `packing_cracking_analysis.py` line 188). Observed MM values are −0.16 pp (majority) and −0.33 pp (minority), both well within the flag. **Defence.** The 3 pp reference tracks the published literature's use. It is not load-bearing in the audit's findings. **Residual vulnerability.** Same character as the 7 % EG bright-line: a reviewer who prefers a different MM threshold changes the verbose language, not the findings.
...
**B.8.2 Population-validation bands (Thresholds #36, #37).** `validate_2026_estimate()` (`packing_cracking_analysis.py` lines 491–512) rejects any 2026 estimate whose total two-party votes fall outside [1,600,000, 1,800,000] or whose NDP share falls outside [0.40, 0.50]. The mid-points (1.7 M total; 0.45 NDP share) are the observed 2023 values (1,706,304 total; 0.4567 share); the ±5 % bands catch gross arithmetic errors without flagging modest blending artifacts. **Defence.** Sanity-check bands at ±5 % around observed values are the conventional data-validation approach. **Residual vulnerability.** None — these are error detectors, not findings.
...
From `packing_cracking_analysis.py` sensitivity output:
