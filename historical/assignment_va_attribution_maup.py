...
def compute_area_weights(
    ...
    """
    print(
        f"  [{label}] Computing VA × ED intersection ({len(vas)} VAs × {len(eds)} EDs)..."
    )
    t0 = time.time()
    ...
    if not (va_areas > 0).all():
        ...
        print(
            f"    WARNING: {n_zero} VAs have zero/negative area; they will get weight 0."
        )
    ...
    print(
        f"  [{label}] overlay returned {len(inter):,} (VA, ED) intersection rows "
    ...
    if (per_va_cover["dpg_coverage_frac"] > 1.0001).any():
        ...
        print(
            f"    WARNING: {len(over)} VAs have total DPG coverage > 1.0001 "
        ...
        if over_ids:
            ...
            inter.loc[mask, "area_weight"] = (
                inter.loc[mask]
                .groupby("OBJECTID")["area_weight"]
                .transform(lambda w: w / w.sum() if w.sum() > 0 else w)
            )
            # Re-compute coverage after normalisation (will equal 1.0 for those)
            per_va_cover = inter.groupby("OBJECTID", as_index=False)[
                "area_weight"
            ].sum()
            per_va_cover.rename(
                columns={"area_weight": "dpg_coverage_frac"}, inplace=True
            )
    ...
    print(
        f"  [{label}] DPG area coverage (of VA substrate): "
        f"{coverage_area_frac*100:.2f}%"
    )
    print(
        f"  [{label}] n VAs fully covered: {n_full}  partial: {n_partial}  "
        f"zero cover: {n_zero}"
    )
...
def apply_crosswalk_fallback(
    ...
    merged = (
        vas[["OBJECTID", "VA_NUMBER", "parent_ed_2019"]]
        .merge(
            per_va_cover,
            on="OBJECTID",
            how="left",
    ...
    if len(bad) > 0:
        print(
            f"    [{label}] WARNING: {len(bad)} fallback rows point to a "
    ...
    print(
        f"  [{label}] crosswalk-fallback rows: {n_fb}  "
...
def conservation_gate(
    apportioned: pd.DataFrame, vas: gpd.GeoDataFrame, label: str
) -> dict:
    """Verify per-VA conservation: sum of apportioned votes == original VA votes."""
    gb = (
        apportioned.groupby("OBJECTID")
        .agg(
    ...
    if not passed:
        print(
            f"    worst VA: OBJECTID={int(worst_va['OBJECTID'])}, "
...
def aggregate_to_ed(apportioned: pd.DataFrame, ed_names: list[str]) -> pd.DataFrame:
    ...
    for n in ed_names:
        ...
        if len(sub) == 0:
            ...
            u, d, o = (
                float(sub["ucp"].iloc[0]),
                float(sub["ndp"].iloc[0]),
                float(sub["other"].iloc[0]),
            )
            rows.append(
                {
                    "ed_name": n,
...
def compute_mm_gap(districts_df: pd.DataFrame) -> float:
    shares = [
        float(d["ndp"]) / (float(d["ndp"]) + float(d["ucp"]))
        for _, d in districts_df.iterrows()
...
def run_one_map(
    ...
    weights_full = apply_crosswalk_fallback(
        weights_df, vas_run, xwalk, set(ed_names), label
    )
    ...
    print(
        f"  [{label}] per-ED aggregated: UCP={ed_totals['ucp_2023'].sum():,.1f}  "
    ...
    mm = compute_mm_gap(
        ed_totals.rename(columns={"ndp_2023": "ndp", "ucp_2023": "ucp"})
    )
    ndp_seats, ucp_seats = compute_seats(
        ed_totals.rename(columns={"ndp_2023": "ndp", "ucp_2023": "ucp"})
    )
    print(
        f"  [{label}] EG (area-weighted) = {eg*100:+.4f}%   "
