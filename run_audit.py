...
for label, gdf in [("majority", maj), ("minority", minn)]:
    ...
    if null_geom or empty_geom or invalid_geom:
        bad = []
        if null_geom:
            bad.append(f"{null_geom} null")
        if empty_geom:
            bad.append(f"{empty_geom} empty")
        if invalid_geom:
            msgs = (
...
if not issues:
    results.append(
        ("2. Geometry validity", "PASS", "no null/empty/invalid in either map")
    )
else:
...
if not dups:
    results.append(
        ("3. Duplicate names", "PASS", "all name_2026 values unique in both maps")
    )
else:
...
if not blanks:
    results.append(
        ("4. Name completeness", "PASS", "no null/blank name_2026 in either map")
    )
else:
...
if all(v[0] < threshold for v in gap_results.values()):
    detail = {
        k: f"{v[0]*100:.4f}% uncovered ({v[1]/1e6:.3f} km²)"
...
for label, gdf in [("majority", maj), ("minority", minn)]:
    ...
    joined = gpd.sjoin(va_centroids, gdf_reproj[["geometry"]], how="left", predicate="intersects")
    # count matches per VA (by original index)
    match_counts = joined.groupby(joined.index).size()
    n_zero = (match_counts == 0).sum() + (
        va.index.isin(match_counts[match_counts > 0].index) == False
    ).sum()
...
if ucp_col and ndp_col and other_col:
    va_total = (
        va[ucp_col].fillna(0) + va[ndp_col].fillna(0) + va[other_col].fillna(0)
    ).sum()
...
if maj_cols == min_cols:
    results.append(("9. Column schema", "PASS", f"identical columns: {maj_cols}"))
else:
