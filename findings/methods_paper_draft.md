...
### 6. Sunset clause as in-repo discipline (~1 page)

Every DPG-dependent claim is published with a dated, public commitment. The current text reads:

> *All DPG-dependent metrics are provisional. Within **two weeks** of the relevant authority publishing official topological shapefiles, the audit commits to: (1) re-running every DPG-dependent analysis against the official geometry, (2) publicly disclosing in a dated amendment any sign-flip or material magnitude change (> threshold), and (3) treating the official-shapefile recomputation as the authoritative result.*

**Provenance and honest accounting.** The commitment was originally posted as **48 hours** and **relaxed to two weeks in Amendment 2 Change 11 (2026-04-26)** with the disclosed reason that a solo researcher without monitoring automation cannot turn a full canonical integration in 48 hours. The recompute *requirement*, the *sign-flip disclosure*, and the *symmetric-application* discipline were unchanged across the relaxation; only the timing window moved.

**This is an in-repo discipline, not an externally pre-registered binding commitment.** The four externally pre-registered files anchored at OSF w2s8k / r3zm7 / qsgy8 / 6pt83 each contain `dpg2_experiment_plan.md` and `drain_v2_plan.md`; neither file specifies a recompute timeline. The 48-hour-then-two-weeks window appears only in this paper, in `preregistration/retraction_conditions.md`, and in `findings/pre_registration_amendment_log.md`. Prior drafts described the clause as a "novel defensibility mechanism" that "inverts the usual peer-review posture into a timestamped obligation"; the more accurate framing is that the clause is a **falsifiable internal discipline** — the timestamps are real (drand-anchored), the recompute artifacts are real (`data/outputs/*_canonical.*`), and the commit history is real, so a reviewer can still verify whether the discipline was honoured. The methodological contribution is the *discipline of recomputation* with public disclosure of substrate-driven sign-flips and magnitude changes, not the speed of recomputation.

**Two trigger types.** The sunset clause covers two kinds of triggering events that have different operational profiles:

1. **Shapefile-release trigger** — the *learning-to-engineering* transition. When official shapefiles arrive after a DPG-era exploration period, the recompute involves not only running the canonical analyses but also pruning DPG infrastructure, refactoring scripts onto canonical inputs, and archiving superseded analyses. This is the transition the Alberta 2025-26 case study illustrates in §7. The two-week window is calibrated to this workload.
2. **Confirmatory-application trigger** — the *engineering-the-exam* event. When a new map is released after the audit's methodology is production-ready (the canonical scripts are written, the substrates are canonical, the thresholds are pre-registered), the recompute is faster because there is no DPG infrastructure to dismantle — only locked-in scripts to run on the new input. For the Lunty Special Select Committee's November 2026 map, the *scorecard methodology* is externally pre-registered at OSF [qsgy8](https://osf.io/qsgy8/) / AsPredicted #289,455, and the audit's internal commitment is to score the new map within **72 hours** of its publication (Amendment 2 Bucket C). The externally pre-registered object is the scorecard itself (thresholds, test battery, pass/fail criteria); the 72-hour operational standard is in-repo.

If the audit fails to honour either window — the two-week shapefile-release window or the 72-hour confirmatory-application window — the failure is itself publicly documented in the commit history and in this section. The discipline is forcing-function-grade, not real-time-grade.
...
**Reading frame.** The case study divides cleanly into two phases. **The DPG era (Stages 1–8) was learning the science**: methodology was developed, the substrate-sensitivity discipline that produced this paper's contribution was discovered in the gap between the v0_2 and v0_8 polygon sets, and the four measurement layers were calibrated against each other. **Canonical recomputation (Stages 9–10 and post-Stage-10 cleanup) is engineering the exam**: the same methodology applied to authoritative inputs, with the §4.1.4 sunset clause acting as the forcing function that ensured canonical results actually got published rather than DPG-era results becoming load-bearing by inertia. The 17-day window between EA-shapefile receipt and full canonical integration on the Alberta case is the cost of that learning-to-engineering transition; subsequent confirmatory triggers (e.g., the Lunty Nov 2026 map) should resolve faster because no equivalent DPG-mess teardown is required.
...
**Stage 9 — Official Elections Alberta shapefiles (canonical, 2026-05-06).** Canonical `ea_majority_2026_eds.gpkg` and `ea_minority_2026_eds.gpkg` received. All geometry-dependent metrics recomputed. Municipal anchoring: majority 80.0%, minority 71.8% (both within 70–85% Canadian comparator norm; the DPG-era 4.9× below-norm anchoring finding did not survive canonical recomputation). EG Layer 1 (crosswalk, unchanged by canonical shapefiles): majority −0.40%, minority −1.81% (crosswalk convention, negative = UCP-favourable). EG Layer 2 (centroid-in-polygon on canonical shapefiles, formula convention positive = UCP advantage): majority +0.098%, minority +4.02%; inter-map gap +3.92 pp. Both layers agree that the minority map is more UCP-favourable than the majority. The apparent sign difference between the two conventions (negative vs positive) is a convention artefact, not a directional disagreement.

**Stage 10 — Canonical MCMC ensemble (1,010,000-plan, 2026-05-12).** Full 4-chain × 252,500-step ReCom run on canonical shapefiles. GR92 R-hat 1.00179–1.01843 (all four chains pass GR92 < 1.10). Partisan-metric ESS 1,429–1,682 (publication-grade). Minority mean-median at p99.98 (UCP-tail), declination at p1.21 (NDP-tail), seats@50/50 at p99.99. Fisher combined Ch1 × Ch2: p = 6.87×10⁻⁸. *(This stage's ensemble was later found to contain a disclosed chain-1 duplication and, on 2026-07-12, was scrapped and rerun clean — see DOCUMENTED CORRECTIONS C9/C10 in `reports/academic/report_academic.md`. The clean rerun moved these figures to minority mean-median p99.97, Fisher p ≈ 4.61×10⁻⁸; Fisher itself was separately retired as the headline 2026-06-10 for channel-dependence reasons. The values above are preserved as the Stage 10 historical record, not the current canonical figures.)*

**Stage 11 — Phase 4C canonical attribution + Phase 4C re-run (2026-05-18, day 12 of the sunset-clause window).** `packing_cracking_analysis.py` re-run with canonical VA centroid-in-polygon spatial join against `ea_majority_2026_eds.gpkg` and `ea_minority_2026_eds.gpkg`. Headline partisan-metric values locked: minority EG +4.02%, majority +0.10% (gap +3.92 pp); minority mean-median +1.03 pp, majority −3.64 pp (gap +4.66 pp); minority seats@50/50 0.5169 (at p99.99 of canonical ReCom 1M ensemble), majority 0.4607 (at p77.8). The §4.1.4 sunset clause's *headline-recompute* obligation closes at this stage. Per `analysis/methodology/canonical_shapefile_log.md` §1: *"The §4.1.4 sunset clause was triggered on receipt and partially fulfilled: the 2026-proposal-shapefile trigger closed on 2026-05-18 with the Phase 4C canonical re-run."* This is **inside** the two-week window relaxed in Amendment 2 Change 11 (2026-04-26).

**Stage 12 — Full canonical integration including §5.3.5 drain re-run and DPG-mess pruning (2026-05-23, day 17 of the sunset-clause window).** Canonical R `redist` SMC cross-check (2026-05-18). Canonical neighbour-drain adjacency re-run on `ea_*_2026_eds.gpkg` substrate with canonical VA-centroid vote attribution (2026-05-23; minority 1 coupled / majority 2 / 2019 5; ratio 0.50× minority/majority; pre-registered PASS holds, replacing v0_2 0.33× and v0_8 1.33× substrate-sensitive predecessors). DPG-era findings whose claims did not survive canonical recomputation moved to `archive/dpg_era/` with explicit SUPERSEDED banners (municipal anchoring, DA anchoring, natural-anchoring secondary check). Scripts purged of stale DPG-era constants (`estimate_2026`, `MAJORITY_2026_MAPPING`, `MINORITY_2026_MAPPING`) and re-wired onto `score_map_by_spatial_join()` against canonical shapefiles. Repo-wide dependency-declaration convention (`Backward:` / `Forward:` blocks in every file's header) added to make the canonical-vs-DPG-vs-proposal status of every artifact grep-walkable. **This is three days outside the two-week window** the audit's sunset clause specifies; the overshoot is disclosed here, in `analysis/methodology/canonical_shapefile_log.md`, and in `findings/pre_registration_amendment_log.md`. The honest framing: the *headline-recompute* obligation closed at Stage 11 inside the window; the *full-integration* obligation closed at Stage 12 three days outside it. A reviewer can verify both timestamps in the commit log.
