...
| RT7 — MCMC neutral-ensemble outlier | Any real map outside 5–95 band on ≥1 metric for flagged pass | **Canonical-substrate refresh 2026-06-12 (T1.7 Ref #17 C1):** under the 1,010,000-plan canonical run (§5.4.9), Majority MM p0.85 (NDP-tail) + EG p15 + s50 p17–p23 (band); Minority MM p99.98 + Decl p98.79 (UCP-tail under Amendment 10) + s50 p99.99 + EG p94.4 (borderline). Earlier Run #3 (250k, v0_7 centroid) numbers are superseded by the canonical ensemble; the gate's earlier "pending" status is closed. | **Flagged pass on minority on 4 of 4 partisan metrics (post-Amendment-10) plus MM tail flag on majority; canonical 1.01M ensemble.** |
...
**Warrington's declination is** δ = (2/π)(θ_R − θ_D). Positive δ indicates the party indexed as R wins its seats more efficiently (smaller winning margins clustered nearer the 50 % threshold) than the party indexed as D, which is mathematically equivalent to packing of party D.

**Convention used throughout this audit (consistent with `analysis/scripts/mcmc_ensemble.py:789`):** R = UCP and D = NDP, so **positive δ = UCP-favourable boundary geometry (NDP packed); negative δ = NDP-favourable boundary geometry (UCP packed).** This is Warrington's original convention applied with R = the dominant party in the modelled election (UCP in 2023 Alberta), and it is the convention preserved in every chain CSV, every real-map JSON, and every reported percentile in this audit. There is *no* sign-flip relative to Warrington (2018).

```python
# Discrete computation (chain code reference: mcmc_ensemble.py:215)
...
declination = (2 / math.pi) * (theta_R - theta_D)
```
...
|---|---|---|
| 2019 enacted | −0.045 | — (baseline; recomputed) |
| Majority 2026 | **+0.027** | p79.6 (UCP-favoured side of median, within null) |
| Minority 2026 | **−0.077** | p1.21 (NDP-favoured tail) |

The minority's δ being negative means that on declination alone the minority's *winning-margin geometry* favours the NDP — its NDP-won districts are unusually compressed near 50 % (tight margins, low share). **This direction disagrees with the minority's EG (UCP-favoured) and MM (UCP-favoured) readings, by design**: declination measures asymmetry in *winning-margin compression* rather than wasted-vote balance, and is therefore the metric that activates under asymmetric packing — packing one party so they win their seats by tight margins. Warrington (2018) and Katz, King & Rosenblatt (2020) both document the cross-metric tension as expected under asymmetric-packing maps. Both metrics are retained; see §5.2.4 for the full reconciliation.

**Substrate-iteration history (see also `findings/post_audit_recompute_deltas.md`).** The minority's canonical declination changed value during the canonical recomputation: −0.0666 (v0_8 DPG substrate) → +0.0105 (canonical first-pass, before the corrected Vote-Anywhere attribution landed) → **−0.0770 (final canonical with corrected attribution)**. The transient sign flip in the first-pass canonical reflects ~9.4% of votes whose attribution method changed between iterations; the final canonical attribution restored the v0_8-direction reading. This is a *substrate* sign change (an attribution refinement), not a *convention* sign change; Warrington's convention has been preserved throughout. The earlier §6.2 reading of "p62.2, near median" derives from the transient first-pass value and is superseded by the final canonical p1.21 reading.
