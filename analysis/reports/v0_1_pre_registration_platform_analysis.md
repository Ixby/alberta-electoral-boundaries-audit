...
forward_dependencies:
  - analysis/reports/v0_1_pre_registration_draft.md (the submission-ready document itself)
...
backward_dependencies:
  - analysis/reports/v0_1_track_c_checklist_baseline_scoring.md (test list, thresholds, pass/fail criteria)
...
**Scope:** Platform survey only. The pre-registration document itself is drafted in `analysis/reports/v0_1_pre_registration_draft.md`; actual submission is reserved for the PO.
...
The pre-registration draft is `analysis/reports/v0_1_pre_registration_draft.md`. The PO retains submission authority. The steps below assume OSF as the chosen platform per the recommendation above. Estimated wall time for a first-time user: 30–45 minutes.

### Pre-submission checklist

- [ ] Confirm the draft document (`analysis/reports/v0_1_pre_registration_draft.md`) is final. Any edit after OSF submission creates a new version; the original timestamp is preserved but subsequent edits are visible.
...
3. **Upload the pre-registration document.**
   - In the project's Files tab, upload the PDF export of `analysis/reports/v0_1_pre_registration_draft.md`.
   - Also upload the markdown source for reproducibility.
   - Optional: upload a copy of `analysis/reports/v0_1_track_c_checklist_baseline_scoring.md` as the baseline scorecard referenced in §3.
...
- Checklist content source: `analysis/reports/v0_1_track_c_checklist_baseline_scoring.md` and `report_public.md` §"What a gerrymander in the 91-seat map would actually look like".
