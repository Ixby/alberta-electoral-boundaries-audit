...
**AI-provenance manifest (added in session 9 per B6 fortification).** Following emerging disclosure norms for AI-assisted scholarship (*Nature* 2023 AI policy; Thorp 2023 in *Science*; ICML and ICLR 2024 author guidelines; ACM 2023 policy on AI-generated content; Lin 2023 on AI-in-research), this audit labels each output category by authorship provenance. **AI-authored, human-verified:** all quantitative findings in §§2–3, the sensitivity tables, the signature-detection numerics, the counter-test results in §3.12, the cycle-lag analysis, the Plan B cross-check, the Track C baseline scorecard, and the 338Canada cross-validation. Each of these was produced by scripts (`analysis/v0_2_packing_cracking_analysis.py`, `analysis/electoral_forensics_population.py`, etc.) or sub-agent pipelines whose prompts and code are in the repository. A human author reviewed each output against the inputs and the stated criteria. **AI-drafted, human-verified:** all prose in §§1, 2, 3, 4, 5, 6, 7, and this preamble. The human author reviewed each paragraph for house-voice compliance, numerical accuracy against the verified outputs, and citation integrity. **Human-authored:** the Disclosure statement at the top of this paper, the author-bias acknowledgement in §1.4, and editorial judgment calls about section ordering and claim-scoping. **Sub-agent prompts archived:** every sub-agent invocation used in session 9 (Tracks K, L, M, N, O, P, Q, R, S, the A4 cascade, Calgary audit, byelection audit) preserved its prompt verbatim in the spawning message; these prompts are reproducible from the session's conversation history. The audit does not claim any section of this paper is fully independent of AI assistance; it claims every quantitative finding traces to named inputs and public code, and every prose paragraph was verified by a human author before publication.
...
None of the efficiency-gap values cross the 7% threshold from *Gill v. Whitford* (2018). Across the full sensitivity range tested in §3.4 (urban weight 0.60–0.80), the minority-majority asymmetry is negative in every setting: −1.36 pp at 0.60, −0.51 pp at 0.70, −1.61 pp at 0.80. The sign is weight-invariant; the magnitude is not. A comparative base rate for inter-map partisan-asymmetry magnitude in Canadian provincial redistribution does not exist in the published literature; the closest available benchmark is Stephanopoulos & McGhee's (2018) US-state inter-map range of approximately 0.5–4 % EG asymmetry. Alberta's 0.5–1.6 pp range sits at the low end of that US benchmark. A first catalogue of Canadian inter-map EG asymmetries (federal 2022, BC 2023, Saskatchewan 2022, plus historical Alberta cycles) is listed in `data/v0_1_canadian_redistribution_base_rate.csv`; per-cycle quantification is flagged as a follow-up project because each cycle requires 4–8 hours of spatial-crosswalk reconstruction. **Report the weight-conditional range, not the 0.70 point estimate, as the paper's headline.** The 0.70 value is a modelling convention for hybrid-district composition (urban core 70% / rural absorption 30%) applied symmetrically to both 2026 maps; it is not an empirical claim about Election Day vs Vote Anywhere apportionment, which observed at 52.8 / 42.9 in 2023.
...
The paper reports stress-test outcomes against the gates RT1–RT6 specified in the v1.2 prompt. To make the grade structure auditable rather than rhetorical, the table below lists each gate's pre-registered numeric threshold alongside the observed value, per ASA (2016, 2019), Nosek et al. (2018), and Munafò et al. (2017) guidance on graded evidence reporting.
...
| RT6 — Assumption inventory | Listed in `v0_1_uncertainty_and_shapefile_impact.md` | Updated through v0.13 | Pass |
...
**Two-way reading of the Saskatchewan Reference.** The "effective representation" standard established in *Reference re Provincial Electoral Boundaries (Saskatchewan)* [1991] 2 SCR 158 is permissive on deviation from population equality: McLachlin J (as she then was) wrote that the guarantee of §3 is "the right to effective representation," not mathematical equality of voting power (para. 26), and that "relative parity of voting power" must be weighed against other factors "including geography, community history, community interests and minority representation" (para. 33). The same case that would ground a legal challenge to an engineered boundary also grounds a defence of deviation on rural-representation policy grounds; *Raîche v. Canada (Attorney General)*, [2004] FC 679, and *Cassista v. Canada (Attorney General)*, 2014 FC 398, apply the standard to specific boundary disputes without producing a bright-line ceiling on partisan-asymmetric outcomes. The audit's §11 framing (the evidence supports but does not prove a Saskatchewan Reference violation) is therefore intentionally two-sided: the same legal standard that the audit's findings implicate also permits the minority commissioners to invoke it for their configurations. Whether the effective-representation balance has been properly struck by either 2026 proposal is a judgment reserved to the courts.
...
**Reproducibility artifacts added in session 9 (C9 defence).** A version-pinned environment manifest (`requirements.txt` at repo root) lists every Python package with exact version; interpreter pin (`setup.md`) names Python 3.14.3 as the tested version; `FROZEN_MANIFEST.md` lists every external URL accessed during session 9 with its access date (28 URLs verified 2026-04-22). A third party running the pipeline 12+ months from today can (a) install the pinned environment, (b) check each URL's state against the frozen snapshot, and (c) reproduce every numeric finding to the tolerance stated in Gate G0. Reproducibility-artifact provenance follows the ICLR 2022 Reproducibility Checklist and Nosek et al. (2018) Open Science Framework preregistration standards.

**Sub-agent prompts archive (C6 defence).** The session 9 pipeline used sub-agents for parallel analysis; each sub-agent prompt is preserved verbatim in `analysis/v0_1_subagent_prompts_appendix.md`. A reviewer can inspect every prompt, verify the output the sub-agent produced from the prompt, and re-run the prompt against a fresh sub-agent instance.
...
- [Sub-agent prompts archive](analysis/v0_1_subagent_prompts_appendix.md) — C6 provenance evidence
