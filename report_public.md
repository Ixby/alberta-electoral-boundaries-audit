...
One number signalled that the 2026 minority commission map was impossibly gerrymandered. Another said it was perfectly legal. A third said the bias didn't have to be that extreme. A fourth said the map had elements of fairness. When the numbers don't agree, you keep asking. So this audit's author ran the standard 100,000-map computer simulation; got conflicting signals; ran 250,000; ran 1,000,000; ran 2,000,000 — and the apparent extremity grew more improbable rather than more clear. Improbability that doesn't resolve under more data is a signal that the question is being asked wrong.
...
| Seats at 50/50 votes (percentile in 100k simulation) | 46.1% — p77 (normal range) | **48.3% — p98.6 (top 1.5%)** |
...
The same picture in numbers — each map's efficiency gap, scored on the topologically-perfect v0_9 substrate and placed in the audit's corrected 100,000-map simulation:

| Map | Efficiency gap (v0_9) | Above Alberta line (~4%)? | Above US line (7%)? | Percentile in the 100k simulation |
|---|---|---|---|---|
...
**The percentile readings are point estimates, not bounds.** The simulation runs as four parallel chains of 25,000 maps each (100,000 total), giving the percentile placements roughly ±1 percentile-point precision in the right tail.
...
To answer this, the audit generated 100,000 computer-simulated, mathematically neutral Alberta maps, holding to the exact same statutory rules and geographic boundaries the commission used. We then placed the commission's two 2026 maps into that distribution to see how normal they are.
...
The full distribution from the corrected 100,000-map simulation:

| Where the map sits | UCP seats at 50/50 votes |
|---|---|
| Median Alberta map | 44.8% — NDP slight seat majority |
| 95th-percentile map | 48.3% — still UCP minority |
| 99th-percentile map | 49.4% — just shy of half |
| **Maximum across 100,000 maps** | **50.6%** |
...
| **Minority 2026** | **48.3%** | **98.57th percentile — top 1.5% of mathematical possibility** |

*Out of 100,000 neutral maps, only 1,426 — about 1.5% — pushed the tipping-point advantage this far.* The majority map is the kind of map a neutral procedure routinely generates. The minority map is the kind of map you have to specifically aim to draw.

#### What this means in plain language

The minority commission map is not a sloppy, widespread gerrymander. It is a *surgical fortification of the tipping point*. The mapmakers didn't waste time manipulating safe rural seats or deep-urban strongholds — which is why the global efficiency gap looks normal. Instead, they executed a targeted strike exactly where it matters: the marginal, 50/50 hybrid districts.
...
|---|---|---|
| Median `seats@50/50` across the ensemble | 44.8% | 46.0% |
| Maximum reached | 50.6% | 52.9% |
| Fraction of plans reaching the minority's 48.3% | **1.4%** (1,426 of 100,000) | **5.6%** (importance-weighted) |

Under ReCom (which builds compact maps), the minority map's 48.3% is a top-1.5% outlier. Under SMC (which doesn't enforce compactness as strongly), it is reached by 5.6% of plans — about *four times more often than under ReCom*.

This is not a contradiction. It is a discovery. Here is what the disagreement means.

ReCom builds plans by recursively merging districts and re-splitting them along randomly-grown spanning trees — a procedure with a known empirical bias toward *compact* maps (see Wong, Cannon, et al. 2024). The audit's measured ReCom Polsby-Popper distribution confirms it: across 10,000 ReCom-generated plans, mean Polsby-Popper compactness sits in a tight band between 0.238 (5th percentile) and 0.265 (95th percentile) — the sampler is effectively a compactness straitjacket. SMC sequentially grows plans from scratch with importance weighting — a procedure that does not have the same compactness preference. The minority map is *explicitly non-compact*: chair-flagged lasso corridors, district extensions into uninhabited national park land, four-way urban splits, urban-rural hybrid configurations.

If the minority's 48.3% were reachable inside the compact slice of the legal-map space, ReCom would land there routinely — and SMC and ReCom would agree. They don't. ReCom can barely reach 48.3% (1.4% of its draws), while SMC reaches it 5.6% of the time. **The roughly 4× gap between the two samplers is the empirical signature of the geometric route the minority commissioners had to take to reach 48.3%.** Compact, conventional boundary lines very rarely deliver that seat advantage; the unusual non-compact shapes the chair flagged deliver it more reliably.

**A falsification test the audit set up against itself.** The natural follow-on hypothesis was: *the SMC plans that reach the minority's value should be the less-compact ones; that would be hard proof that breaking compactness is the mechanism through which the minority commissioners achieved their advantage.* The audit ran the test directly. Across 5,000 R-SMC plans, the 2,762 that reach `seats@50/50` ≥ 0.4831 had mean Polsby-Popper compactness of 0.2391; the other 2,238 plans had 0.2339. The high-UCP-advantage plans were *very slightly more compact* than the rest, not less — the opposite of what the simple "non-compact geometry is the load-bearing mechanism" story predicts. Welch t-test p-value 7.7 × 10⁻²³⁴: the small effect is statistically rock-solid in the wrong direction. The two-sampler disagreement therefore *cannot* be explained by "SMC explores less-compact territory and finds the minority's value there." Some other geometric or graph-structural feature distinguishes the maps SMC reaches from the maps ReCom reaches, and the audit does not currently know which one. Candidates worth investigating: the *minimum* per-district compactness inside a plan (the "weakest district" rather than the average), city-cracking patterns specifically, urban-rural hybridization configurations, or cut-edge density. (Full falsification write-up: [`analysis/reports/redist_python_comparison.md`](analysis/reports/redist_python_comparison.md).)

**What survives the falsification.** What the audit *can* still claim, after the test: the minority map's `seats@50/50` is reached more often by SMC than by ReCom (the ratio depends on PRNG-state-stability in the SMC run; across three R runs with the same nominal seed it ranged from 1.4× to ~40×). That two-sampler gap *is* a real signal that something non-trivial about the legal-map space distinguishes ReCom-reachable maps from SMC-reachable maps. What the audit *cannot* claim, after the test, is that compactness specifically is that something. The Lane 2 structural-irregularity findings (chair-flagged lasso shapes, the four-way Airdrie split, the urban-hybridization pattern, the 15% municipal anchoring) remain the audit's strongest evidence — they don't depend on any sampler. The Lane 1 percentile under ReCom (98.6) remains a defensible single-sampler statement.

The strongest honest framing is that *Lane 2 carries the case*, and the Lane 1 sampler-disagreement is a methodological-honesty finding rather than a load-bearing piece of the central argument.
...
**Why Lane 2 carries the case.** The audit pre-registered five structural-irregularity tests on March 14, 2026, before the commission filed its report. The minority crosses every one of them; the majority crosses none. Those measurements are geometric — they don't depend on which statistical sampler one uses or which election one scores against. Lane 1 (the partisan-fairness numbers) corroborates Lane 2: under the Python ReCom ensemble the minority's `seats@50/50` sits at the 98th percentile (top 1.5%), under the R SMC ensemble it is reached more often than ReCom would predict, and the audit ran a falsification test against itself to check whether the simple "non-compact geometry is the load-bearing mechanism" story explains the gap — it does not, or at least not at the per-district mean compactness level the test measured. Lane 1 therefore stops being the central finding the audit defends. *Lane 2 is the central finding.* Lane 1 reports what the partisan-fairness numbers do under the corrected pipeline; the falsification write-up at [`analysis/reports/redist_python_comparison.md`](analysis/reports/redist_python_comparison.md) reports what the audit ran to test its own working hypothesis and what survived.
