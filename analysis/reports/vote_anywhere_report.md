...
Computed from `alberta_2023_results.csv` (`total_valid_votes` column, 87 EDs):
...
The Election Day vote total was estimated from the VA-level spatial attribution file (`votes_2023_majority.csv`), which aggregates VA-polygon Election Day votes per 2026 proposed ED. After excluding two known data-quality outliers (Edmonton-Enoch: 33,176 attributed from over-extended polygon; Edmonton-West Henday: disaggregated into Enoch sub-area), the province-wide Election Day vote total is approximately:
