...
def count_splits(csds: gpd.GeoDataFrame, eds: gpd.GeoDataFrame,
                 label: str, min_overlap_m2: float = 50000.0) -> pd.DataFrame:
    """
...
def count_splits_2019(csds: gpd.GeoDataFrame, va: gpd.GeoDataFrame,
                       min_overlap_m2: float = 50000.0) -> pd.DataFrame:
    """
    ...
    joined = gpd.sjoin(va_c[["va_ucp", "va_ndp", "parent_ed_2019", "geometry"]],
...
def va_votes_per_csd(csds: gpd.GeoDataFrame, va: gpd.GeoDataFrame) -> dict:
    ...
    joined = gpd.sjoin(va_c[["va_ucp", "va_ndp", "geometry"]],
                       csds_proj[["CSDNAME", "geometry"]],
                       how="left", predicate="within")
    agg = joined.groupby("CSDNAME").agg(
        total_votes=pd.NamedAgg(column="va_ucp",
...
def main():
    ...
    sig_csds = csds[
        (csds["CSDTYPE"].isin(INCLUDE_TYPES)) &
        (csds["va_votes"] >= MIN_VOTES)
    ].copy()
    print(f"  Significant municipalities: {len(sig_csds)} "
    ...
    # Merge
    df = df_2019.merge(df_maj[["csd_name","n_eds_maj","eds_maj"]],
                       on="csd_name", how="outer")
    df = df.merge(df_min[["csd_name","n_eds_min","eds_min"]],
                  on="csd_name", how="outer")
    ...
    print(f"  Majority 2026: {split_maj} municipalities split "
          f"({'+'  if split_maj>=split_2019 else ''}{split_maj-split_2019:+d} vs 2019)")
    print(f"  Minority 2026: {split_min} municipalities split "
    ...
    print(changed[["csd_name","csd_type","va_votes",
                    "n_eds_2019","n_eds_maj","n_eds_min",
                    "delta_maj","delta_min"]].to_string(index=False))
    ...
    for _, r in worse_maj.iterrows():
        print(f"  {r['csd_name']:30s} 2019={r['n_eds_2019']} → maj={r['n_eds_maj']} "
              f"(+{r['delta_maj']:.0f})")
    print(f"\nNew splits introduced by minority 2026: {len(worse_min)}")
    for _, r in worse_min.iterrows():
        print(f"  {r['csd_name']:30s} 2019={r['n_eds_2019']} → min={r['n_eds_min']} "
              f"(+{r['delta_min']:.0f})")
    ...
    out = {
        "summary": {
            ...
            "majority_2026": {"n_splits": int(split_maj), "delta_vs_2019": int(split_maj - split_2019)},
            "minority_2026": {"n_splits": int(split_min), "delta_vs_2019": int(split_min - split_2019)},
        },
        "new_splits_majority": worse_maj[["csd_name","csd_type","va_votes",
                                          "n_eds_2019","n_eds_maj"]].to_dict("records"),
        "new_splits_minority": worse_min[["csd_name","csd_type","va_votes",
                                          "n_eds_2019","n_eds_min"]].to_dict("records"),
    }
    ...
    if len(worse_maj):
        ...
        md.append("|---|---|---|---|---|")
        for _, r in worse_maj.iterrows():
            md.append(f"| {r['csd_name']} | {r['csd_type']} | "
