...
| Test | What it measures | Source | Script |
|---|---|---|---|
| B1 | Vote-distribution histogram across 10 margin bins | descriptive | `packing_cracking_analysis.py` |
...
Community-of-interest is the hardest Canadian-charter criterion to operationalise. Pure perimeter-anchoring (C5 municipal+DA) is the cleanest measurable proxy: Canadian commissions have historically preserved community-of-interest by snapping boundaries to municipal edges where population allows. The rationale for selecting this test was that a map outside the 70–85% Canadian comparator norm would be a measurable structural departure. On DPG geometry this test fired in a strong direction (DPG-era majority 71 % / minority 14.5 %, a 4.9× asymmetry that would have been a clear signal); on canonical Elections Alberta shapefiles the same test produces majority 80.0 % / minority 72.0 %, both inside the norm, so the DPG-era result was **retracted** and the test now contributes no signal to the Lane 2 case (see §4.1.4 DPG sunset clause and academic-report §5.8.5). The test-selection rationale (anchoring as a measurable community-of-interest proxy) remains methodologically sound; the substantive finding it produced did not carry across substrates.
...
- **Municipal anchoring (2e6c351, Issue #4) — RETRACTED on canonical geometry** — DPG-era result was 71.0 % maj / 14.5 % min to CSD edges (4.9× asymmetry); canonical result on official EA shapefiles is 80.0 % maj / 72.0 % min, both within the 70–85 % Canadian comparator norm. Retracted per §4.1.4 DPG sunset clause; see §5.8.5
- **DA-boundary anchoring (f319960) — SUPERSEDED on canonical geometry** — DPG-era extension reported +7.7 pp maj / +6.6 pp min to DA edges and a 5.1× asymmetry ratio "preserved"; carried the same DPG substrate as the retracted municipal-anchoring headline and did not survive canonical recomputation
