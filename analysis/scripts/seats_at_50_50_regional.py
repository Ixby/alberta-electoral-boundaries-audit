...
-----
    ...
    PYTHONIOENCODING=utf-8 python analysis/scripts/seats_at_50_50_regional.py \\
        --all-three   # runs 2019 enacted, canonical majority, canonical minority
...
VA_PATH_ELECTION_DAY = DATA / "shapefiles" / "derived" / "va_polygons_with_2023_votes.gpkg"
VA_PATH_FULL = DATA / "shapefiles" / "derived" / "va_polygons_with_full_2023_votes.gpkg"
# election-day only cols (consistent with canonical MCMC ensemble, total ~932k votes)
VA_UCP_COL_ED = "va_ucp"
VA_NDP_COL_ED = "va_ndp"
# full-vote cols including advance/special ballots apportioned back to VAs (~1.71M votes)
VA_UCP_COL_FULL = "va_ucp_full"
VA_NDP_COL_FULL = "va_ndp_full"
RESULTS_2019 = DATA / "reference" / "alberta_2019_results.csv"
RESULTS_2023 = DATA / "reference" / "alberta_2023_results.csv"

# Canonical Elections Alberta shapefiles (active inputs for all analysis)
...
def load_va_with_regions(full_votes: bool = False) -> (
    ...
    full_votes=False  → election-day only (va_ucp/va_ndp, ~932k votes;
                        consistent with the canonical 1M MCMC ensemble)
    full_votes=True   → all votes including advance/special apportioned back
                        to VAs (va_ucp_full/va_ndp_full, ~1.71M votes;
                        consistent with the SZAT Ch2 bootstrap substrate)
    """
    va_path = VA_PATH_FULL if full_votes else VA_PATH_ELECTION_DAY
    ucp_col = VA_UCP_COL_FULL if full_votes else VA_UCP_COL_ED
    ndp_col = VA_NDP_COL_FULL if full_votes else VA_NDP_COL_ED
...
def main(argv=None):
    ...
    p.add_argument(
        "--shapefile",
        type=Path,
        help="Path to a candidate-map gpkg.",
    )
    p.add_argument("--id-col", default=None,
                   help="District name column (default: EDName2025 for canonical, name_2026 for legacy).")
    p.add_argument(
        "--canonical",
        action="store_true",
        help="Score 2019 enacted, canonical EA majority, canonical EA minority (official shapefiles).",
    )
    p.add_argument(
        "--all-three",
        action="store_true",
        help="Score 2019 enacted, v0_9/DPG majority, v0_9/DPG minority (legacy; for historical comparison only).",
    )
    p.add_argument("--full-votes", action="store_true",
                   help="Use full advance-vote substrate (va_ucp_full/va_ndp_full, ~1.71M votes). "
                        "Default: election-day only (~932k votes, consistent with canonical MCMC).")
    p.add_argument("--output", type=Path, help="Write JSON results here.")
    p.add_argument("--verbose", action="store_true")
    args = p.parse_args(argv)

    if not args.shapefile and not args.all_three and not args.canonical:
        p.error("provide --shapefile PATH, --canonical, or --all-three")
    ...
    vote_substrate = "full (~1.71M votes incl. advance)" if args.full_votes else "election-day only (~932k votes)"
    print(f"Vote substrate: {vote_substrate}")
    va, ucp, ndp, region = load_va_with_regions(full_votes=args.full_votes)
    results = []
    ...
    if args.canonical:
        ...
        for path, label in [
            (MAJ_CANONICAL, "canonical_majority"),
            (MIN_CANONICAL, "canonical_minority"),
        ]:
            asg = _district_assignment_from_polygons(va, path, id_col=CANONICAL_ID_COL)
    ...
    if args.all_three:
        ...
        for path, label in [(MAJ_V9, "v0_9_majority"), (MIN_V9, "v0_9_minority")]:
            ...
            results.append(score_one(va, ucp, ndp, region, ratios, asg, label))
    if args.shapefile:
        id_col = args.id_col or CANONICAL_ID_COL
        asg = _district_assignment_from_polygons(va, args.shapefile, id_col=id_col)
        results.append(
