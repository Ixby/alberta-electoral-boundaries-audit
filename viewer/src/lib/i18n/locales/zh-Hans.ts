...
export default {
	...
	editorial_intro: {
		...
		p3: '那是标准程序。这一次,标准程序产生了不寻常的结果。委员会的五名成员就地图应是什么样子分裂成了3比2,与其达成一项建议,他们提出了两份:一份<em>多数派提议</em>(由三位委员支持)和一份<em>少数派提议</em>(由两位支持)。两份都摆在桌上——但立法机构已将两份都搁置。根据政府第37号议案(2026年4月21日),一个由Brandon Lunty主持的独立MLA委员会——由省长为此特定决定任命——现监督一个由四人组成的独立咨询小组,负责制定一份全新的91席选区地图。立法机构必须在2026年11月之前批准该委员会提交的方案。',
