...
export default {
	...
	editorial_intro: {
		...
		p3: '那是标准程序。这一次,标准程序产生了不寻常的结果。委员会的五名成员就地图应是什么样子分裂成了3比2,与其达成一项建议,他们提出了两份:一份<em>多数派提议</em>(由三位委员支持)和一份<em>少数派提议</em>(由两位支持)。两份都摆在桌上。一个由Brandon Lunty主持的独立MLA委员会——由省长为此特定决定任命——正在两者之间作出选择。立法机构必须在2026年11月之前批准在该委员会中存活下来的那一份。',
