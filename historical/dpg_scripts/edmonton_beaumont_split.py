...
# --- 2019 populations ---
pop19_path = os.path.join(DATA, "alberta_2019_populations.csv")
pop19 = pd.read_csv(pop19_path)
print(f"\nalberta_2019_populations.csv columns: {list(pop19.columns)}")
...
print(
    pop19[pop19[name_col_19].str.contains("Edmonton-South", na=False)].to_string(
        index=False
    )
)
...
pop26_path = os.path.join(DATA, "majority_2026_populations.csv")
pop26 = pd.read_csv(pop26_path)
print(f"\nmajority_2026_populations.csv columns: {list(pop26.columns)}")
...
print(
    f"Edmonton-Beaumont 2026 population target: {eb_26_pop:,}  (crosscheck: {EB_2026_POP:,})"
)

# --- Derived fractions ---
beaumont_from_es = EB_2026_POP - BEAUMONT_CITY_POP  # the Edmonton-South portion in EB
es_26_implied = (
    es_19_pop - beaumont_from_es
)  # what ES-2026 should be if split correctly
...
print(
    f"  Fraction of ES-2019 VAs to reassign to EB     : {pop_fraction:.4f}  ({pop_fraction*100:.2f}%)"
)
...
if HAS_GEOPANDAS:
    ...
    try:
        ...
        total_es_votes = (
            es_vas["va_ndp"] + es_vas["va_ucp"] + es_vas["va_other"]
        ).sum()
        ...
        es_vas_sorted = es_vas.sort_values("centroid_y", ascending=True).reset_index(
            drop=True
        )
        print(
        ...
        print(
            f"  Northernmost VA centroid y: {es_vas_sorted['centroid_y'].iloc[-1]:.1f}"
        )
...
if not HAS_GEOPANDAS:
    # ── Fallback: reconstruct VA list from assignment_va_to_2026_assignments.csv
    print(f"\n[FALLBACK] Building Edmonton-South VA list from assignments CSV")
    assign_path = os.path.join(ANALYSIS, "assignment_va_to_2026_assignments.csv")
...
totals_path = os.path.join(ANALYSIS, "assignment_2026_synthetic_totals.csv")
baseline_all = pd.read_csv(totals_path)
print(f"\nassignment_2026_synthetic_totals.csv: {len(baseline_all)} rows")
...
print(
    f"Baseline Edmonton-South   : NDP={es_baseline['ndp']:.1f}, UCP={es_baseline['ucp']:.1f}, "
...
verdict_text = (
    ...
    if max_delta < 0.10
    else "The maximum absolute EG shift across the entire sweep "
...
Canonical files (`assignment_2026_synthetic_totals.csv`, `.gpkg` shapefiles)
were not modified.*
"""
