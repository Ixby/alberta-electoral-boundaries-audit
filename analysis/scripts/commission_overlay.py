...
def render_our_map(ax, gdf: gpd.GeoDataFrame, extent: tuple, title: str):
    ...
    if not gdf.empty:
        gdf.plot(ax=ax, facecolor="#dcd4c4", edgecolor="#2a2a2a",
...
def render_overlay(ax, img: np.ndarray, gdf: gpd.GeoDataFrame,
                   extent: tuple, title: str, png_wh: tuple[int, int]):
    ...
    if not gdf.empty:
        ...
        gdf.boundary.plot(ax=ax, color=BOUNDARY_COLOR,
                          linewidth=BOUNDARY_LW, zorder=2)
...
def process_spec(spec: MapSpec, plans: dict[str, gpd.GeoDataFrame],
    ...
    render_our_map(ax_our, gdf, spec.extent,
                   f"Our {version_label} boundaries\n({spec.label})")
    ...
    render_overlay(ax_ov, img, gdf, spec.extent,
