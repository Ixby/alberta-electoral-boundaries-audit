...
def render_event(event, eds_2019, eds_2026):
    ...
    ax2.set_title(f"2. Overlay ({etype} Effect)", fontsize=20, weight='bold', color='purple')
    ...
    if etype == 'Cracking':
        ...
        gpd.clip(cutters, target).plot(ax=ax2, facecolor='none', edgecolor='black', linewidth=4, linestyle='--')
    elif etype == 'Draining':
        target = eds_2019[eds_2019['name'] == title_target]
        target.plot(ax=ax2, facecolor='#cce5ff', edgecolor='blue', linewidth=2)
        swallower = eds_2026[eds_2026['name'] == event['recipient_2026']]
        gpd.clip(swallower, target).plot(ax=ax2, facecolor='#ffcc99', edgecolor='orange', linewidth=4, hatch='///')
        swallower.plot(ax=ax2, facecolor='none', edgecolor='orange', linewidth=3, linestyle='-.')
    elif etype == 'Packing':
        donors = eds_2019[eds_2019['name'].isin(event['donors'])]
        donors.plot(ax=ax2, facecolor='#ffcccc', edgecolor='red', linewidth=2)
        sink = eds_2026[eds_2026['name'] == title_target]
        sink.plot(ax=ax2, facecolor='none', edgecolor='black', linewidth=4, linestyle='--')
